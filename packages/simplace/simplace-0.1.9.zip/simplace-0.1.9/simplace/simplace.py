...
**Example 2** - *Running a solution with changed simulation parameters:*
    ...
    >>> simplace.runSimulation(sh)
    >>> result = resultToList(getResult(sh,'DailyOutput',simid))
    >>> simplace.closeProject(sh)
...
def resultToList(result, expand = True, start=None, end=None):
    ...
    return {names[i]:_objectArrayToData(obj[i], types[i], expand) 
...
def varmapToList(result, expand = True):
    ...
    obj =  result.getDataObjects()
    types = result.getTypeStrings()
    return {names[i]:_objectToData(obj[i], types[i], expand) 
                for i in range(0, len(names))}
...
def getUnitsOfResult(result):
    ...
    return {names[i]:units[i] for i in range(0,len(names))}
...
def _objectArrayToData(obj, simplaceType, expand = True):
    ...
    if (simplaceType in ['DOUBLE','INT','BOOLEAN']):
        return [o for o in jpype.JClass(au).toPrimitive(obj)]
    elif expand and simplaceType in ['DOUBLEARRAY','INTARRAY']:
        return [[o for o in jpype.JClass(au).toPrimitive(row)] for row in obj]
    elif simplaceType in ['CHAR','DATE']:
...
def _parameterListToArray(parameter):
    if parameter is None:
        ...
        return jpype.JArray(jpype.java.lang.Object, 2)(
          [[key, _getScalarOrList(parameter[key])] for key in parameter.keys()])
