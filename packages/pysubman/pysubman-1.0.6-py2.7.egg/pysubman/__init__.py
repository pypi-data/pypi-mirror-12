#!/usr/bin/env python
# coding=utf-8
# flake8: noqa

__version__ = "1.0.6"

from pykafka import KafkaClient

__all__ = ["SubmanConsumer", "SubmanProducer", "SubmanException"]


class SubmanException(Exception):
    pass


class SubmanConsumer(object):
    def __init__(self, kafka_hosts, zookeeper_hosts=None):
        self.kafka_hosts = kafka_hosts
        self.zookeeper_hosts = zookeeper_hosts
        self.kafka_client = KafkaClient(hosts=self.kafka_hosts)

    def C(self, topic_name, group_id,
          auto_commit_enable=True,
          zookeeper_connect=None, **kafka_kwargs):

        if not zookeeper_connect:
            zookeeper_connect = self.zookeeper_hosts

        if not zookeeper_connect:
            raise SubmanException("please set zookeeper_connect")

        def _consumer(func):
            if topic_name not in self.kafka_client.topics:
                raise SubmanException(
                    """
                        please create topic !
                        ./bin/kafka-topics.sh --create
                            --topic {topic_name}
                            --partitions {partitions}
                            --replication-factor {replication_factor}
                            --zookeeper {zookeeper_client}
                    """)

            kafka_kwargs["auto_commit_enable"] = auto_commit_enable
            kafka_kwargs["zookeeper_connect"] = zookeeper_connect

            self.topic = self.kafka_client.topics[topic_name]
            self.group_id = group_id
            self.consumer_func = func
            self.kafka_kwargs = kafka_kwargs
        return _consumer

    def run(self):
        if not self.topic:
            raise SubmanException("please using @C(topic_name, group_id)")

        balanced_consumer = self.topic.get_balanced_consumer(
            self.group_id, **self.kafka_kwargs)

        while True:
            message = balanced_consumer.consume()
            self.consumer_func(
                message.value,
                topic=self.topic,
                group_id=self.group_id,
                message_partition=message.partition,
                message_partition_id=message.partition_id,
                message_offset=message.offset)


class SubmanProducer(object):
    def __init__(self, kafka_hosts):
        self.client = KafkaClient(hosts=kafka_hosts, use_greenlets=False)
        if not self.client:
            raise SubmanException("connect kafka server{}".format(kafka_hosts))

    def send(self, topic_name, messages):
        if topic_name not in self.client.topics:
            raise SubmanException("kafka topic({}) not exist".format(topic_name))

        kafka_topic = self.client.topics[topic_name]
        with kafka_topic.get_producer() as producer:
            producer.produce(messages)


class SubmanProducer(object):
    def __init__(self, kafka_hosts):
        self.client = KafkaClient(hosts=kafka_hosts)
        if not self.client:
            raise SubmanException("connect kafka server{}".format(kafka_hosts))

    def add_topic(self, topic_name):
        return _ProducerTopic(self.client, topic_name)


class _ProducerTopic(object):
    def __init__(self, producer, topic):
        self.producer = producer

        if topic_name not in self.producer.topics:
            raise SubmanException("kafka topic({}) not exist".format(topic_name))

        kafka_topic = self.client.topics[topic_name]
        self.producer = kafka_topic.get_producer()

    def send(self, messages):
        self.producer.produce(messages)
