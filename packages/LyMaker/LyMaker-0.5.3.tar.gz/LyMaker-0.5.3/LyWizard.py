...
from LyMaker.ScaleUtil import ScaleUtil,Scale
...
class LyWizard(object):
    ...
    def process(self):
        ...
        self.structure =  self._getInput("Enter the structure of your song [ABCBCBCBCBDC]:",self._checkStructure,self._transStructure,"use only A-Z and only so many characters as you have parts")   
        xml = self._asXml()
        outx = None
        try:
    ...
    def _checkDownbeats(self,input):
        if len(input) == 0:
            ...
            beats =  input.split(',')
        if self.eighths > 0:
            if len(beats) == 0 or len(beats) > self.eighths / 2:
                return False
        else:    
            if len(beats) == 0 or len(beats) > self.quarters / 2:
                return False
        for b in beats:
            c = int(b) # test value
        return True
    ...
    def _header(self):
        ...
        self.tempo = self._getInput("Enter the tempo of the song [100]:",self._noCheck,self._transTempo,"Tempo must be between 20 and 200")
        ...
        if sig2 == 8:
            ...
            self.quarters = sig1
            self.eighths = 0
        self.downbeats =  self._getInput("Enter the downbeats of the song [1,3]:",self._checkDownbeats,self._transDownbeats,"enter the downbeats of the song separated by comma,e.g 1,3 for 4/4")   
    ...
    def _part(self,part):
        ...
        if self.quarters == 4:
            ...
            part.percbeat = 0
        if self.chordhelper == False:
            part.progressions = self._getInput("Enter the progressions for the part, notes separated by comma, chords separated by semicolon, bars separated by | no spaces:",self._checkProgressions,self._noAction,"Enter at least one chord, e.g. c,e,g or c,e,g;d,f,a|e,g,b")
        else:
            part.progressions = self._getInput("Enter the progressions for the part, chordnames separated by semicolon, bars separated by | no spaces:",self._checkProgressions,self._transChords,"Enter at least one chord, e.g. c or c;dm|g6/9\nSupported are all chords that are listed in the LyMaker documentation.")
...
if __name__ == '__main__':
    print "LyWizard Version 0.0.1 beta"
    chordhelper = True
    try:
