...
class TestMVPolyRBF(unittest.TestCase):
    ...
    def check_positive_smoothing(self, rbf_class, poly_class):
        ...
        msg = "abs-diff: %f" % abs(f0 - f1).max()
        self.assertTrue(np.allclose(f0, f1, atol=eps),
                        "%s: positive smoothing (%s)" % (rbf.name, msg))
    ...
    # all rbfs need an needs_epsilon attribute (a boolean)

    def check_needs_epsilon_attribute(self, rbf_class):
        x = np.linspace(0, 10, 9)
        f = np.sin(x)
        rbf = rbf_class(x, f)
        self.assertTrue(hasattr(rbf, 'needs_epsilon'))

    def test_rbf_needs_epsilon(self):
        for rbf_class in RBF_CLASSES:
            self.check_needs_epsilon_attribute(rbf_class)
    ...
    # all rbfs need an needs_radius attribute (a boolean)

    def check_needs_radius_attribute(self, rbf_class):
        x = np.linspace(0, 10, 9)
        f = np.sin(x)
        rbf = rbf_class(x, f)
        self.assertTrue(hasattr(rbf, 'needs_radius'))

    def test_rbf_needs_radius(self):
        for rbf_class in RBF_CLASSES:
            self.check_needs_radius_attribute(rbf_class)
    ...
    def check_radius_auto(self, rbf_class):
        ...
        if rbf.needs_radius:
            self.assertTrue(rbf.radius > 0,
                            '%s: non-positive radius' % rbf.name)
    ...
    def check_epsilon_stability(self, rbf_class, poly_class):
        ...
        self.assertTrue(np.abs(f0-x0).max() / np.abs(f-x).max() < 1.2,
                        "%s: default epsilon/radius stability" % rbf.name)
