...
class HashContext(object):
    ...
    def  __init__(self, *args, **kwargs):
        ...
        if len(args) == 1:
            ...
            if ffi.typeof(args[0]) != ffi.typeof("gcry_md_hd_t *"):
                raise TypeError("With only one arg, __init__ must be called with a <gcry_md_ht *> type")
            self.__ctx = args[0][0]
            return
    ...
    def reset(self):
        ...
        algorithms.
        """
        lib.gcry_md_reset(self.get())

    def get(self):
        """
        Get the C-data object representing the context
        """
        return self.__ctx
    ...
    def enable(self, algo):
        ...
        """
        error = ffi.cast("gcry_error_t", 0)
        error = lib.gcry_md_enable(self.get(), self.isvalid(algo))
        if error > 0:
            raise errors.GcryptException(ffi.string(lib.gcry_strerror(error)), error)
    ...
    def setkey(self, key):
        ...
        error = ffi.cast("gcry_error_t", 0)
        error = lib.gcry_md_setkey(self.get(), key_buffer, len(key))
        if error > 0:
            raise errors.GcryptException(ffi.string(lib.gcry_strerror(error)), error)
    ...
    def copy(self):
        ...
        error = ffi.cast("gcry_error_t", 0)
        error = lib.gcry_md_copy(dst_ctx, self.get())
    ...
    def write(self, data):
        ...
        lib.gcry_md_write(self.get(), buffer, len(data))
    ...
    def read(self, algo=0):
        ...
        return ffi.string(lib.gcry_md_read(self.get(), algo))

    def __getattr__(self, attr):
        """
        Let's get some specific attriutes 
        """
        if attr == 'algo':
            # let's get the algorithm used by our hash context
            return ffi.string(lib.gcry_md_algo_name(lib.gcry_md_get_algo(self.get())))

        if attr == 'hashlen':
            return int(ffi.cast("int", lib.gcry_md_get_algo_dlen(lib.gcry_md_get_algo(self.get()))))

        if attr == 'secure':
            valid = lib.gcry_md_is_secure(self.get())
            if valid == 0:
                return False
            return True
