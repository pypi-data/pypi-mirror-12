...
class HMACContext(object):
    ...
    def reset(self):
        ...
        """
        error = ffi.cast("gcry_error_t", 0)
        error = lib.gcry_mac_ctl(self.get(), lib.GCRYCTL_RESET, ffi.NULL, 0)
        if error > 0:
            raise errors.GcryptException(ffi.string(lib.gcry_strerror(error)), error)
    ...
    def write(self, data):
        ...
        error = ffi.cast("gcry_error_t", 0)
        error = lib.gcry_mac_write(self.get(), buffer, len(data))
        if error > 0:
            raise errors.GcryptException(ffi.string(lib.gcry_strerror(error)), error)
    ...
    def read(self):
        ...
        error = lib.gcry_mac_read(self.get(), buffer, length)
        if error > 0:
            raise errors.GcryptException(ffi.string(lib.gcry_strerror(error)), error)
    ...
    def verify(self, data):
        ...
        error = ffi.cast("gcry_error_t", 0)
        error = lib.gcry_mac_verify(self.get(), buffer, self.maclen)
        if error > 0:
            raise errors.GcryptException(ffi.string(lib.gcry_strerror(error)), error)
        return True
    ...
    def __setattr__(self, attr, value):
        if attr == 'key':
            ...
            error = ffi.cast("gcry_error_t", 0)
            error = lib.gcry_mac_setkey(self.get(), value, len(value))
            if error > 0:
                raise errors.GcryptException(ffi.string(lib.gcry_strerror(error)), error)
        ...
        if attr == 'iv':
            ...
            error = ffi.cast("gcry_error_t", 0)
            error = lib.gcry_mac_setiv(self.get(), value, len(value))
            if error > 0:
                raise errors.GcryptException(ffi.string(lib.gcry_strerror(error)), error)
