...
INITIAL_LOG_LEVEL = logging.ERROR
...
class Runner(object):
    ...
    def handler(self, logfile, queue):
        ...
        while True:
            try:
                ...
                empty = True
            if empty and enumeratedMessages:
    ...
    def put(self, **kwargs):
        ...
        if self.burst:
            ...
            if logfile not in self.queue:
                q = Queue()
                self.queue[logfile] = q
                spawn(self.handler, logfile, q)
                Logger.debug('Created queue for %s', logfile)
            # Queue message to handler
            Logger.debug('put %s: put %s', logfile, enumeratedMessages)
            self.queue[logfile].put(enumeratedMessages)
        else:
...
class Scanner(object):
    ...
    def parseDo(self, items):
        ...
        Logger.debug('Do clause, language=%s, burst=%s', language, burst)
    ...
    class Program(object):
        ...
        def __init__(self):
            ...
            burstGroup.add_argument(
                ...
                action='store',
                type=int,
                help='force reporting bursts of BURST seconds together'
            )
            argp.add_argument(
                '--context', '-C',
                action='store',
