...
class MarketoRest(object):
	...
	def get_leads_by_filter_type(self, filter_type, filter_values, fields=None, batch_size=300, next_page_token=None):
		...
		url_params.update(self.token_params)
		...
		if type(filter_values) == list:
			url_params['filterValues'] = ','.join(filter_values)
		else:
	...
	def get_pagination_api_token(self, date=None):
		...
		request_data.update(self.token_params)
	...
	def describe(self):
		...
		resp = self.get_with_handler(url, params=self.token_params)
	...
	def create_or_update_leads(self, leads, lookup_field='email', action='createOrUpdate', async_processing=False, partition_name=None):
		...
		resp = self.post_with_handler(
			'{0}{1}?{2}'.format(self.URL_ENDPOINT, '/rest/v1/leads.json', urllib.urlencode(self.token_params)),
	...
	def associate_lead(self, lead_id, cookie):
		...
		# MINOR: do I need the json dumps here? Can the auth be a param?
		self.post_with_handler('{0}{1}?{2}'.format(self.URL_ENDPOINT, url, urllib.urlencode(self.token_params)), data=json.dumps(post_data))
	...
	def merge_lead(self, winning_lead_id, losing_lead_ids, merge_in_crm=False):
		...
		if len(losing_lead_ids) == 1:
			...
			post_data.update({'leadIds': ','.join(losing_lead_ids)})
	...
	def get_lead_partitions(self):
		return self.get_with_handler(u'{0}{1}'.format(self.URL_ENDPOINT, '/rest/v1/leads/partitions.json'), params=self.token_params)['result']


	def get_list_by_id(self, list_id):
		url = '/rest/v1/lists/{}.json'.format(list_id)
		return self.get_with_handler(u'{0}{1}'.format(self.URL_ENDPOINT, url), params=self.token_params)['result']

	def remove_leads_from_list(self, list_id, lead_ids):
		post_data = {'input': ','.join(lead_ids), '_method': 'POST'}
		url = '/rest/v1/lists/{}/leads.json'.format(list_id)

		resp = self.post_with_handler('{0}{1}?{2}'.format(self.URL_ENDPOINT, url, urllib.urlencode(self.token_params)), data=json.dumps(post_data))

		return resp['result']

	def get_campaign_by_id(self, campaign_id):
		return self.get_with_handler(
			u'{0}{1}'.format(self.URL_ENDPOINT, '/rest/v1/campaigns/{}.json'.format(campaign_id)),
			params=self.token_params
		)['result']
	...
	def get_lead_activities(self, activity_type_ids, next_page_token, batch_size=300, list_id=None):
		...
		return (resp['result'], resp.get('nextPageToken', None))
	...
	def add_leads_to_list(self, lead_ids, list_id):
		...
		"""
		resp = self.post_with_handler(
			'{0}{1}?{2}'.format(self.URL_ENDPOINT, '/rest/v1/lists/{0}/leads.json'.format(list_id), urllib.urlencode(self.token_params)),
