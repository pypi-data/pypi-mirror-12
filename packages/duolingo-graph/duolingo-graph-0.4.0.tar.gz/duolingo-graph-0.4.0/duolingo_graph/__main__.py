...
from .numbers import rank_courses, rank_max_distance, \
...
def print_ranks(ranks, label, labels, fmt):
    ...
    for amt, langs in ranks:
        num = float(amt[0] if isinstance(amt, tuple) else amt)
        print(('  {:' + str(width) + fmt + '}{} {}').format(
            amt,
            labels if num > 1 else label,
...
@rank_by.command(name='max-distance')
@click.pass_context
def max_distance(ctx):
    """Rank by maximum distance to a language."""
    ranks = rank_max_distance(ctx.obj.graph(), ctx.obj.kind)
    print_ranks(
        ranks if ctx.obj.n == 0 else ranks[:ctx.obj.n],
        ' course:', ' courses:', '',
    )
