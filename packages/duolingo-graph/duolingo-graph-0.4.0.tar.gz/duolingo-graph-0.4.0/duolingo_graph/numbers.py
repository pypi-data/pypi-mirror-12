...
class LangDist(namedtuple('LangDist', ('dist', 'lang'))):
    """Tuple of a maximum distance and a target language."""
    def __format__(self, format_spec):
        if format_spec[0:1].isdigit():
            width = int(format_spec)
        else:
            width = 0
        return '{}, {}'.format(self.lang, self.dist).rjust(width)
...
def distance(graph, kind, aggregator=max):
    """
    Figures out the maximum distance along the given paths (learning, speaking,
    ...
    for vertex in graph.vs:
        ...
        name = vertex['name']
        dists[name] = aggregator(
            (
                LangDist(len(path), graph.vs[list(path)[-1]]['name'])
                for path in paths
            ), key=lambda x: x[0]
        )
    return dists
...
def rank_max_distance(graph, kind=CourseType.BOTH):
    """
    Calculates a list of int, list pairs which correspond to the maximum
    distance from the learner of one language to to another language, sorted
    in ascending order by maximum distance.
