...
from ..server.enip.client import connector

log				= logging.getLogger( __package__ )



class poller_enip( poller, threading.Thread ):
    """A PLC object that communicates with a physical PLC via EtherNet/IP CIP.  Schedules polls of
    various Tags at various poll rates, prioritizing the polls by age.
    ...
    def __init__( self, description, client=None,
                  host=None, port=None, multiple=0, depth=1, timeout=None, **kwargs ):
        poller.__init__( self, description=description, **kwargs )
        threading.Thread.__init__( self, target=self._poller )
        if client is None:
            client		= connector( host=host, port=port, timeout=timeout )
        else:
        ...
        self.daemon		= True
        self.done		= False
        self.multiple		= multiple	# Use Multiple Service Packets (of specified max. size)
        self.polling		= set()		# Tags known to be successfully polling
        self.failing		= set() 	# Tags known to be failing
    ...
    def join( self, timeout=None ):
        ...
        self.stop()
        super( poller_enip, self ).join( timeout=timeout )

    def _poller( self, *args, **kwargs ):
        """ Asynchronously (ie. in another thread) poll all the specified
        Tags, on the designated poll cycle.  Until we have something to do
        (self.rate isn't None), just wait.

        We'll log whenever we begin/cease polling any given Tag.
        """
