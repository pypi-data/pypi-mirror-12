...
class ForkedSuite(TestSuite):

    def __init__(self, suite, unique):
        super(ForkedSuite, self).__init__()
        self.suite = suite
        self.unique = unique
        self.stream = None
        self.child_pid = None
    ...
    def __hash__(self):
        ...
        # id() instead or something testtools can provide (their is already a
    ...
    def run(self, result):
        ...
        if self.child_pid:
            os.close(write)
            self.stream = os.fdopen(read, 'rb', 1)
            try:
            ...
            os.close(read)
            self.stream = os.fdopen(write, 'wb', 1)
            ...
            # a way that a single process with reach it. As such, it should be
...
class SubprocessedSuite(TestSuite):

    def __init__(self, suite, unique):
        super(SubprocessedSuite, self).__init__()
        self.suite = suite
        self.unique = unique
        self.cmd = None
        self.stream = None
    ...
    def setUp(self):
        super(SubprocessedSuite, self).setUp()
        self.write_suite()

    # FIXME: Poorly named -- vila 2015-12-09
