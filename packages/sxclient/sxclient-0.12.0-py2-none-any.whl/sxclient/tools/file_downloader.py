...
class SXFileDownloader(object):
    ...
    def _stream_blocks_to_file(self, file_name, file_size, block_batch):
        ...
        # We could possibly pass fo all the way down from
        # _get_sx_tmp_file_stream however that would require
        # additional lock because there's a race condition
        # between .seek() and .write().
    ...
    def _get_sx_tmp_file_stream(
        ...
        context = Context(exception=None)
        ...
        with open(path, 'rb') as fo:
            while blocks:
                ...
                if exc:
                    self._thread_pool.clear_task_queue()
                    self._thread_pool.wait_for_completion()
    ...
    def clean_cached_files(
        ...
        for file_name in os.listdir(self._tmp_dir):
            if file_name.startswith(pattern):
                os.remove(os.path.join(self._tmp_dir, file_name))
