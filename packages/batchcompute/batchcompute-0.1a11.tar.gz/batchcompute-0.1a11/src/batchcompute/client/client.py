...
from batchcompute.utils import (
    CamelCasedClass, remap, add_metaclass, set_log_level, get_logger,
)
from batchcompute.utils.constants import STRING

logger = get_logger('batchcompute.client.client')
...
class ResponseChecker(object):
    ...
    def _check(self, response):
        ...
        if status//100 == 2:
            ...
            data = response.read()
            if data:
                error = remap(data, self._h)
                err_code = error['Code']
                err_msg = error['Message']
            else:
                err_code = 'Unkown code'
                err_msg = 'Unkown msg'
            request_id = response.getheader('request-id')
            logger.error("Error data: %s\n"
                         "Error status: %s\n"
                         "Error code: %s\n"
                         "Error request Id: %s\n"
                         "Error message: %s\n",
                         data, status, err_code, request_id, err_msg)
...
class CheckedApi(object):
    ...
    def __init__(self, region, 
                 access_key_id, 
                 access_key_secret, 
                 access_security_token='',
                 human_readable=False):
        self._c = Api(region, access_key_id, access_key_secret, access_security_token)
