...
MirageAPI = {
    "scenario_base": "/stubo/api/v2/scenarios",
    "scenario_list_detail": "/stubo/api/v2/scenarios/detail",
    "scenario": "/stubo/api/v2/scenarios/objects/%s",
    "session_control": "/stubo/api/v2/scenarios/objects/%s/action"
}
...
class Client:
    ...
    def create_scenario(self, scenario_name):
        ...
        :return:
        """
        resp = requests.put(url=self.uri + MirageAPI['scenario_base'],
    ...
    def begin_session(self, scenario_name, session, mode):
        ...
        resp = requests.post(url=self.uri + api % scenario_name,
                             json=payload)
    ...
    def remove_scenario(self, scenario_name):
        ...
        resp = requests.delete(url=self.uri + api % scenario_name)
        if resp.status_code == 200:
