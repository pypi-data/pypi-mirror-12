...
class ApiFiltered(object):
    ...
    def date_generator(start, num_of_days):
        for num in range(int(num_of_days)):
            yield (start + datetime.timedelta(days=num)).strptime("%YY-%mm-%dd")

    def incremental_reach(self):
        # finding the number of days
        start_changed = datetime.strptime(self.start, "%Y-%m-%d")
        end_changed = datetime.strptime(self.end, "%Y-%m-%d")
        ...
        for generator_end_date in dates:
            call = self.filtered_call(token=self.token,
                   ad_account=self.ad_account, start=self.start, end=generator_end_date,
                   campaign_id=self.campaign_id, save=False)
