...
class ApiFiltered(object):
    ...
    def only_ids(self):
        if self.adgroup_id != '':
            ...
            columns = '["account_name","account_id","campaign_group_name","campaign_group_id","campaign_id",' \
                      '"campaign_name","adgroup_id","adgroup_name","reach","frequency",' \
                      ...
                      '"video_complete_watched_actions","cpc","ctr","country"]'
            non_action_items = ["account_name", "account_id", "campaign_group_name", "campaign_group_id", "campaign_id",
                                "campaign_name", "adgroup_id", "adgroup_name", "reach", "frequency",
                                "impressions", "spend", "cpm", "cpp", "cpc", "ctr", "country"]
            relevance_score = 0
            ...
            if self.save:
                with open(str(self.save_location) + 'data_relevance_score.csv',
                          'wb+') as g:
                    s = csv.writer(g)
                    s.writerow(["account_name", "account_id", "campaign_group_name", "campaign_group_id", "campaign_id",
                                "campaign_name", "adgroup_id", "adgroup_name", "reach",
                                "frequency",
                                "impressions", "spend", "cpm", "cpp", "cpc", "ctr", "country",
                    ...
                    for lines in data_array:
                        for row in lines:
                            ...
                            for element in row:
                                try:
                                    temp.append(element.encode('utf-8'))
                                except:
                                    temp.append(element)
                            s.writerow(temp)
            else:
