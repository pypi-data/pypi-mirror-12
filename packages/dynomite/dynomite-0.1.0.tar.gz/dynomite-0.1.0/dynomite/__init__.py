...
def create_pipeline((dp_conn, dynamodb_conn, s3_conn_dest, s3_conn_src,
    ...
    __check_status__(dp_conn, pipeline_id, table, action, debug)
...
def __check_status__(conn, pipeline_id, table, action, debug):
    ...
    logger.debug('%s pipeline state: %s' % (table, state))
    try:
        logger.debug('%s pipeline health: %s' % (table, health_status))
    except NameError:
        logger.debug('%s pipeline health status not available' % (table))
    if state == 'FINISHED' and health_status == 'ERROR':
        logger.error('%s %s failed' % (table, action))
    elif state != 'FINISHED':
        logger.debug('%s %s still running' % (table, action))
