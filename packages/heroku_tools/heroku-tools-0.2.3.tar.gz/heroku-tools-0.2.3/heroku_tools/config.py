    ...
    for key, local, remote, status in diff:
        if status in statuses:
            ...
            if status == '=':  # it's a match
                click.echo(u'  %s: %s' % (kmax, local))
            elif status == '+':  # it's a new setting
                click.echo(u'+ %s: %s' % (kmax, local))
            elif status == '!':  # it's a mismatch
                click.echo(u'! %s: %s (remote = %s)' % (kmax, local, remote))
            elif status == '?':  # it's a remote setting
                click.echo(u'? %s: %s (remote only)' % (kmax, remote))
            else:
                click.echo(u"Unknown status for %s: %s" % (key, status))
...
def configure_application(target_environment):
    ...
    try:
        ...
        click.echo(u"Configuration error: %s" % e)
        sys.exit(1)
    ...
    for key, local, _, status in updates:
        click.echo(u"%s %s=%s" % (status, key, local))
    click.echo(u"")
