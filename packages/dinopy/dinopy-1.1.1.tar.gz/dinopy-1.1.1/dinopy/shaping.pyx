...
Note:
    The :py:func:`dinopy.processors.qgrams` function that creates (shaped)
    q-grams from a sequence is considered a processor and can be found
    in the :py:mod:`dinopy.processors` module.
...
cpdef apply_shape(object qgram, object shp):
    ...
    Calls :py:func:`_apply_shape_array` or :py:func:`_apply_shape_unicode`,
    depending on the type of q-gram.

    Arguments:
        qgram (dtype): A q-gram of any :ref:`dtype <dtype>`.
        qgram_shape (Shape): see :py:class:`dinopy.shape.Shape`

    Returns:
        dtype: A reduced q-gram. The result of applying `qgram_shape` to `qgram`,
        e.g. applying `#__##` to `ACGTA` results in `ATA`.
    """
    cdef int len_qgram = qgram.bit_length() / 2 - 1 if isinstance(qgram, int) else len(qgram)  # fixed to 2 bit encoding for now, -1 because of common prefix
    ...
    elif isinstance(qgram, int):
        return _apply_shape_int(qgram, bshape, 2)  # fixed to 2 bit encoding for now
...
cdef unsigned long _apply_shape_int(unsigned long qgram, Shape qgram_shape, int bits_needed):
...
cpdef list windows_list (seq, object qgram_shape):
    ...
    Returns:
        list: A list containing all gapped q-grams generated by applying :py:func:`qgram_shape`
