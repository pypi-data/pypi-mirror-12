...
def fai_entry_to_chromosome_info_entry(fai_entry):
    ...
    return [chr_name, chr_len,
            (chr_start - len(chr_name) - (1 + len(os.linesep)),
             chr_start - len(chr_name) - (1 + len(os.linesep)) + chr_len)]
...
def chromosome_info_to_fai(chr_info, line_length=80):
    ...
    for item in chr_info:
        ...
        nameline_lengths += len(chr_name) + 1 + len(os.linesep)  # literally: len(chr_name) + len('>') + len(newline)
        chr_start_bytes = chr_start + nameline_lengths
        if isinstance(chr_name, str):
            chr_name = chr_name.encode()
        fai.append([chr_name, chr_length, chr_start_bytes, line_length, line_length + len(os.linesep)])
        nameline_lengths += ceil(chr_length / (line_length * len(os.linesep)))  # because we need that many additional newline bytes
    return fai
