...
by :py:meth:`dinopy.FastaReader.lines()`)
.
...
def qgrams(source, object qgram_shape, object dtype=None, type encoding=None, bool wrap=False):
    ...
    if encoding is None:
        ...
        if wrap: # use 2bit encoding, with wrap
            if dtype is None:
                for item in _general_wrap(source, shape_length):
                    if shp.is_solid():
                        yield from _2bit_qgrams(item, shp.length)
                    else:
                        yield from _2bit_qgrams_with_shape(item, shp)
            elif dtype == str:
        ...
        if wrap: # use 4bit encoding, with wrap
            if dtype is None:
                for item in wrap(source, shape_length):
                    if shp.is_solid():
                        yield from _4bit_qgrams(item, shp.length)
                    else:
                        yield from _4bit_qgrams_with_shape(item, shp)
            elif dtype == str:
                for item in _wrap_str(source, shape_length):
                    if shp.is_solid():
                        yield from _4bit_qgrams(item, shp.length)
                ...
                try:
                    for item in wrap(source, shape_length):
                        for qgram in windows(item, shp):
                            yield encode(qgram, _4bit_from_any)
                except TypeError:
                    raise TypeError("Unsupported type {} of source.".format(str(type(source))))
...
cpdef unsigned long _first_qgram_2bit(object seq):
    ...
    """
    cdef unsigned long qgram = 0b11 # initialize qgram with sentinel bits
    cdef int base
    for item in seq:
...
def _2bit_qgrams(object seq, int q):
    ...
    """
    cdef unsigned long sentinel = <unsigned long>0b11 << (2*q) # mask for the two sentinel bits
    cdef unsigned long valid_bits_mask = (<unsigned long>1 << (2*(q+1)))-1 # all valid bits for a q-gram
    ...
    for item in seq[q:]:
        ...
        base = _2bit_from_any_with_iupac_replacement[item]
        qgram = (((qgram << 2) & valid_bits_mask) | sentinel) | base
        yield qgram

def _2bit_qgrams_with_shape(object seq, Shape shp):
    """Create 2bit encoded q-grams from a sequence.
    ...
    """
    cdef int q = len(shp)
    cdef unsigned long sentinel = <unsigned long>3 << (2*q)  # mask for the two sentinel bits
    cdef unsigned long valid_bits_mask = (<unsigned long>1 << (2*(q+1)))-1 # all valid bits for a q-gram
...
cpdef unsigned long _first_qgram_4bit(object seq):
    ...
    """
    cdef unsigned long qgram = 0b1111 # initialize q-gram with sentinel bits
    cdef int base
    for item in seq:
...
def _4bit_qgrams(object seq, int q):
    ...
    """
    cdef long sentinel = <unsigned long>0b1111 << (4*q) # mask for the two sentinel bits
    cdef unsigned long valid_bits_mask = (<unsigned long>1 << (4*(q+1)))-1 # all valid bits for a q-gram
    ...
    for item in seq[q:]:
        ...
        base = _4bit_from_any[item]
        qgram = (((qgram << 4) & valid_bits_mask) | sentinel) | base
        yield qgram

def _4bit_qgrams_with_shape(object seq, Shape shp):
    """Create 4bit encoded q-grams from a sequence.
