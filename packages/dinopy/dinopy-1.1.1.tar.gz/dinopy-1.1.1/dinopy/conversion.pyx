...
This includes:
    ...
    - bytes
    - bytearray
    - :py:class:`dinopy.basenumbers <dinopy.basenumbers>`
...
cdef object _change_dtype_bytearray(bytearray line, type dtype):
    ...
    Returns:
        dtype: The line converted to the specified datatype.
...
cpdef unsigned long encode_twobit(object seq):
    ...
    """
    return encode(seq, _2bit_from_any_with_iupac_replacement)

cpdef unsigned long encode_fourbit(object seq):
    ...
    """
    return encode(seq, _4bit_from_any)

cpdef unsigned long encode(object seq, object codemap):
    ...
    cdef unsigned long bits = <unsigned long> cpow(2, bits_needed) - 1  # ensure common prefix for easier decoding
    for c in seq:
        bits <<= bits_needed
        bits |= codemap[c]
    return bits

cpdef object decode_twobit(unsigned long twobit_seq, type dtype=bytes):
    ...
    """
    if dtype == bytes:
        return bytes(decode(twobit_seq, _byte_from_2bit))
    elif dtype == bytearray:
        return bytearray(decode(twobit_seq, _byte_from_2bit))
    elif dtype in (str, unicode):
        return "".join(decode(twobit_seq, _str_from_2bit))
    elif dtype == basenumbers:
        return bytes(decode(twobit_seq, _basenumber_from_2bit))
    else:
        raise InvalidDtypeError("Invalid dtype {}".format(dtype))

cpdef object decode_fourbit(unsigned long fourbit_seq, type dtype=bytes):
    ...
    """
    if dtype == bytes:
        return bytes(decode(fourbit_seq, _byte_from_4bit))
    elif dtype == bytearray:
        return bytearray(decode(fourbit_seq, _byte_from_4bit))
    elif dtype in (str, unicode):
        return "".join(decode(fourbit_seq, _str_from_4bit))
    elif dtype == basenumbers:
        return bytes(decode(fourbit_seq, _basenumber_from_4bit))
    else:
        raise InvalidDtypeError("Invalid dtype {}".format(dtype))

cpdef list decode(unsigned long bit_seq, dict inv_codemap):
    ...
    cdef int length = bit_seq.bit_length() / bits_needed - 1  # -1 to skip common prefix
    seq = []
