...
def task_status_changed(stage):
    stage.log.info(u'%s %s (%s tasks)' % (stage, stage.status, len(stage.tasks)))
...
class StageEdge(Base):
    __tablename__ = u'stage_edge'
    parent_id = Column(Integer, ForeignKey(u'stage.id', ondelete=u"CASCADE"), primary_key=True)
    child_id = Column(Integer, ForeignKey(u'stage.id', ondelete=u"CASCADE"), primary_key=True)
    ...
    def __str__(self):
        return u'<StageEdge: %s -> %s>' % (self.parent, self.child)
...
class Stage(Base):
    __tablename__ = u'stage'
    __table_args__ = (UniqueConstraint(u'execution_id', u'name', name=u'_uc_execution_name'),)
    ...
    execution_id = Column(ForeignKey(u'execution.id', ondelete=u"CASCADE"), nullable=False, index=True)
    started_on = Column(DateTime)
    finished_on = Column(DateTime)
    ...
    tasks = relationship(u"Task", backref=u"stage", cascade=u"all, merge, delete-orphan", passive_deletes=True)
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not re.match(u'^[a-zA-Z0-9_\.-]+$', self.name):
            raise Exception(u'invalid stage name %s' % self.name)
    ...
    @property
    def url(self):
        return url_for(u'cosmos.stage', execution_name=self.execution.name, stage_name=self.name)
    ...
    def delete(self, delete_files=False, delete_descendants=False):
        ...
        if delete_descendants:
            self.log.info(u'Deleting all descendants of %s' % self)
            for stage in reversed(list(self.descendants())):
                stage.delete(delete_files)

        self.log.info(u'Deleting %s. delete_files=%s' % (self, delete_files))
        if delete_files:
            for t in self.tasks:
    ...
    def get_task(self, tags, default=u'ERROR'):
        tags = dict((k, v) for k, v in tags.items() if isinstance(v, ACCEPTABLE_TAG_TYPES))  # These are the only tags that actually get saved to the DB
        for task in self.tasks:
            if task.tags == tags:
                return task
        if default == u'ERROR':
            raise KeyError(u'Task with tags %s does not exist' % tags)
        else:
            return default
    ...
    def descendants(self, include_self=False):
        ...
        if include_self:
            return sorted(set([self]).union(x), key=lambda stage: stage.number)
        else:
            return x

    @property
    def label(self):
        return u'{0} ({1}/{2})'.format(self.name, self.num_successful_tasks(), len(self.tasks))

    def __repr__(self):
        return u'<Stage[%s] %s>' % (self.id or u'', self.name)
