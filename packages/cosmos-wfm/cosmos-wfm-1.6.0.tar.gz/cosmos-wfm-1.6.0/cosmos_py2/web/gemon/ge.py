...
def qstat(user=getpass.getuser()):
    ...
    try:
        et = ET.fromstring(sp.check_output([u'qstat', u'-ext', u'-xml', u'-u', user], preexec_fn=preexec_function))
    except (sp.CalledProcessError, OSError):
        # Error occurs if there are no jobs
        return pd.DataFrame()

    dicts = list( job_list_to_dict(jl) for jl in et.findall(u'.//job_list') )
