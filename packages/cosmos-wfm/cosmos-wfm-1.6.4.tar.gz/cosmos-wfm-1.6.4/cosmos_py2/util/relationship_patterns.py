...
def _group_paths(list_of_files_tag_tuples, by):
    ...
    def f(xxx_todo_changeme):
        ...
        try:
            return dict((k, tags[k]) for k in by)
        except KeyError, k:
            raise KeyError(u'keyword %s is not in the tags of %s' % (k, (file_path, tags)))
...
def group(tasks_or_tuples, by):
    ...
    if isinstance(tasks_or_tuples[0], tuple):
        assert isinstance(tasks_or_tuples[0][0], unicode) and isinstance(tasks_or_tuples[0][1],
                                                                     dict), u'Tuple must be of type (str, dict)'
        for x in _group_paths(tasks_or_tuples, by):
            yield x
    elif not isinstance(tasks_or_tuples[0], Task):
        raise AssertionError(u'`tasks_or_tuples` must be an iterable of Tasks or tuples')

    def f(task):
        try:
            return dict((k, task.tags[k]) for k in by)
        except KeyError, k:
            raise KeyError(u'keyword %s is not in the tags of %s' % (k, task))
...
def one2one(cmd_fxn, parents, tag=None, out_dir=None, stage_name=None):
    ...
    assert isinstance(tag, dict), u'`tag` must be a dict'

    def g():
        for parent in parents:
            new_tags = parent.tags.copy()
            new_tags.update(dict((k, v.format(**new_tags) if isinstance(v,unicode) else v) for k,v in tag.items()))
            yield execution.add_task(cmd_fxn, tags=new_tags, parents=[parent], out_dir=out_dir or parent.output_dir, stage_name=stage_name)

    return list(g())


def many2one(cmd_fxn, parents, groupby, tag=None, out_dir=u'', stage_name=None):
    u"""
    :param func cmd_fxn: the function that runs the command
    ...
    def g():
        for new_tags, parent_group in group(parents, groupby):
            ...
            yield execution.add_task(cmd_fxn, tags=new_tags, parents=parent_group,
                                     out_dir=out_dir(new_tags) if hasattr(out_dir, u'__call__') else out_dir,
...
def one2many(cmd_fxn, parents, splitby, tag=None, out_dir=u'', stage_name=None):
    u"""
    ...
    def g():
        for parent_task in parents:
            ...
            tag_itrbl = splitby(parent_task) if hasattr(splitby, u'__call__') else combinations(splitby)
...
def make_tags(new_tags, tag):
    return new_tags.update(dict((k, v.format(**new_tags) if isinstance(v,unicode) else v) for k,v in tag.items()))
