...
def render_recipe(execution, recipe, default_drm):
    ...
    convert = dict((recipe_stage, f(recipe_stage)) for recipe_stage in nx.topological_sort(recipe.recipe_stage_G))
    ...
    for i, stage in enumerate(nx.topological_sort(stage_g)):
        ...
        if not stage.resolved:
            successful_tasks = dict((frozenset(t.tags.items()), t) for t in
        ...
        if len(tagz) != len(set(tagz)):
            d = duplicates(list(imap(dict, tagz))).next()
            raise AssertionError(u'Duplicate tags detected in %s: %s' % (stage, d))
