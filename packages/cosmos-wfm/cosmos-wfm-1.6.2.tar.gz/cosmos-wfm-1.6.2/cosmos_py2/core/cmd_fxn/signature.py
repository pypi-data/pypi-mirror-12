...
def call(cmd_fxn, task, input_map, output_map):
        ...
        for arg in argspec.args[1:-1 * ndefaults]:
            if arg not in params:
                raise AttributeError(
                    u'%s requires the parameter `%s`, are you missing a tag?  Either provide a default in the cmd() '
                    u'method signature, or pass a value for `%s` with a tag' % (cmd_fxn, arg, arg))
    ...
    for param_name in [u'cpu_req', u'mem_req', u'drm']:
        if param_name in sig.parameters:
            param_val = kwargs.get(param_name, sig.parameters[param_name].default)
            setattr(task, param_name, param_val)

    assert isinstance(out, unicode) or out is None, u'cmd_fxn %s did not return a str or None' % cmd_fxn
    return out
...
def default_prepend(execution_output_dir, task_output_dir):
    if task_output_dir and task_output_dir != u'':
