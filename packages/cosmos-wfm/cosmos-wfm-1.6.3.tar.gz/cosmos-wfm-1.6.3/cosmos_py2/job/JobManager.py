...
class JobManager(object):
    def __init__(self, cosmos_app, get_submit_args, log_out_dir_func=default_task_log_output_dir, default_queue=None, cmd_wrapper=None):
        ...
        self.drms[u'lsf'] = DRM_LSF(self)
        self.drms[u'ge'] = DRM_GE(self)
        self.drms[u'drmaa'] = DRM_DRMAA(self)
    ...
    def get_drm(self, drm_name):
        u"""This allows support for drmaa:ge type syntax"""
        return self.drms[drm_name.split(u':')[0]]
    ...
    def submit_task(self, task, command):
        if command == NOOP:
            ...
            assert task.drm is not None, u'task has no drm set'
    ...
    def run_tasks(self, tasks):
        ...
        commands = list(imap(self.call_cmd_fxn, tasks))
        ...
        list(imap(self.submit_task, tasks, commands))
...
def _create_command_sh(task, command):
    u"""Create a sh script that will execute a command"""
    with open(task.output_command_script_path, u'wb') as f:
        f.write(str(command).encode('UTF-8'))
    os.system(u'chmod 700 "{0}"'.format(task.output_command_script_path))
