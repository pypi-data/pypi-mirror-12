...
class Enum34_ColumnType(types.TypeDecorator):
    ...
    def process_bind_param(self, value, dialect):
        assert isinstance(value, self.enum_class) or value is None, u"'%s' must be of type %s" % (value, self.enum_class)
...
class ListOfStrings(types.TypeDecorator):
    ...
    def process_bind_param(self, value, dialect):
        assert isinstance(value, list), u'%s must be a list' % value
        return u', '.join(value)
...
class JSONEncodedDict(TypeDecorator):
    u"Represents an immutable structure as a json-encoded string."
    ...
    def process_bind_param(self, value, dialect):
        if value is not None:
            ...
            value = six.text_type(json.dumps(dict((k, v) for k, v in value.items()
...
class MutableDict(Mutable, dict):
    ...
    def coerce(cls, key, value):
        u"Convert plain dictionaries to MutableDict."
    ...
    def __setitem__(self, key, value):
        u"Detect dictionary set events and emit change events."

        dict.__setitem__(self, key, value)
        self.changed()

    def __delitem__(self, key):
        u"Detect dictionary del events and emit change events."
