...
@event.listens_for(Engine, u"connect")
def set_sqlite_pragma(dbapi_connection, connection_record):
    u"""Turn on sqlite foreignkey support"""
    if isinstance(dbapi_connection, SQLite3Connection):
        cursor = dbapi_connection.cursor()
        cursor.execute(u"PRAGMA foreign_keys=ON")
        cursor.close()
...
class Base(declarative_base()):
    ...
    def attrs_as_dict(self):
        l = [(c.name, getattr(self, c.name)) for c in self.__table__.columns if
             c.name != u'id' and c.name not in self.exclude_from_dict]
