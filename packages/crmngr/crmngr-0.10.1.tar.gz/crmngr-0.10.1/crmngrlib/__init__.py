...
class Report:
    ...
    def _module_object(self, name, module_data):
        ...
        if module_data['type'] == 'forge':
            module = ForgeModule(name, module_data['author'])
            if 'version' in module_data:
                module.version = Forge(module_data['version'])
            if self._args.version_check:
                module.info = get_forge_module_info(
                    module.forgename, self._cache
                )
        else:
            module = GitModule(name, module_data['url'])
            if self._args.version_check:
                module.info = self._git.get_module_info(module.url)
            if 'branch' in module_data:
                module.version = GitBranch(module_data['branch'])
            elif 'commit' in module_data:
                module.version = GitCommit(module_data['commit'])
            elif 'ref' in module_data:
                module.version = GitRef(module_data['ref'])
            elif 'tag' in module_data:
                module.version = GitTag(module_data['tag'])
        return module
        ...
        for module in sorted(self.modules):
            ...
            for _, module_details in self._modules[module].items():
                if report_branches:
                    ...
                    requested_branches = self.branches
                    match = True
