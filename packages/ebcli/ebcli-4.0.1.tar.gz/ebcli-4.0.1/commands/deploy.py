...
def initialize_parser(subparsers):
    ...
    commands.add_standard_argument(parser, commands.WORKSPACE_ID_ARG)
    commands.add_standard_argument(parser, commands.TAGS_ARG)
    commands.add_standard_argument(parser, commands.CLAIMS_ARG)
...
def execute_command(args, _unknown):
    ...
    for instance_name, spec in _load_application(args).iteritems():
        ...
        session.post('/services/instances', json=request)
