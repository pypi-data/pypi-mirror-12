...
BOXES_PATH_ARG = ['--boxes-path'], {'help': 'Path where boxes are stored'}
...
class Boxes(object):
    ...
    def __init__(self, path, session=None):
        ...
        if os.path.exists(self._path):
            for item in os.listdir(self._path):
                ...
                if os.path.isdir(item) and os.path.exists(box_path):
    ...
    def has_box(self, box_id):
        return box_id in self._box_index

    def get_box(self, box_id):
        if box_id in self._box_index:
            return self._box_index[box_id]
        else:
            raise Exception("Box '%s' not found." % box_id)
    ...
    def copy_box(self, box_id, destination):
    ...
    def export_box(self, box_id, recursive=False, source=None):
        if source and source.has_box(box_id):
            self._export_local_box(box_id, recursive, source)
        else:
            self._export_remote_box(box_id, recursive, source)

    def import_box(self, box_name, comment=None, workspace=None, docker_image=None):
