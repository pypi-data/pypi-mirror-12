...
def execute_command(args, _unknown):
    ...
    if args.box_id:
        ...
        formatter.write(response.json(), prefix='BOX', fields=ebcli.DEFAULT_BOX_FIELDS)

    if args.instance_id:
        response = session.get('/services/instances/{0}', args.instance_id).json()
        formatter.write(response.json(), prefix='INSTANCE', fields=ebcli.DEFAULT_INSTANCE_FIELDS)

    if args.workspace_id:
        response = session.get('/services/workspaces/{0}', args.workspace_id).json()
        formatter.write(response.json(), prefix='WORKSPACE', fields=ebcli.DEFAULT_WORKSPACE_FIELDS)
