...
class Instance(object):
    ...
    def get_location(self, box_id, path):
        if self._session:
            ...
            return 'file://{0}/{1}/'.format(os.path.join(os.getenv('ELASTICBOX_PATH'), 'boxes'), box['name'], path)
    ...
    def set(self, name, value, scope):
        if self._session:
            ...
            self._session.post('/services/instances/{0}/set', self._instance['instance'], json=data)
        else:
            found = False
    ...
    def execute_event(self, event_name):
        self._execute_event(Context(self._instance['box'], None, self), event_name)
    ...
    def _execute_event(self, context, event_name):
        ...
        for _, value in context.iteritems():
            if type(value) is Context:
                self._execute_event(value, event_name)
        ...
        try:
            with tempfile.NamedTemporaryFile('w', delete=False) as script:
                ...
                with open(script_file, 'r') as template:
                    ...
                    print >> script, self.config(contents, context.get_scope(), context=context)
