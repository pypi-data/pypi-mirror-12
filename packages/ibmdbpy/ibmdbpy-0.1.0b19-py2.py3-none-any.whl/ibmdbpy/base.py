    ...
    def append(self, idadf, df, maxnrow=None):
        ...
        if df.shape[0] > 1.5 * maxnrow:
            ...
            for i, chunk in enumerate(split, 0):
                ...
                try:
                    self._insert_into_database(chunk, idadf._name, silent=True)
                except:
                    raise
        ...
        for column in dataframe.columns:
            if dataframe.dtypes[column] in [object,bool]:
                ...
                if set(dataframe[column].unique()).issubset([True, False, 0, 1, np.nan]):
                    column_string += "\"%s\" SMALLINT," % column.strip()
                else:
                    if column == primary_key:
                        column_string += "\"%s\" VARCHAR(255) NOT NULL, PRIMARY KEY (%s)," % (column.strip(), column.strip())
                    else:
                        column_string += "\"%s\" VARCHAR(255)," % column.strip()
                ...
                if dataframe.dtypes[column] in [np.int64, np.int, np.int8, np.int32]:
                    ...
                    datatype = "DOUBLE"
                if column == primary_key:
                    column_string += "\"%s\" %s NOT NULL, PRIMARY KEY (%s)," % (column.strip(), datatype, column.strip())
                else:
                    column_string += "\"%s\" %s," %(column.strip(), datatype)
        if column_string[-1] == ',':
