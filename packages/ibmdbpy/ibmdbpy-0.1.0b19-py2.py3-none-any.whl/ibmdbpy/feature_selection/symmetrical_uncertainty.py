...
@idadf_state
@timed
def su_matrix_pareto(idadf, features = None, filter_out = 0.8):
    # Check input 
    if features is None:
    ...
    count_serie = idadf.count_groupby(features, count_only = True)
    ...
    for column_pair in combinations:
        if column_pair[0] not in entropy_dict:
            entropy_dict[column_pair[0]] =  _compute_entropy(len(idadf)-n_records_out, data, target=column_pair[0])
        if column_pair[1] not in entropy_dict:
            entropy_dict[column_pair[1]] =  _compute_entropy(len(idadf)-n_records_out, data, target=column_pair[1])
        subset_entropy = _compute_entropy(len(idadf)-n_records_out, data, target=column_pair[0], conditional=column_pair[1]) 
...
@idadf_state
@timed
def su_pareto(idadf, target, features = None, filter_out = 0.8):
...
def su(idadf, target , features = None, having = None):
    ...
    # Compute
    value_dict = OrderedDict()
    #data_entropy = _compute_entropy(len(idadf), data, target=target)
    data_entropy = _compute_entropy(data, target=target)
    
    for feature in features:
        #feature_entropy = _compute_entropy(len(idadf), data, target=feature)
        feature_entropy = _compute_entropy(data, target=feature)
        #subset_entropy = _compute_entropy(len(idadf), data, target=target, conditional=feature)
        subset_entropy = _compute_entropy(data, target=target, conditional=feature)
        gain = (data_entropy - subset_entropy)
        symmetrical_uncertainty = 2.0 * (gain/(data_entropy + feature_entropy))
...
def su_matrix(idadf, features = None, having = None):
    ...
    for column_pair in combinations:
        if column_pair[0] not in entropy_dict:
            entropy_dict[column_pair[0]] =  _compute_entropy(len(idadf), data, target=column_pair[0])
        if column_pair[1] not in entropy_dict:
            entropy_dict[column_pair[1]] =  _compute_entropy(len(idadf), data, target=column_pair[1])
        subset_entropy = _compute_entropy(len(idadf), data, target=column_pair[0], conditional=column_pair[1]) 
        gain = (entropy_dict[column_pair[0]] - subset_entropy)
        values.append(2.0 * (gain/(entropy_dict[column_pair[0]] + entropy_dict[column_pair[1]])))
