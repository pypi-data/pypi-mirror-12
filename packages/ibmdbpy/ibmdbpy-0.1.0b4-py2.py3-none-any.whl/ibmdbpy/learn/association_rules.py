...
class AssociationRules(object):
    """
    Association rules mining can be used for the discovery of interesting
    and useful relations between items in a large-scale transaction table.
    You can identify strong rules between related items by using different
    measures of interestingness. Well-known algorithms for association rules
    mining are Apriori or FP-Growth. For analytic stored procedures, the
    PrefixSpan algorithm is preferred due to its scalability.
    ...
    def __init__(self, modelname = None, minsupport = None, maxlen = 5, maxheadlen = 1, minconf = 0.5):
        ...
        ----------
        modelname : str
            The name of the Association Rules model that is to be build. If
            the parameter corresponds to an existing model in the database, it
        ...
        get_params and set_params. But we recommend creating a new AssociationRules
    ...
    def get_params(self):
        """
        Returns the parameters of the Association Rules model.
        """
        params = dict()
    ...
    def set_params(self, **params):
        """
        Modifies the parameters of the Association Rules model.
        """
        if not params:
    ...
    def fit(self, idadf, transaction_id, item_id,  nametable=None, namecol=None, verbose=False):
        """
        Creates an Association Rules model from an IdaDataFrame.
    ...
    def prune(self, itemsin = None, itemsout = None, minlen = 1, maxlen = None,
        ...
        """
        Prunes the rules and patterns of an association rules model.
        To remove rules and pattern which you are not interested in,
        you can use filters to exclude these rules and patterns.
        These rules and patterns are then marked as not valid in
        the model and are no longer shown.
    ...
    def predict(self, idadf, outtable=None, transaction_id=None, item_id=None,
        ...
        """
        Applies the rules and patterns of an association rules model to other
        ...
        outtable : str, optional
            The name of the output table which the mapping between the input
            sequences and the associated rules or patterns is written to. If
            the parameter corresponds to an existing table in the database, it
            will be replaced.

        transaction_id : str, optional
            The column of the input table that identifies the transaction ID.
            Default: the same tid column that is specified in the stored
            procedure to build the model.

        item_id : str, optional
            The column of the input table that identifies an item of the
            transaction. Default: the same item column that is specified in the
        ...
        limit : int, optional, >=1, default: 1
            The maximum number of rules or patterns that is written to the
            output table for each input sequence.

        sort : str or list, optional
            A list of keywords that indicate the order in which the rules or
            patterns are written to the output table. The order of the list is
            descending. The items are separated by semicolons. Possible values
            are 'support', 'confidence', 'lift', and 'length'. The 'confidence'
            value can only be specified if the type parameter is 'rules'.
    ...
    def fit_predict(self, idadf, transaction_id, item_id,  nametable=None,
        ...
        """
        Convenience function for fitting the model and use it to make prediction
        on the same dataset. Please Refer to fit and predict documentation for
        explanation about attributes.

        Notes
        -----
        By using this function your are not able to use the prune step between
        the fit and the predict step. However you can still prune afterwards
        and reuse the predict function.
    ...
    def describe(self):
        """
        Returns a description of Association Rules Model.
        """
        if self._idadb is None:
    ...
    def _retrieve_AssociationRules_Model(self, modelname, verbose = False):
        ...
        Association Rules IDAX functions stores its result in 4 tables:
        ...
        Notes
        -----
        Needs a better formatting instead of printing the tables
        """
