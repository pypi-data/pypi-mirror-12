...
class IdaDataFrame(object):
    ...
    dashDB/DB2. IDA stands for In-DataBase Analytics. IdaDataFrame copies
    pandas interface for DataFrame objects, to ensure intuitive interaction
    for end-users.
    ...
    def __init__(self, idadb, tablename, indexer = None):
        ...
        indexer : str, optional
            ...
            optional. However, if no indexer was given, the real order of
            rows with the functions head and tail is not guaranteed. Also,
            several in-database machine learning algorithms needs an indexer
            as parameter to be executed.

        Attributes
        ----------
        _idadb : IdaDataBase
            IdaDataBase object parent from current instance.
        tablename : str
            Name of the table self is referencing.
        name : str
            Fullname of the table self is referencing, including schema.
        schema : str
            Name of the schema to which the table belong.
        indexer : str
            Name of the column to use as an index. "None" if no indexer.
        loc : str
            Indexer that enable selection and projection for IdaDataFrame instances.
            For more information, please refer to the loc class documentation.
        internal_state : InternalState
            Object used to store internally the state of the IdaDataFrame and
            allows several non destructive manipulation.
        type : str
            Type of the IdaDataFrame : "Table", "View" or "Unknown".
        dtypes : DataFrame
            Datatype in database for each column.
        ...
        Notes
        -----
        Attributes type, dtypes, index, columns axes and shape are
        evaluated in a lazy way so to avoid overhead when creating an
        IdaDataFrame. Sometimes the index may be even too big to be downloaded.
        ...
        if idadb.exists_table_or_view(tablename) is False:
            ...
            if "." in tablename:
                ...
                self.name = idadb.current_schema + '.' + tablename
...
###############################################################################
    ...
    def internal_state(self):
        """
        InternalState instances are responsible for managing the state of an
        IdaDataFrame instance and allow several non destructive data
        manipulation such as Selection, Projection, Filtering, Aggregation of
        columns.
        """
    ...
    def indexer(self):
        ...
        used to index the table. This makes especially sense because dashDB
        is per default a column-based store so rowIDs does not make sense
        and are not deterministic. As a consequence, the only way to address
        a particular row is to refer to it through the index. If no indexer
        is provided, ibmdbpy still works but a correct row order is not
        guaranteed as far as the dataset is not sorted. Note also that the
        indexer column is not taken into account in data mining algorithms. 
        """
        ...
        * All values are unique
        ...
        if value not in self.columns:
            raise IdaDataFrameError("'%s' cannot be used as indexer "%value +
                                    " because this is not a column in '%s'"%self.name)

        count = self[value].count_distinct()
    ...
    def dtypes(self):
        """
        Datatype in database for each column in self.
    ...
    def index(self):
        ...
        Notes
        -----
        Since index can be only numeric in a database, it is not that interesting
        for an IdaDataFrame but still usefull sometimes. May break if the table
        is too large. Ask user's approval before downloading a index which has
    ...
    def columns(self):
        """
        Index containing the columns names in self.
    ...
    def axes(self):
        ...
        -------
        list
            List containing two index (index and columns attributes)
    ...
    def __getitem__(self, item):
        ...
        Enable rows filtering.

        The syntax is similar as Pandas.
        ...
        Notes
        -----
        Row order is not guaranteed if no indexer was given and the dataset was not sorted
        """
    ...
    def __setitem__(self, key, item):
        """
        Enable creation and aggregation of columns.
    ...
    def __delitem__(self, item):
        """
        Enable non destructive deletion of columns using pandas style syntax.
        This takes place inplace, i.e. the current IdaDataFrame is modified
    ...
    def __lt__(self, value):
        ...
        """
        return ibmdbpy.filtering.FilterQuery(self.columns, self.name, "lt", value)

    def __le__(self, value):
        """
        ibmdbpy.filtering.FilterQuery object when comparing self using "<=".
        """
        return ibmdbpy.filtering.FilterQuery(self.columns, self.name, "le", value)

    def __eq__(self, value):
        """
        ibmdbpy.filtering.FilterQuery object when comparing self using "==".
        """
        return ibmdbpy.filtering.FilterQuery(self.columns, self.name, "eq", value)

    def __ne__(self, value):
        """
        ibmdbpy.filtering.FilterQuery object when comparing self using "!=".
        """
        return ibmdbpy.filtering.FilterQuery(self.columns, self.name, "ne", value)

    def __ge__(self, value):
        """
        ibmdbpy.filtering.FilterQuery object when comparing self using ">=".
        """
        return ibmdbpy.filtering.FilterQuery(self.columns, self.name, "ge", value)

    def __gt__(self, value):
        """
        ibmdbpy.filtering.FilterQuery object when comparing self using ">".
        """
        return ibmdbpy.filtering.FilterQuery(self.columns, self.name, "gt", value)

    ################ Arithmetic operations

    def __add__(self, other):
        """
        Performs an addition between self and another IdaDataFrame or a number.
    ...
    def __radd__(self, other):
        """
        Enables the reflexivity of the addtion operation.
    ...
    def __div__(self, other):
        """
        Performs a division between self and another IdaDataFrame or a number.
    ...
    def __rdiv__(self, other):
        """
        Enables the reflexivity of the division operation.
    ...
    def __floordiv__(self,other):
        """
        Performs a integer division between self and another IdaDataFrame or a number.
    ...
    def __rfloordiv__(self, other):
        """
        Enables the reflexivity of the integer division operation.
    ...
    def __mod__(self,other):
        """
        Performs a modulo operation between self and another IdaDataFrame or a number.
    ...
    def __rmod__(self, other):
        """
        Enables the reflexivity of the modulo operation.
    ...
    def __mul__(self,other):
        """
        Performs a multiplication between self and another IdaDataFrame or a number.
    ...
    def __rmul__(self, other):
        """
        Enables the reflexivity of the multiplication operation.
    ...
    def __neg__(self):
        """
        Performs the absolute negative of all columns in self
    ...
    def __rpos__(self,other):
        """
        Performs the absolute positive. No operation required.
        """
        return self

    def __pow__(self,other):
        """
        Performs a power operation between self and another IdaDataFrame or a number.
    ...
    def __rpow__(self, other):
        """
        Enables the reflexivity of the power operation.
    ...
    def __sub__(self,other):
        """
        Performs a substraction between self and another IdaDataFrame or a number.
    ...
    def __rsub__(self, other):
        """
        Enables the reflexivity of the substraction operation.
...
###############################################################################
    ...
    def exists(self):
        ...
        """
        return self._idadb.exists_table_or_view(self.name)
    ...
    def get_primary_key(self):
        ...
        """
        Get the name of the primary key of self, if there is one. Otherwise returns 0.
        May be deprecated in future versions because not very usefull.
        """
    ...
    def ida_query(self, query, silent = False, first_row_only = False):
        ...
        in a Tuple. See IdaDataBase.ida_query documentation.
        """
        return self._idadb.ida_query(query, silent, first_row_only)
    ...
    def ida_scalar_query(self, query, silent = False):
        ...
        See IdaDataBase.ida_query documentation.
        """
        return self._idadb.ida_scalar_query(query, silent)
...
###############################################################################
    ...
    def head(self, nrow=5):
        """
        Print the n first rows of the instance, n is set to 5 per default.
        ...
        -------
        DataFrame or Series
            Index is the coresponding row numbers and columns are all
            columns of self. If the IdaDataFrame has only one column then
            return a Serie.
...
#                data = ibmdbpy.utils._convert_dtypes(self, data)
    ...
    def tail(self, nrow=5):
        """
        Print the n last rows of the instance, n is set to 5 per default.
...
#            data = ibmdbpy.utils._convert_dtypes(self, data)
    ...
    def pivot_table(self, values=None, columns=None, max_entries=1000,
                    ...
                    aggfunc='count'):
        """
        Compute an aggregation function over all rows of each columns specified
        as values on the dataset, grouped by the columns defined in "columns".

        Parameters
        ----------
        values: str or list or str optional
            List of columns on which "aggfunc" has to be computed.
        columns: str or list or str optional
            List of columns that should be used as index and on which
            the dataframe has to be grouped by.
        max_entries: int, default=1000
            Number of cells above which the output table should not
            be outputed because too big, set per default to 1000.
        sort: str, optional
            Admissible values are "alpha" and "factors".
            * If "alpha", the index of the output will be sorted according
            to alphabetical order.
            * If "factors", the index of the output will be sorted
            according to increasing number of distinct value.
            Per Default, the index will be sorted in the same order it was
            specified in "columns" argument.
        factor_threshold: int, default: 20
            Number of distinct values above which a categorical column
            should not be considered as categorical anymore and under
            which a numerical column column should not be considered as
            numerical anymore.
        interactive: bool
            If True, the user will be asked if  he wish to display the output,
            given its size.
        aggfunc: str
            aggregation function to be computed on each columns specified
            in argument "values", admissible values are "count", "sum",
            "avg". This entry is case insensitive.

        Returns
        -------
        Series with Multi-index (columns)
    ...
    def sort(self, columns=None, axis=0, ascending=True, inplace=False):
        ...
        columns : str or list of str
            Columns that should be used to sort rows in the IdaDataFrame.
            If columns is set to None and axis to 0, then the IdaDataFrame
            columns are sorted in lexicographical order
        axis : int (0/1)
            Axis that should be sort. 0 for sorting row-wise, 1 for sorting column-wise.
        ascending : bool, default: True
            Sorting order, True : ascending, False : descending
        inplace : bool, default: False
            Should be the current object modified or create a modified copy.
            False : create a modified copy of the current dataframe
            True  : modify the current dataframe

        Raises
        ------
        ValueError
            * When sorting by column (column not None), axis must be 0 (rows)
            * A column does not belong to self
            * Axes has a different value than 0 or 100000000

        Notes
        -----
        If columns is set to None and axis to 0, this is equivalent to undo
        all sorting into the IdaDataFrame and return to the original sorting
        in dashDB/DB2.

        No actual changes are done in DashDB/DB2, only the querying changes.
        Everything is registered in a InternalState object. Changes can be
        observed by using function head and tail for example.
        """
        ...
        if columns:
            ...
            for column in columns:
                if column not in self.columns:
                    raise ValueError("Column "+column+" is not in "+self.name)
        if axis not in [0,1]:
...
###############################################################################
    ...
    def describe(self, percentiles=[0.25, 0.50, 0.75]):
        """
        A basic statistical summary about current IdaDataFrame. If there
        exist at least one numerical column, the summary includes:
            * Count of non missing values for each numerical column
            * Mean for each numerical column
            * Standart deviation for each numerical column
            * Minimum and Maximum for each numerical column
    ...
    def cov(self):
        """
        Computes the covariance matrix, composed of covariance coefficients
        between all pairs of columns in self.

        Returns
        -------
        covariance matrix: DataFrame
            The axes are the columns of self and values are the covariance
            coefficients.
        """
    ...
    def corr(self):
        """
        Computes the correlation matrix, composed of correlation coefficients
        between all pairs of columns in self.

        Returns
        -------
        correlation matrix: DataFrame
            The axes are the columns of self and values are the correlation
            coefficients.
        """
    ...
    def mode(self):
        """
        Computes the most common value for each non numeric column self. NOTIMPLEMENTED
    ...
    def quantile(self, q=0.5):
        """
        Computes quantiles row-wise for each numeric column.
        ...
        quantiles: Series or DataFrame
            If q is an array, return a DataFrame where the index is q, the
            columns are the columns of sel, and the values are the
            quantiles. If q is a float, a Series will be returned where the
    ...
    def mad(self):
        """
        Computes the mean absolute distance for all numeric columns of self.

        Returns
        -------
        mad: Series
            The index is the columns of self and values are the mean
            absolute distance.
        """
    ...
    def min(self):
        """
        Computes the minimum value for all numerics column of self.

        Returns
        -------
        min: Series
            The index is the columns of self and values are the minimum.
        """
    ...
    def max(self):
        """
        Computes the maximum value over for all numeric columns of self.

        Returns
        -------
        max: Series.
            The index is the columns of self and values are the maximum.
        """
    ...
    def count(self):
        """
        Computes the count of non-missing values for all columns of self.

        Returns
        -------
        count: Series.
            The index is the columns of self and values are the number of
            non missing values.
        """
    ...
    def count_distinct(self):
        """
        Computes the count of distinct values for all numeric columns of self.
    ...
    def std(self):
        """
        Computes the standart deviation for all numeric columns of self.

        Returns
        -------
        std: Series
            The index is the columns of self and values are the standart
            deviation.
        """
    ...
    def var(self):
        """
        Computes the variance for all numeric columns of self.

        Returns
        -------
        var: Series
            The index is the columns of self and values are the variance.
        """
    ...
    def mean(self):
        """
        Computes the mean over all entries for each numeric columns of self.
    ...
    def sum(self):
        """
        Computes the sum of values for all numeric columns of self.

        Returns
        -------
        sum: Series
            The index is the columns of self and values are the sum.
        """
    ...
    def median(self):
        """
        Computes the median for all numeric columns of self.

        Returns
        -------
        median: Series
            The index is the columns of self and values are the median.
        """
        # Behave like having the option "numeric only" to true
        # TODO: Implement the option
    ...
    @idadf_state
    def rank(self):
        """Computes the rank over all entries for all columns of self."""
        raise NotImplementedError("TODO")
...
###############################################################################
    ...
    def save_as(self, tablename, clear_existing = False):
        """
        Save self as a tablename in the remote dataBase with name tablename.
        May erase an existing table is tablename already exists and clear_existing
        is True.
        """
        # TODO: to test !
    ...
    def as_dataframe(self):
        ...
        if os.environ['VERBOSE'] == 'True':
            ...
            if raw_estimation < 0.01:
                ...
                estimation = raw_estimation + " minutes."
...
###############################################################################
    ...
    def _clone(self):
        ...
        Clone actual object
        """
        newida = IdaDataFrame(self._idadb, self.name, self.indexer)
    ...
    def _clone_as_serie(self, column):
        ...
        """
        newida = ibmdbpy.IdaSeries(idadb = self._idadb, tablename = self.name,
    ...
    def _get_type(self):
        ...
        Type of the IdaDataFrame : "Table", "View" or "Unknown".
        """
        if self._idadb.is_table(self.name):
            return "Table"
        elif self._idadb.is_view(self.name):
            return "View"
        else:
            return "Unkown"
    ...
    def _get_index(self, force=False):
        """
        Index containing a list of the name of the rows in self.
        """
        ...
        return Index(df[df.columns[0]])

    def _get_shape(self):
        """
        Tuple containing the number of rows and the number of columns of self.
        """
        name = self.internal_state.current_state
    ...
    def _get_columns_dtypes(self):
        """
        DataFrame containing the name of columns and database types of self.
        """
        name = self.internal_state.current_state
    ...
    def _reset_attributes(self, attributes):
        """
        Delete an attribute for self to force refreshing it at next call.
        """
...
###############################################################################
    ...
    def _table_def(self, factor_threshold=None):
        """
        Classify columns in the idaDataFrame into 4 classes: CATEGORICAL, STRING, NUMERIC or NONE.
        Use database datatype and a user-threshold "factor_threshold":
            * CATEGORICAL columns having a number of distinct values is greater than factor_threshold should be considered as STRING.
            * NUMERIC columns having a number of distinct values smaller or equal to factor_threshold should be considered as CATEGORICAL.
    ...
    def _get_numerical_columns(self):
        """
        Get columns of self considered as numerical.
        Decision is made according to their datatype in the database.
        Types to be considered as categorical are :
    ...
    def _get_categorical_columns(self):
        """
        Get columns of self considered as categorical.
        Decision is made according to their datatype in the database.
        Types to be considered as categorical are :
    ...
    def _prepare_and_execute(self, query, autocommit = True, silent = False):
        ...
        autocommit : bool
            If set to true, then the autocommit function is available.
        """
        return self._idadb._prepare_and_execute(query, autocommit, silent)

    def _autocommit(self):
        """
        Autocommit the connection.
        If the environment variable 'AUTOCOMMIT' is set to True, then commit.

        Notes
        -----

        In the case of a commit, all changes that are made in the Database
        after the last commit, including those in the children IdaDataFrames,
        will be commited.
    ...
    def _combine_check(self, other):
        """
        Check if self and other refers to the same table and if all columns in
        self and other are numeric. This sanity check is used before
        performing aggregation operation between IdaDataFrame/IdaSeries.
        """
