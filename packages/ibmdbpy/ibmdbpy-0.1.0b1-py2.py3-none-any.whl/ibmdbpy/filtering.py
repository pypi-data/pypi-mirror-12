...
class FilterQuery(object):
    """
    FilterQuery are used to represent the filtering of an IdaDataFrame
    in ibmdbpy. The use of comparison operator such as <, <=, ==, >=, >
    on a IdaDataFrame produce a FilterQuery instance which acts as a
    container for the where clause of the corresponding SQL request.

    Filtering is possible using Pandas-like syntax. Applying comparision
    operators to IdaDataFrames produce a FilterQuery instance which
    contains in its attribute "wherestr" the string embedding the
    corresponding where clause.

    Filterqueries objects also contains the logic that allow them to be
    combined to allow complex filtering.

    Combination is available for operators |, &, ^ (OR, AND and XOR)
    ...
    Notes
    -----
    One limit is that is not possible to filter an IdaDataFrame using a
    IdaDataFrame opened in a different data source in the database. This
    difference is due to the fact that, in Pandas-like syntax,
    "idadf['petal_width'] < 5" will return a boolean array which is then used
    to subset the original DataFrame. This is a fundamental restriction of
    ibmdbpy: we cannot afford to compute and download such an array because we
    cannot assume that the result will fit into user's memory. The download
    time can also be a performance issue.
    ...
    def __init__(self, columns, tablename, method, value):
        ...
        if not ((isinstance(value,six.string_types)|isinstance(value,Number))):
            raise IdaDataBaseError("Value for Filterquery is expected to be a string or a number.")
