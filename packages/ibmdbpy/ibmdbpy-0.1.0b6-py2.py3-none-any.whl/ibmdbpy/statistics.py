...
def _numeric_stats(idadf, stat, columns):
    """
    Computes various stats from one or several numerical columns of an IdaDataFrame.
    ...
    -------
    Tuple
        One value for each columns.

    Notes
    -----
    Currently supported functions are : count, mean, median,
    std, var, min, max, sum. Should return a tuple.
    For now available only for numerical columns
    """
...
def _get_percentiles(idadf, percentiles, columns):
    """
    Return percentiles over all entries of some columns in the IdaDataFrame.
    ...
    for index_col, column in enumerate(columns):
        ...
        #print(tuplelist)
        #print(indexvalues)
        indexfinal = [(indexvalues[x[0]]+indexvalues[x[1]])/2 for x in tuplelist]
...
def _get_number_of_nas(idadf, columns):
    """
    Returns the count of missing values for a list of columns of the IdaDataFrame.
...
def _count_level(idadf, columnlist=None):
    """
    Computes the count of dictinct levels accross a list of columns of an
    IdaDataFrame grouped by themself.

    Parameters
    ----------
    columnlist : list
        List of column names existing in the IdaDataFrame
        Default = all columns in IdaDataFrame
...
def _count_level_groupby(idadf, columnlist=None):
    """
    Computes the count of dictinct levels accross a list of columns in the
    IdaDataFrame grouped by themself. This is used to get the dimension of the
    resulting crosstable.

    Parameters
    ----------
    columnlist : list
        List of column names existing in the IdaDataFrame
        Default = columns of self
...
def _factors_count(idadf, columnlist, valuelist=None):
    """
    Computes the count of non missing values for all columns in a list
    (valuelist) over the IdaDataFrame grouped by a list of columns(columnlist).

    Parameters
    ----------
    columnlist : list
        List of column names existing in self.
    valuelist : list
        List of column names existing in self.
...
def _factors_sum(idadf, columnlist, valuelist):
    """
    Computes the arithmetic sum over for all columns in a list (valuelist)
...
def _factors_avg(idadf, columnlist, valuelist):
    """
    Computes the arithmetic average for all columns in a list (valuelist)
    over the IdaDataFrame grouped by a list of columns (columnlist).
