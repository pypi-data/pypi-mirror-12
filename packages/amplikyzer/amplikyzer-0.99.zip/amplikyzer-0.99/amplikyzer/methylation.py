...
MODES = {"cpg":      AnalysisMode("CpG", ("CG" , 0), ("CH" , 0)),
         "nome":     AnalysisMode("GpC", ("GCH", 1), ("HCH", 1)),
         "cpg-nome": AnalysisMode("CpG", ("HCG", 1), ("HCH", 1)),
         "gcg-nome": AnalysisMode("GpCpG", ("GCG", 1), ("HCH", 1)),
         "wcg-nome": AnalysisMode("CpG", ("WCG", 1), ("HCH", 1))}
MODES_LIST = ("cpg", "nome", "cpg-nome", "gcg-nome", "wcg-nome")
...
def buildparser(p):
    ...
    p.add_argument("--mode",
        ...
        help=("analyze methylation of CpGs with mode 'cpg' in normal "
              "(i.e. non-NOMe-seq) reads. "
              "For NOMe-seq data use 'nome' to analyze GpCs or 'cpg-nome' for CpGs. "
              "More than one mode can be specified (e.g. '--mode nome cpg-nome ...'). "
              "NOTE: The overlapping pattern GpCpG is excluded in NOMe-seq analysis."))
    ...
    show.add_argument("--show",
        ...
        help="show simple indices, positions or Cytosine indices. "
             "Multiple options can be specified (e.g. '--show index position ...'.")
...
def main(args):
    ...
    with utils.get_executor(args.parallel) as executor:
        ...
        if "all" in mode_names:
            mode_names.update(OrderedDict.fromkeys(MODES_LIST))
            del mode_names["all"]
        if len(mode_names) > 1:
