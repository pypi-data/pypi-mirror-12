...
def get_session_token(gk_auth_server, token):
    url = '{auth_server}/v1/session/open/auth'.format(auth_server=gk_auth_server)
...
def get_username(gk_auth_server, session):
    url = '{auth_server}/v1/user'.format(auth_server=gk_auth_server)
...
def login_as_user(client, docker_registry, auth_server, gk_auth_server, username=None, token=None, retry=3):
    if token:
        psw = get_session_token(gk_auth_server, token)
        usr = get_username(gk_auth_server, psw)
    else:
        if retry == 0:
            return False
        if username:
            ...
            usr = input("Username: ")
        if not usr:
            login_as_user(client, docker_registry, auth_server, gk_auth_server, username=None, retry=retry-1)
        ...
        if response.status_code in (401, 402):
            ...
            return login_as_user(client, docker_registry, auth_server, gk_auth_server, username=usr, retry=retry-1)
        psw = response.json()['session_id']
