...
class PygCatalog(object):
    ...
    def describe_table(self, tablename, format='Markdown', **kwargs):
        """Return the table's schema

        """
        schema = self._which_schemas(**kwargs)

        columns = self._get_columns_extended(tablename, schema)
    ...
    def get_operator_class(self, name, **kwargs):
        ...
        rows = self._execute_sql(qry, (name,))

    def get_triggers(self, name, **kwargs):
        ...
        """
        qry = """
              SELECT tgame, proname
              FROM pg_catalog.pg_trigger as t
              INNER JOIN pg_catalog.pg_proc as p
              WHERE t.tgfoid = p.oid
              """

        rows = self._execute_sql(qry)
