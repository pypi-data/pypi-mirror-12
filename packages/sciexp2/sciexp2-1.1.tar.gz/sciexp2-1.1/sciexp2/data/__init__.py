    ...
    * a string with the first argument for the `meta.Dim` constructor
    ...
    def __init__(self, values):
        ...
        if isinstance(values, DataDims):
            self._dims = np.array(list(values.copy()), dtype=object)
...
class DataIndexer(pp.Pretty):
    ...
    def __init__(self, array=None, dims=None, indexes=None):
        ...
        if dims is None:
            # get a copy to avoid modifying the originals
            # (the ticks are not necessary)
            dims = []
            for d in array.dims:
                try:
                    d = meta.Dim(d.expression)
                except meta.DimExpressionError:
                    d = meta.Dim()
                dims.append(d)
            self._dims = DataDims(dims)
        else:
            self._dims = dims
        if indexes is None:
