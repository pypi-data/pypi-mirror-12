...
class Launchgen (pp.Pretty):
    ...
    contents : optional
        Initial contents.
    out : optional
        Initial value for the `out` attribute (defaults to `OUTPUT_DIR`).
    dereference : bool, optional
        Whether to dereference source symlinks during pack (defaults to
        `PACK_DEREFERENCE`).
    ...
    def __init__(self, contents=None, **kwargs):
        ...
        if out is None:
            out = self.OUTPUT_DIR
        if dereference is None:
            dereference = self.PACK_DEREFERENCE
    ...
    def params(self, *filters, **params):
        ...
        If a variable in `params` is already present in `contents`, the latter
        is overwritten with the former (note that new values can reference the
        old ones).
    ...
    def _pack(self, select, from_expr, to_expr, dereference):
        if dereference is None:
    ...
    def _expand(self, select, expression, *filters, **kwargs):
        with_view = kwargs.pop("with_view", False)
        utils.assert_kwargs(kwargs)

        use_filter = select is not None or len(filters) > 0
        ...
        expansions = _EXPANDER.expand(expression, self._contents,
                                      filter_=filter_ if use_filter else None,
                                      recursive=True,
    ...
    def expand(self, expression, *filters, **kwargs):
        ...
        filters : filter, optional
            Expand only selected instances.
        ...
        [('0', Launchgen([Instance({'a': 0, 'b': 'foo'}),
         ...
         ('2', Launchgen([Instance({'a': 2, 'b': 'foo'}),
                          Instance({'a': 2, 'b': 'bar'})],
                         out='./out'))]
        >>> l.expand("@a@", "b == 'foo'", with_view=True) #doctest: +NORMALIZE_WHITESPACE
        [('0', Launchgen([Instance({'a': 0, 'b': 'foo'})],
                         out='./out')),
         ('2', Launchgen([Instance({'a': 2, 'b': 'foo'})],
                         out='./out'))]
        ...
        """
        return self._expand(None, expression, *filters, **kwargs)
    ...
    def _launcher(self, select, template, to_expr, *export, **values):
        ...
        to_expr_flat = _EXPANDER.flatten(to_expr, group)
        export_ = set(tpl_system.assumes())
        export_ |= depends
        if len(export) > 0:
            export_ |= set(export)
        else:
            export_ |= set(varref.expr_get_vars("@", to_expr)) |\
                       set(varref.expr_get_vars("@", to_expr_flat))
        export = export_
        if len(export - set(group.variables())) > 0:
            raise ValueError("Unknown variables to export: " +
                             ", ".join(export - set(group.variables())))

        # generate job descriptor file
        launcher.save(jd, jd_base, tpl.system, group, export, depends,
                      submit_args)
    ...
    def launcher(self, template, to_expr, *export, **values):
        ...
        export : list of str, optional
            Variable names that will be made available in the job descriptor
            file (later available to *launcher* filters).
        values : dict
            Additional variables to translate in destination files.
        ...
        *LAUNCHER* Expansion of `to_expr`.
        ...
        The following variables can be specified in argument `values`:
