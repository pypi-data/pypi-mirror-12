...
ATdistance (Operator* ops, int baselat, int baselong, int pydistance, char operate, int set)
{
	...
	baselatf = (baselat / 10000.0) - 180.0;
	baselongf = (baselong / 10000.0) - 180.0;

	delta = (int) (distance / (LATITUDE1DEGREE / 10000.0)); /* meters per 1 / 10000 degree */
	maxlat = baselat + delta;
	minlat = baselat - delta;
	delta = (int) (distance / (LONGITUDE1DEGREE / 10000.0));
	...
	if (load) {
		for (ic  = ops->scorer->numdoc, i = 0; ic-- ; i++) {
			...
			/*
				Simple Measurement
				tdistance = sqrt (pow (LATITUDE1DEGREE * ((double)(otherlat - baselat) / 10000.0), 2.0) + pow (LONGITUDE1DEGREE * ((double)(otherlong - baselong) / 10000.0), 2.0));
			*/
			tdistance = mdistance (baselatf, baselongf, (double)(otherlat / 10000.0 - 180.0), (double)(otherlong / 10000.0 - 180.0));
