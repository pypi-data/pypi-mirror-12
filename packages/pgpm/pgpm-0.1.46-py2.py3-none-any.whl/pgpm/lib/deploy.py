...
class DeploymentManager:
    ...
    """
    def __init__(self, connection_string, source_code_path=None, config_path=None, config_dict=None,
        ...
        self._config = None
        self._set_config(config_path, config_dict)
        self._pgpm_version = pgpm.lib.utils.config.Version(pgpm.lib.version.__version__,
                                                           pgpm.lib.utils.config.VersionTypes.python)

    def _set_config(self, config_path=None, config_dict=None):
        """
        Sets configuration object by getting info from a file or/and a dict
        ...
        if config_dict and config_file:
            config_dict = dict(json.load(config_file) + list(config_dict.items()))
            config_file.close()
        elif config_file:
            config_dict = json.load(config_file)
            config_file.close()
        if config_dict:
            self._config = pgpm.lib.utils.config.SchemaConfiguration(config_dict, project_path)
    ...
    def deploy_schema_to_db(self, mode='safe', files_deployment=None, vcs_ref=None, vcs_link=None,
                            ...
                            config_path=None, config_dict=None, source_code_path=None):
        """
        Deploys schema
        ...
        self._logger.debug('Configuration of project {0} of version {1} loaded successfully.'
        ...
        # Get scripts
        type_scripts_dict = self._get_scripts(self._config.types_path, files_deployment, "types")
        function_scripts_dict = self._get_scripts(self._config.functions_path, files_deployment, "functions")
        view_scripts_dict = self._get_scripts(self._config.views_path, files_deployment, "views")
        trigger_scripts_dict = self._get_scripts(self._config.triggers_path, files_deployment, "triggers")
        table_scripts_dict = self._get_scripts(self._config.tables_path, files_deployment, "tables")
        ...
        if len(table_scripts_dict) > 0:
            ...
            for key, value in sorted_table_scripts_dict.items():
                pgpm.lib.utils.db.SqlScriptsHelper.set_search_path(cur, self._pgpm_schema_name)
                cur.callproc('_is_table_ddl_executed'.format(self._pgpm_schema_name), [key])
    ...
    def _get_scripts(self, scripts_path_rel, files_deployment, script_type):
        """
        ...
        if scripts_path_rel:
            ...
            scripts_dict = pgpm.lib.utils.collect_scripts_from_sources(scripts_path_rel, files_deployment, False, self._logger)
