...
def get_submodules(filename, recursive=True, free_format=True):
    """
        ...
        for m in prog.findall(content):
            for m in m:
                ...
                for ext in get_compilable_extensions(
                        include_preparsers=True,
                        include_upper=not api.system.WINDOWS):
                    ext = ext.replace('*', '')
                    pth = os.path.join(directory, module_base_name + ext)
                    if os.path.exists(pth) and \
                            pth.lower() not in submodules:
                        if filename != pth and pth:
