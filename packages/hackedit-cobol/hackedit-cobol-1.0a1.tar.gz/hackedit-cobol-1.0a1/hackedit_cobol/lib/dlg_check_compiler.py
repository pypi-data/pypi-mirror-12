...
class DlgCheckCompiler(QtWidgets.QDialog):
    ...
    def _check_compiler(self):
        ...
        if exit_code == 0:
            ...
            msg = '<h1>GnuCOBOLCompiler check failed!</h1>'
            self.ui.textEdit.setText(msg)
            self.ui.textEdit.append('<p><b>Exit code<b>: %d</p>' % exit_code)
            self.ui.textEdit.append('<p><h2>GnuCOBOLCompiler output:</h2></p>')
            self.ui.textEdit.setTextColor(QtCore.Qt.red)
            self.ui.textEdit.append(output.strip())
            tips = '<h2>Tips:</h2><p><i>  - You might need to adapt the ' \
                   'environment variables set by the IDE to make it work.'\
                   '</i></p>'
            if system.WINDOWS:
                tips += "<p><i>  - Also make sure that you don't have a " \
