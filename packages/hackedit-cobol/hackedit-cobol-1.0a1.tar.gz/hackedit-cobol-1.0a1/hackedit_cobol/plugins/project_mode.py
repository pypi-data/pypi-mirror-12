...
class CobProjectManager(CobolManagerPluginBase):
    ...
    def setup_ui(self):
        ...
        self.mnu_run_configs = cob_menu.addMenu('Run configurations')
        self.combo_run_configs = QtWidgets.QComboBox()
        self.combo_run_configs.setToolTip('The list of run targets')
    ...
    def apply_preferences(self):
        ...
        self.a_rebuild.setShortcut(
            api.shortcuts.get('Rebuild', 'Ctrl+Alt+B'))
    ...
    def load_run_configs(self):
        ...
        self.combo_run_configs.blockSignals(True)
        ...
        for cfg in configs:
            try:
                ...
                name = QtCore.QFileInfo(cfg['path']).baseName()
        ...
        self.combo_run_configs.blockSignals(False)
        ag.triggered.connect(self._on_action_run_cfg_triggered)

    def configure(self):
        prj = api.project.get_current_project()
        ret_val = DlgConfigureProject.configure(prj, self._window)
        if ret_val:
            self.configuration_changed.emit()
            return True
        return False

    def get_preparser_cobol_output(self, cfg):
    ...
    def clean(self):
        ...
        ev = api.events.Event(
            'Clean finished', 'The following files were removed:\n  -%s' %
                              '\n  -'.join(removed_paths) if removed_paths else
            'There were no file to remove...')
        api.events.post(ev, force_show=True,
                        show_balloon=not self._window.isVisible())
        self.enable_build_actions()
    ...
    def build(self):
        if self.build_thread:
            ...
            self.a_build.setText('Cancel build')
            self.a_build.setToolTip('Cancel build')
    ...
    def get_executable_path(self, cfg):
        project_path = api.project.get_current_project()
    ...
    def _run_embedded(self, cfg, path):
        ...
        if cfg['filetype'] == MODULE:
            ...
            args = [QtCore.QFileInfo(path).baseName()] + \
                cfg['run']['arguments']
        else:
            pgm = path
...
class DlgConfigureProject(DlgConfigureBase):
    ...
    def __init__(self, project_path, parent):
        ...
        row = 0
        for i, file_cfg in enumerate(config):
    ...
    def _update_current_config_display(self, _):
        if self._update_flg is False:
            ...
            self._display_config(self.ui.list_build.currentItem().data(
                QtCore.Qt.UserRole))
            self._update_flg = False
    ...
    def _rm_file(self):
        ...
        a = QtWidgets.QMessageBox.question(
            self, 'Remove file from build',
            'Are you sure you want to remove %r from the project build '
    ...
    def _add_file(self):
        ...
        path, _ = QtWidgets.QFileDialog.getOpenFileName(
            self, 'Add file to build', self.project_path,
            get_compilable_filter() + ';; All files (*.*)')
        if path and os.path.exists(path):
            rel_path = os.path.relpath(path, self.project_path)
            if rel_path.startswith('..'):
                answer = QtWidgets.QMessageBox.question(
                    self, 'File outside of project root!',
                    'The file is outside of the project root! This might cause'
                    ' some issues if you put the project configuration under '
                    'versioning and open it on another environemnt.\n\n'
    ...
    @classmethod
    def configure(cls, project_path, parent):
        """
        ...
        if dlg.exec_() == dlg.Accepted:
            ...
            try:
                ...
                try:
                    current = dlg.ui.list_build.currentItem().text()
                except AttributeError:
                    pass  # empty project config
                else:
                    set_current_run_config(
                        project_path, QtCore.QFileInfo(current).baseName())
                return True
        return False
...
def get_secondary_compilers(project_path):
    ...
    usd = api.project.load_user_config(project_path)
    try:
        return usd['secondary-compilers']
    except KeyError:
        return []
