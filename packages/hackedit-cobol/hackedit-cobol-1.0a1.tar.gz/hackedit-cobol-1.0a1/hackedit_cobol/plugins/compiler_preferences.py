...
class CompilerPreferences(api.widgets.PreferencePage):
    ...
    def _load_config(self, index):
        ...
        if cfg is not None:
            ...
            self.ui.edit_compiler_path.setEnabled(not default)
            self.ui.edit_cobcrun_path.setText(cfg.get('cobcrun', ''))
            self.ui.edit_cobcrun_path.setEnabled(not default)
            ...
            self.ui.edit_vcvars.setEnabled(not default)
            self.ui.combo_arch.setEnabled(not default)
            self.ui.edit_output_dir.setText(cfg['output-directory'])
            self.ui.edit_output_dir.setEnabled(not default)
            self.ui.cb_debug.setChecked(cfg['-debug'])
            self.ui.cb_debug.setEnabled(not default)
            self.ui.cb_static.setChecked(cfg['-static'])
            self.ui.cb_static.setEnabled(not default)
            self.ui.cb_g.setChecked(cfg['-g'])
            self.ui.cb_g.setEnabled(not default)
            self.ui.cb_ftrace.setChecked(cfg['-ftrace'])
            self.ui.cb_ftrace.setEnabled(not default)
            self.ui.cb_ftraceall.setChecked(cfg['-ftraceall'])
            self.ui.cb_ftraceall.setEnabled(not default)
            self.ui.cb_debugging_line.setChecked(cfg['-fdebugging-line'])
            self.ui.cb_debugging_line.setEnabled(not default)
            self.ui.edit_extra_flags.setEnabled(not default)
            self.ui.edit_extra_flags.setText(
                ' '.join(cfg['extra-compiler-flags']))
            self.ui.list_copy_paths.clear()
            self.ui.list_copy_paths.setEnabled(not default)
            for path in cfg['copybook-paths']:
                self.ui.list_copy_paths.addItem(path)
            self.ui.list_lib_paths.clear()
            self.ui.list_lib_paths.setEnabled(not default)
            for path in cfg['library-paths']:
                self.ui.list_lib_paths.addItem(path)
            self.ui.edit_libs.setText(' '.join(cfg['libraries']))
            self.ui.edit_libs.setEnabled(not default)
            self.ui.bt_abs_copy_path.setEnabled(not default)
            self.ui.bt_abs_lib_path.setEnabled(not default)
            self.ui.bt_rel_copy_path.setEnabled(not default)
            self.ui.bt_rel_lib_path.setEnabled(not default)
            self.ui.bt_rm_copy_path.setEnabled(not default)
            self.ui.bt_delete_lib_path.setEnabled(not default)
            self.ui.bt_choose_compiler_path.setEnabled(not default)
            self.ui.bt_choose_cobcrun_path.setEnabled(not default)
            self.ui.bt_vcvars.setEnabled(not default)
            ...
            self.ui.lbl_cobc_version.setText(
                'cobc v%s' % GnuCOBOLCompiler.get_version(
                    cfg, include_all=False))
        else:
        ...
        while not ok:
            ...
            if not ok:
                QtWidgets.QMessageBox.warning(
                    self, 'Invalid name', 'The name %r is already used for '
                    'another configuration. Please choose another name...' %
                    name)
        return name
    ...
    def _remove_cfg(self):
        answer = QtWidgets.QMessageBox.question(
            self, 'Remove compiler configuration',
            'Are you sure you want to remove the configuration %r?' %
    ...
    def _add_abs_copy_path(self):
        path = QtWidgets.QFileDialog.getExistingDirectory(
            self, 'Select a copybook path', os.path.expanduser('~'))
        if path:
    ...
    def _add_abs_lib_path(self):
        path = QtWidgets.QFileDialog.getExistingDirectory(
            self, 'Select a library path', os.path.expanduser('~'))
        if path:
    ...
    def _select_compiler_path(self):
        path, _ = QtWidgets.QFileDialog.getOpenFileName(
            self, 'Select path to a GnuCOBOL compiler executable (cobc).',
    ...
    def _select_cobcrun_path(self):
        path, _ = QtWidgets.QFileDialog.getOpenFileName(
            self, 'Select path to a GnuCOBOL module runner executable '
            '(cobcrun)', self.ui.edit_cobcrun_path.text())
        if path:
    ...
    def _get_compiler_info(self):
        ...
        DlgInfo.show_information(
            self, 'Compiler information', info)
    ...
    def _get_runner_info(self):
        ...
        DlgInfo.show_information(
            self, 'Module runner runtime environment', info)
...
class DlgInfo(QtWidgets.QDialog):
    ...
    @classmethod
    def show_information(cls, parent, title, information):
