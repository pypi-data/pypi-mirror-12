...
class BuildThreadBase(QtCore.QThread):
    ...
    def compile(self, cfg, compiler, abs_path, prj_path):
        ...
        try:
            ...
            if compile_submodules:
                submodules = utils.get_submodules(
                    abs_path, recursive=False,  # already recursive
                    free_format=cfg['free-format'])
    ...
    @classmethod
    def setup_overrides(cls, compiler_config, cfg, preparser=None):
        try:
