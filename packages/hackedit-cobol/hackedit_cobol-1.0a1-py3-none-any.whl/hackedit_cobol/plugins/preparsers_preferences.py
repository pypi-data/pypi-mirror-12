...
class PreparserPreferences(api.widgets.PreferencePage):
    ...
    def __init__(self):
        ...
        super().__init__(
            'Preparsers', icon=icon, category='COBOL')
    ...
    def add(self):
        ...
        for i in range(self.ui.combo_configs.count()):
            try:
                choices.remove(self.ui.combo_configs.itemText(i))
            except ValueError:
                pass
        ...
        if len(choices) > 1:
            item, status = QtWidgets.QInputDialog.getItem(
                self, 'Add preparser',
                'What kind of COBOL preparser would you like to add?',
                choices, editable=False)
        else:
            item = CUSTOM_CONFIG_NAME
            status = True
        if status:
            config = self._get_initial_config(item, choices)
            if config:
                configs = get_configs()
    ...
    def _get_initial_config(self, item, choices):
        forbidden_names = [
        ...
        if item == dbpre.DBPRE_NAME:
            ...
            name, status = QtWidgets.QInputDialog.getText(
                self, 'Preparser configuration name', 'Please, enter a '
                'name for the preparser configuration you want to add: ')
            if not status:
                return None
    ...
    def _select_parser_path(self):
        path, _ = QtWidgets.QFileDialog.getOpenFileName(
            self, 'Select preparser executable',
