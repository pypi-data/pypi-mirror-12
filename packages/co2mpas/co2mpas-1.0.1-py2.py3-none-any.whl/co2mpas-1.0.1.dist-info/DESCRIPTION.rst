...
.. Tip::
    - Commands beginning with ``$`` symbol are for the *bash-console* (UNIX)
      ...
      the *all-in-one* distribution-archive (see ref:`begin-install`_ below).
...
under sections ref:`begin-install`_ and ref:`begin-usage`_.
...
4. Use this console and follow CO2MPAS-executable installation instructions
   (see ref:`begin-co2mpas-install`_, below)
...
you have installed CO2MPAS (see ref:`begin-install`_ above) and type:
...
6. In the case of errors, or if the results are not satisfactory, repeat the
   ...
   See also ref:`begin-debug`_, below.
