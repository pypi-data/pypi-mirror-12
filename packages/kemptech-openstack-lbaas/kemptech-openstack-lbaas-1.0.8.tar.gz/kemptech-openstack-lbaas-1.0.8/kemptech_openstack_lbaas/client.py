...
class KempClient(object):
    ...
    def update_virtual_service(self, old_params, params):
        old_virtual_service = VirtualService(self.loadbalancer, old_params)
        virtual_service = VirtualService(self.loadbalancer, params)
        try:
            old_virtual_service.update(virtual_service)
        except KempClientRequestError:
            raise
    ...
    def update_real_server(self, old_params, params):
        ...
        real_server = RealServer(self.loadbalancer, params)
        try:
            old_real_server.update(real_server)
        except KempClientRequestError:
            raise
    ...
    def update_health_check(self, check_params, os_health_monitor, def_health):
        """Update health check parameters for virtual services."""
        # pop the checker params, call modhealth with those
        # create a health and a vs object for the rest of the params
        checkers = {}
        try:
            if (check_params['retryinterval'] > 8 and
                    check_params['timeout'] > 3 and
                    check_params['retrycount'] > 1):
                for checker_opt in kemp_consts.CHECKER_OPTS:
                    if checker_opt in check_params.keys():
                        # convert from neutron to kemp
                        checkers[checker_opt] = check_params[checker_opt]
                        check_params[checker_opt] = None
            else:
                checkers = def_health
        except KeyError:
            checkers = def_health
        ...
        new_vs_health = {
            'port': os_health_monitor.pool.listener.protocol_port,
            'prot': 'tcp',
            'vs': os_health_monitor.pool.listener.loadbalancer.vip_address,
        }
        virtual_service = VirtualService(self.loadbalancer, new_vs_health)
        if os_health_monitor.type.lower() == "ping":
            virtual_service.checktype = "icmp"
        else:
            virtual_service.checktype = os_health_monitor.type.lower()
        virtual_service.update(virtual_service)
        try:
...
class BaseKempModel(object):
    ...
    def exists(self):
        ...
        """
        return self._get_request('show' + self.api_name, self.id)[0] < 300
    ...
    def to_dict(self):
        ...
        for attr, value in self.__dict__.items():
            if value is None:
                continue
            if attr != 'id' and attr != 'api_name' and attr != 'loadbalancer':
