...
class PoolManager(driver_base.BasePoolManager):
    ...
    def create(self, context, pool):
        ...
        prep_pool = self.api_helper.prepare_listener(context, pool.listener,
                                                     pool=pool)
        try:
            self.client.update_virtual_service(prep_pool, prep_pool)
            self.driver.openstack_driver.pool.successful_completion(
                context, pool
            )
        except KempClientRequestError:
            self.driver.openstack_driver.pool.failed_completion(context, pool)
...
class HealthMonitorManager(driver_base.BaseHealthMonitorManager):
    ...
    def __init__(self, driver):
        ...
        self.client = self.driver.client
        self.api_helper = self.driver.api_helper
        self.default_health_check = driver.default_checker_settings
    ...
    def create(self, context, health_monitor):
        ...
        params = self.api_helper.prepare_health_monitor(context,
                                                        health_monitor)
        check_params.update(params)
        try:
            self.client.update_health_check(check_params, health_monitor,
                                            self.default_health_check)
...
class MapNeutronToKemp(object):
    ...
    def prepare_listener(self, context, listener, pool=None):
        ...
        if pool is not None:
            if pool['healthmonitor_id'] is not None:
                health_mon = self.plugin.get_healthmonitor(
                    context,
    ...
    def prepare_member(self, member):
        rs_params = {
            'port': member.pool.listener.protocol_port,
            'prot': 'tcp',
            'vs': member.pool.listener.loadbalancer.vip_address,
        }
