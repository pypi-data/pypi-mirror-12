...
class TestMapNeutronToKemp(object):
    ...
    def setup(self):
        ...
        self.plugin.get_healthmonitor.return_value = self.hm
        self.session_persistence = SessionPersistence()
        self.listener = Listener()

    def test_prepare_listener_with_no_pool(self):
    ...
    def test_prepare_listener_pool_with_round_robin(self):
        self.listener.protocol_port = 80
        self.listener.protocol = "HTTP"
    ...
    def test_prepare_listener_pool_with_least_connections(self):
        self.listener.protocol_port = 80
        self.listener.protocol = "HTTP"
        self.listener.default_pool_id = 1
        self.pool['lb_algorithm'] = "LEAST_CONNECTIONS"
        result = self.api_helper.prepare_listener(self.context, self.listener)
        expect = {'vs': '10.0.0.1', 'port': 80, 'schedule': 'lc', 'prot': 'tcp'}
        assert_equal(result, expect)

    def test_prepare_listener_pool_with_source_ip(self):
        self.listener.protocol_port = 80
        self.listener.protocol = "HTTP"
        self.listener.default_pool_id = 1
        self.pool['lb_algorithm'] = "SOURCE_IP"
        result = self.api_helper.prepare_listener(self.context, self.listener)
        expect = {'vs': '10.0.0.1', 'port': 80, 'schedule': 'sh', 'prot': 'tcp'}
        assert_equal(result, expect)
