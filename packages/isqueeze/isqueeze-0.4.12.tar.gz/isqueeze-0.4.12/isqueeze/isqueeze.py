...
class isqueeze(object):
	...
	def recherche_radio(self):
		...
		radios = self.SB.radios() # En fait ça ne marche pas!!!
	...
	def get_ampli_status(self):
		'''Renvoie True si l'ampli est en ecoute sur la squeezeBox
			False sinon
			None si pas d'information de l'ampli
		'''
		ampli_status = self.ampli.get_power()
		if ampli_status == None:
			return None
		elif ampli_status == 'on':
			time.sleep(0.3)# pour donner du temps à l'ampli de répondre (A supprimer???)
	...
	def check_ampli(self):
		...
		if self.ampli!=None:
			...
			logging.debug("ampli_status = " + str(ampli_status)) 
			logging.debug("self.ampli_status = " + str(self.ampli_status)) 
			#Si le status de l'ampli change, on re-test une seconde fois.
			if self.ampli_status != ampli_status:
				logging.debug("ampli_status change! check again.") 
				prec_ampli_status = ampli_status
				time.sleep(0.33)
				ampli_status = {'ampli' : self.get_ampli_status(), 'sb' : self.SB.get_mode()}
				logging.debug("ampli_status = " + str(ampli_status)) 
				#Si la lecture n'est pas confirmée, on revient à la valeur précédente
				if ampli_status != prec_ampli_status:
					ampli_status = self.ampli_status
					logging.debug("Change canceled.")
				else:
					logging.debug("Change confirmed.")
			...
			if self.ampli_status['sb']=='play' and ampli_status['sb']=='stop':
				...
				if ampli_status['ampli']==False:
					logging.info("Ampli set on by squeezebox (stop => play)")
					time.sleep(0.5)
					self.ampli.on()
					timeout = time.time()+5
					while time.time() < timeout and self.ampli.get_input() != self.ampli_input:
						time.sleep(0.5)
						self.ampli.set_input(self.ampli_input)
					
			elif self.ampli_status['ampli'] and ampli_status['ampli']==False:
				if ampli_status['sb']!='stop':
					logging.info("SqueezeBox set off by ampli.")
					self.SB.stop()
			elif self.ampli_status['ampli']==False and ampli_status['ampli']:
				if ampli_status['sb']!='play':
					logging.info("SqueezeBox set on by ampli.") 
					self.SB.play()
			if ampli_status['ampli']!=None and ampli_status['sb']!=None:
				if self.ampli_status != ampli_status:
					self.ampli_status = ampli_status
					logging.debug("self.ampli status changed : " + str(self.ampli_status))
		time.sleep(5)
		time.sleep(5)
