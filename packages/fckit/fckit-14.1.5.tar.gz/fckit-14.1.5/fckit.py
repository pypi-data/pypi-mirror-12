...
def Path(component, *components):
	return os.path.expanduser(os.path.join(component, *components))
...
class BuildTarget(object):
	"manage the incremental build of a target from its sources"

	def __init__(self, path, on_build, phony = False, sources = None, on_clean = None, on_digest = None):
		assert not phony or on_build, "missing phony target build handler"
		self.hmap_path = os.path.join(os.path.dirname(path), ".%s.hmap" % os.path.basename(path))
		self.on_digest = on_digest # required if the target is not a file (e.g. directory)
		self.on_build = on_build
		self.on_clean = on_clean
		self.sources = sources or ()
		self.phony = phony  # a phony target has no output and is always updated
		self.path = path
	...
	def digest(self):
		"return user-supplied digest() result or return the digest of the file content"
		assert not self.phony, "illegal call on phony target"
		assert self.exists(), "illegal call on missing target -- call build() first"
		if self.on_digest:
			self.on_digest(self.path)
		else:
			with open(self.path, "r") as fp:
				return md5.new(fp.read()).hexdigest()

	def build(self):
		"return True if the target has been updated (i.e. callback called), False otherwise"
		src_builds = [src.build() for src in self.sources]
		if self.phony:
			self.on_build(src.path for src in self.sources)
			return True
		elif not os.path.exists(self.hmap_path)\
		and self.exists() and all(not updated for updated in src_builds):
			trace("assuming target '%s' is up to date" % self.path)
			return False
		else:
			oldhmap = unmarshall(self.hmap_path, extname = ".json", default = {})
			newhmap = {os.path.abspath(src.path): src.digest() for src in self.sources if not src.phony}
			if self.exists() and newhmap == oldhmap:
				trace("target '%s' is up to date" % self.path)
				return False
			else:
				trace("updating target '%s'" % self.path)
				dirname = os.path.dirname(self.path)
				if dirname and not os.path.exists(dirname):
					mkdir(dirname) # create parent directories
				marshall(
					obj = newhmap,
					path = self.hmap_path,
					extname = ".json",
					overwrite = True) # save new hmap
				if callable(self.on_build):
					self.on_build(
						tgtpath = self.path,
						srcpaths = tuple(src.path for src in self.sources))
				elif self.on_build:
					shutil.copyfile(self.hmap_path, self.path)
				else:
					raise Error(self.path, "target cannot be built")
				assert self.exists(), "missing expected target"
				return True

	def clean(self):
		if os.path.exists(self.hmap_path):
			remove(self.hmap_path)
		if not self.phony and self.on_build and self.exists():
			remove(self.path)
		if self.on_clean:
			self.on_clean(self.path)
