...
class BuildTarget(object):
	...
	def __init__(self, path, callback, phony = False, sources = None, filtered = None):
		self.hmap_path = os.path.join(os.path.dirname(path), ".%s.hmap" % os.path.basename(path))
		self.callback = callback
		self.filtered = filtered # for a directory target, allow to filter entries
	...
	def digest(self):
		...
		if os.path.isdir(self.path):
			...
			hmap = self.filtered(hmap) if self.filtered else hmap
	...
	def build(self):
		...
		for src in self.sources:
			src.build()
		if self.phony:
			self.callback(self.sources) # a phony target has no output and is always updated
			return True
		else:
