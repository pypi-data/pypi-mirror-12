...
class BuildTarget(object):
	...
	def build(self):
		...
		src_changed = any(src.build() for src in self.sources)
		if self.phony:
			self.on_build(src.path for src in self.sources) # a phony target has no output and is always updated
			return True
		elif self.exists() and not os.path.exists(self.hmap_path) and not src_changed:
			trace("assuming '%s' is up to date, clean everything to update" % self.path)
			return False
		else:
