...
class BuildTarget(object):
	...
	def __init__(self, path, on_build, phony = False, sources = None, on_digest = None):
	...
	def build(self):
		...
		if self.phony:
			...
			if self.exists() and newhmap == oldhmap:
				...
				if self.exists():
					remove(self.path) # delete obsolete object
				if not os.path.exists(os.path.dirname(self.path)):
					mkdir(os.path.dirname(self.path)) # create parent directories
				marshall(
					obj = newhmap,
					path = self.hmap_path,
					extname = ".json",
	...
	def clean(self, purge = False):
		...
		if purge and not self.phony and self.exists():
			remove(self.path)
		for src in self.sources:
