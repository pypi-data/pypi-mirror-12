...
class BuildTarget(object):
	...
	def build(self):
		...
		for src in self.sources:
			src.build()
		if self.phony:
			self.on_build(self.sources) # a phony target has no output and is always updated
			return True
		else:
			oldhmap = unmarshall(self.hmap_path, extname = ".json", default = {})
			newhmap = {src.path: src.digest() for src in self.sources if not src.phony}
			if self.exists() and newhmap == oldhmap:
				trace("target '%s' is up to date" % os.path.basename(self.path))
				return False
			else:
				if self.exists():
