...
class UnscopedChecker(Checker):
    ...
    def __init__(self, func, to_str_func=None):
        ...
        if not callable(self._func):
            raise ValueError(
                "Cannot create an UnscopedChecker from func={func}.  "
...
class ScopedChecker(Checker):
    ...
    def __init__(self, func, to_str_func=None):
        ...
        if not callable(self._func):
            raise ValueError(
                "Cannot create an ScopedChecker from func={func}.  "
...
class NamedTupleArgChecker(ScopedChecker):
    def __init__(self, elements):
        """
        An ArgChecker that is satisfied if the argument
        whose ith element satisfies the ith checker.
        """
        self._arg_names = tuple(n for n, c in elements)
        self._arg_checkers = tuple(make_arg_checker(c) for n, c in elements)
        self._kwarg_checkers = {n: make_arg_checker(c) for n, c in elements}

        def f(arg, eval_scope):
            expected_length = len(self._arg_checkers)
            args, kwargs = arg
            try:
                arg_len = len(args) + len(kwargs)
            except TypeError as e:
                raise TypeCheckViolation(value=(args, kwargs),
                                         expected_type=Seq)
            if expected_length != arg_len:
                raise LengthMismatchViolation(value=(args, kwargs),
                                              expected_length=expected_length)
            try:
                for a, checker in zip(args, self._arg_checkers):
                    r = checker(a, eval_scope)
                    if not r:
                        return False
                for name in kwargs:
                    r = self._kwarg_checkers[name](kwargs[name], eval_scope)
                    if not r:
                        return False
            except CheckViolation as e:
                raise
        ...
        def to_str(s):
            return "(%s)" % (", ".join(["%s: %s" % (n, str(c)) for n, c in
                                        zip(s._arg_names, s._arg_checkers)]))
