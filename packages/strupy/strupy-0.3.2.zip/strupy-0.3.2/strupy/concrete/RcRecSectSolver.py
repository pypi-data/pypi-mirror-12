...
File version 0.2 date 2015-12-15
...
class RcRecSectSolver():
    ...
    def resist_forcetomoment(self, sect, dividenumber=10, progress=None):
        ...
        start_N = (self.resist(sect, 1*u.kN, 0*u.kNm, 0*u.kN, 0*u.kNm)[0] + self.resist(sect, -1*u.kN, 0*u.kNm, 0*u.kN, 0*u.kNm)[0]) / 2
        start_M = 0*u.kNm
        rysAp = sect.rysAp
        rysAn = sect.rysAn
        loop = 0
