...
class SQLWriter(object):
  ...
  def __init__(self, named_placeholder, unnamed_placeholder, typecaster, *, sort_columns=False):
    ...
    typecaster must be a string accepting two items: a subject and a db type
    """
  ...
  def to_placeholder(self, name=None, db_type=None):
    ...
    If name is given, returns a named placeholder, if name is None, returns the unnamed placeholder.
    """
    if name is None:
  ...
  def to_expression(self, lhs, rhs, op):
    ...
    At it's most basic, returns 'lhs op rhs' such as '5 + 3'. However, it also specially handles the
  ...
  def transform_op(self, op, value):
    ...
    and the '!=' operator must be replaced with ' is not '. This function handles that
    conversion. It's up to the caller to call this function only on comparisons and not on
    assignments.
    """
    if value is None:
      if _EQ_RE.match(op):
  ...
  def value_comparisons(self, values, comp="=", is_assignment=False):
    ...
    If is_assigment is False, transform_op will be called on each operator.
    """
