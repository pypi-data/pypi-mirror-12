...
class UtilConvertTestCase(unittest.TestCase):
  ...
  def test_to_date(self):
    dt = arrow.get(2014, 1, 1)
    dts = dt.strftime("%Y-%m-%d")
    self.assertEqual(cv.to_date(dt), dt.date())
    self.assertEqual(cv.to_date(dts), dt.date())
    self.assertEqual(cv.to_date(dt.timestamp), dt.date())
    self.assertEqual(cv.to_date(str(dt.timestamp)), dt.date())

  def test_to_datetime(self):
    dt = arrow.get(2014, 1, 1, 13, 15, 30)
    dts = dt.format("YYYY-MM-DD HH:mm:ss")
    self.assertEqual(cv.to_datetime(dt), dt)
    self.assertEqual(cv.to_datetime(dts), dt)
    self.assertEqual(cv.to_datetime(dt.timestamp), dt)
    self.assertEqual(cv.to_datetime(str(dt.timestamp)), dt)

  def test_to_time(self):
    self.assertEqual(cv.to_time("13:30:00"), time(13, 30))
  ...
  def test_formatted_date_time(self):
    dt = "1/31/2013"
    fmt = "M/DD/YYYY"
    to_date = cv.formatted_datetime(fmt)
    self.assertEqual(to_date(dt), arrow.get(2013, 1, 31))
