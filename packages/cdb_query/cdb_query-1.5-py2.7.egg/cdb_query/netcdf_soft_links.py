...
class create_netCDF_pointers:
    ...
    def record_paths(self,output,paths_list,var,time_frequency,years,months):
        #First order source files by preference:
        soft_links=create_soft_links.soft_links(time_frequency,paths_list,self.file_type_list,self.data_node_list,semaphores=self.semaphores)
        soft_links.create(output)
        return

    def record_meta_data(self,output,paths_list,var,time_frequency,years,months):
    ...
    def record_fx(self,output,paths_list,var):
        ...
        remote_data=remote_netcdf.remote_netCDF(path['path'].split('|')[0].replace('fileServer','dodsC'),path['file_type'],self.semaphores)
...
class read_netCDF_pointers:
    ...
    def retrieve(self,output,retrieval_function,
                    year=None,
                    month=None,
                    day=None,
                    min_year=None,
