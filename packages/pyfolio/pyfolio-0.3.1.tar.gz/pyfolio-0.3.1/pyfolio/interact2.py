...
for color, name in zip(colors, names):
    p.line('index', name, legend=name, name=name,
           source=source, color=color, line_width=2.)

p.line('index', 'portfolio', legend='portfolio', name='portfolio',
       source=source, line_width=3., line_dash='dashed', color='red')
...
def on_slider_change(obj, attr, old, new):
    ...
    xname, yname, values, colors = create_corr(
        latest_dt + (new - 1) * month, latest_dt + (new) * month)
