...
from AMSSettings import *

## using borg pattern for singletons
class AMSLogger:
    __shared_state = {}
    def __init__(self, log_filename=AMS_LOG_FILE_LOCATION):
        self.__dict__ = self.__shared_state
        logging.basicConfig(level=AMS_LOGGING,
                        format='AMS: %(asctime)s %(levelname)s : %(message)s',
                        filename=log_filename, filemode=AMS_APPEND_MODE)

        if logging.getLogger("requests") is not None:
            logging.getLogger("requests").setLevel(logging.WARNING)
        self.logger = logging.getLogger('AMSLogger')
    ...
    def log_and_print(msg):
        print msg
        AMSLogger().get_logger().info(msg)

    @staticmethod
    def log_and_print_error(msg):
        print msg
        AMSLogger().get_logger().error(msg)
...
class AMSAppConfig:
    ...
    def __init__(self):
        ...
        self._config_attributes = [AMSAPP_PORT]
    ...
    def init_from_string(self, jstr):
        try:
            ...
            AMSLogger.log_and_print_error('AMSAppConfig: failed json.load check syntax %s. %s' % (jstr, e))
            return False
        return self.validate_config()

    def init_from_file(self, file_json):
        if file_json is None:
            AMSLogger.log_and_print_error('AMSAppConfig file path cannot be null')
            rv = False
        else:
            try:
                ...
                if statinfo.st_size > 0:
                    with open(file_json) as data_file:
                        try:
                            ...
                            AMSLogger.log_and_print_error('AMSAppConfig: failed json.load check syntax %s. %s' % (file_json, e))
                            rv = False
                        else:
                            rv = True
                else:
    ...
    def validate_config(self):
        for v in self._config_attributes:
            if v not in self._config_dict:
                AMSLogger.log_and_print_error("AMSAppConfig: failed to find %s in AMSAppConfig.json" % v)
                return False
        return True
...
class AMSHelpers:
    ...
    def convert(the_input):
        if isinstance(the_input, dict) is True:
            ...
            for key, value in the_input.iteritems():
                rd[key.encode('utf-8')] = AMSHelpers.convert(value)
            return rd
        elif isinstance(the_input, list):
            return [AMSHelpers.convert(element) for element in the_input]
    ...
    def create_timedelta_none(td_str):
        ...
        if td_str is None or isinstance(td_str, basestring) is False:
            AMSLogger().get_logger().error('create_timedelta: unable to get timedelta string %s' % td_str)
            return None
            ...
            try:
                ...
                if day_str is not None:
                    try:
                        ...
                        AMSLogger().get_logger().error('create_timedelta: unable to get timedelta days %s' % td_str)
                        return None
                    if len(hour_str.split(" ")) != 2:
                        AMSLogger().get_logger().error('create_timedelta: unable to get timedelta space %s' % td_str)
                        return None
                    else:
                        hs = hour_str.split(" ")[1]
                        try:
                            ...
                            AMSLogger().get_logger().error('create_timedelta: unable to get timedelta hours %s' % td_str)
                            return None
            if hours > 23:
        ...
        if have_error is True:
            AMSLogger().get_logger().error('create_timedelta: unable to parse timedelta string %s' % td_str)
            return None
        else:
            return t
    ...
    def create_timedelta(td_str):
        t = AMSHelpers().create_timedelta_none(td_str)
        if t is None:
            return timedelta()
        if t.day > 0:
    ...
    def create_datetime_none(t_str):
        ...
        if have_error is True:
            td = AMSHelpers().create_timedelta_none(t_str) ## returns datetime
            if td is not None:
                return td
            else:
                return None
    ...
    def create_datetime(t_str):
        if t_str is None:
            ...
            t = AMSHelpers().create_datetime_none(t_str)
            if t is None:
                t = datetime.now()
        return t
