...
'''
    ...
    def atmosphere_hybrid_height_coordinate(self, axiscode):
        '''Can't calculate eta (because we don't know the height of the top
level), so we'll create dimension coordinate for model_level_number
instead.
...
C(k)=[1-eta_value(k)/eta_value(first_constant_rho_level)]**2 forlevels
less than or equal to first_constant_rho_level
...
:Returns:
    ...
    out : cf.DimensionCoordinate or None
        ...
        ac.standard_name = 'height_above_reference_ellipsoid'
        ac.long_name = 'height based hybrid coeffient Zsea'
        ...
        ac.id        = 'atmosphere_hybrid_height_coordinate_c'
        ac.long_name = 'height based hybrid coeffient C'
        domain.insert_aux(ac, axes=[zdim], copy=False)
...
:Parameters:
    ...
    um_version : number, optional
        ...
        header. Valid versions are, for example, ``402``, ``606.3``
        and ``1001``.  The default version is ``405``. The version is
        ignored if it can be inferred from the PP headers, which will
        generally be the case for files created at versions 5.3 and
