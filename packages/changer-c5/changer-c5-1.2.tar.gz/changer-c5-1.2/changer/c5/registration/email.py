...
def registration_complete_handler(request, redirect_url='/'):
    messages.add_message(request, messages.SUCCESS, 'Registration successful. An email has been sent to your inbox with which you can complete the registration process.')
...
class EmailRegistrationBackend(object):
    ...
    * The setting ``ACCOUNT_ACTIVATION_DAYS`` be supplied, specifying
      (as an integer) the number of days from changer.c5.registration during
    ...
    * The creation of the templates
    ...
    def register(self, request, **kwargs):
        ...
        SEND_EMAIL_ON_REGISTRATION = getattr(settings, 'SEND_EMAIL_ON_REGISTRATION', True)
        new_user = RegistrationProfile.objects.create_inactive_user(username=email,
                                                                    email=email,
                                                                    password=password,
                                                                    site=site,
                                                                    send_email=SEND_EMAIL_ON_REGISTRATION)
