...
class LengthEstimator(object):
	...
	def __get_factor__(self,abs_result):
		...
		if abs_result < (total * 0.01):
			return 1
		elif abs_result < (total * 0.1):
		 	return random.sample([2,4,20], 1)[0]
		elif abs_result < (total * 0.3):
		 	return random.sample([20,50,100,200], 1)[0]
		else:
			return int(log(abs_result)*300) / random.sample([1,2], 1)[0]

	def is_same_ratio(self,comp,boun):
		difference = abs((comp - self._complete) + (boun - self._boundary))
		if self._read_thresh <= difference:
			return 0
		else:
...
if __name__ == "__main__":
	#print run_simulator_par(300, 50, 11871, 510, 16, 100,N=1)
