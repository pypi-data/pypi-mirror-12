...
def send_transmit_request_2(msg, address=0xFFFFFFFF0000, options=0x01, frame_id=0x00, port='/dev/ttyUSB0', baudrate=9600, timeout=3.0):
    msg = msg.encode('utf-8')
    if not msg:
        return 0

    hex_message = '7E 00 {:02X} 01 {:02X} {:02X} {:02X} {:02X}'.format(
        len(msg) + 5,
        frame_id,
        (address & 0xFF00) >> 8,
        address & 0xFF,
    ...
    frame = bytearray.fromhex(hex_message)
    frame.extend(msg)
    ...
    print hex(int('0xFFFFFFFF0000',16))
    print frame;
