...
from generic.op import add, mul
...
class VecClassTest(unittest.TestCase):
    ...
    def test_dispatch(self):
        assert (Vec[2, float], Vec[2, int]) in add
        assert (Vec[2, float], int) in mul
        assert (int, Vec[2, float]) in mul
