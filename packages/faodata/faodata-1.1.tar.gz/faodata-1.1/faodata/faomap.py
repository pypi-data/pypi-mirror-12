...
def plot(bmap, data, cat, \
    ...
    >>> coast = map.drawcoastlines(color='grey')
    >>> countries = map.drawcountries(color='grey')
    >>> # Define categories
    >>> cat = [np.percentile(df['value'], pp) for pp in range(10, 100, 10)]
    >>> # Plot data
    >>> faomap.plot(map, df, cat, ndigits=0)
    >>> # Plot decorations
    >>> coords = ax.axis('off')
    >>> leg = map.ax.legend(loc=3)
    >>> txt = ax.set_title('%s population, %d' % (item, year), fontsize=15)
