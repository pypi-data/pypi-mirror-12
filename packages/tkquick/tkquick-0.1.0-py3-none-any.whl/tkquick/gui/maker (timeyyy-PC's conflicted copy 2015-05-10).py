...
def set_module_defaults(new_settings = None):
	'''
	...
	Remember to include the tkquick-maker.ini file with your installation, otherwise
	will get some buggy shit.
	...
	if new_settings:
		...
		with ignored(FileNotFoundError):			#if same entry just leave it (read write permissions etc)
...
class GuiMaker(tk.Frame, GuiApi):
	...
	def __init__(self, parent=None,app=None):	# app is assigned here to the gui_maker frame, so elements can call home
		...
		self.formRef= {}		# custom form refernces saved in here
		self.tbarRef ={}		# toolbar button references saved here 
		with ignored(tk.TclError):	# because this fails if i want to inbed this in a notebook or something
			self.make_menu_bar()	
	...
	def varCheck(self,funct,thewidg,dict_name,text=None):	
		...
		elif funct in (tk.Radiobutton, tk.Checkbutton, ttk.Checkbutton, ttk.Radiobutton):	
			tkvar = tk.IntVar()	
	...
	def winRef(self,key_name,widget,parent,lowerparent=None,app=None):	# IS THIS NEEDED TBD		
		"""
	...
	def _custom_form_evaluate(self,funct,text,dict_name,where,config,frm,h,j,default_packager=True):
		...
		if 'tkinter' in str(funct):							#if a tkinter object 
			...
			if text:									#pass in functions for setting etc
				print(text) #TBD LOL WTF IS THIS SHIT
				text()
