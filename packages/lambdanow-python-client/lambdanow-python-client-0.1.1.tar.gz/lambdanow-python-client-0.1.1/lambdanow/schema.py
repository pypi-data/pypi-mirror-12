    ...
    def validate_fields(self):
        ...
        for field in self.fields:
            ...
            if 'name' not in field_keys or 'type' not in field_keys:
                raise ValueError('Fields %s should provide name, type subfields' % field)

            if field['type'] not in self._FIELD_TYPES:
                raise ValueError('Field %s\'s type should be a valid one.' % field['name'])
