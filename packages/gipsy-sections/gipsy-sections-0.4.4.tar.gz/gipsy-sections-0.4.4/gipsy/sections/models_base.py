...
class SectionBase(UserStampedModel, ActivatorModel, TitleSlugDescriptionModel,
    ...
    def clone(self, section_content_object=None):
        """Make a duplicate of section along with all subsections

        :returns: cloned section object
        """
        old_section_pk = self.pk

        self.pk = None
        if section_content_object:
            self.section_object_id = section_content_object.pk
        self.save()

        old_section = self.__class__.objects.get(pk=old_section_pk)
        for section in old_section.sections:
            section.clone(section_content_object=self)
