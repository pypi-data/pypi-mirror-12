...
__date__    = '2015-June-1'
__url__     = 'https://engineering.purdue.edu/kak/distICP/ICP-2.0.html'
...
INSTALLATION:
    ...
    The ICP class was packaged using Distutils.  For installation, execute
    the following command-line in the source directory (this is the
...
Initializations and Interframe Homographies" by Han, Aeschliman, Park, and
...
#    for d in sorted(data_dict.keys(), lambda x,y: cmp(int(x.lstrip('md')),int(y.lstrip('md')))):
...
class ICP(object):
    ...
    def __init__(self, *args, **kwargs ):
        if args:
            raise ValueError(  
                   '''ICP constructor can only be called with keyword arguments for 
                      ...
                      smoothing_low_medium_or_high, debug1, and debug2''')       
        ...
        max_num_of_pixels_used_for_icp=smoothing_low_medium_or_high=None
        image_polarity=auto_select_model_and_data=debug1=debug2=None
    ...
    def extract_pixels_from_color_image(self, model_or_data):
        ...
        if self.corners_or_edges == "edges":
            ...
            edge_pixel_list = []    
            corner_pixels = []
            # Note that array indexing is 'opposite' of the image indexing
            # with the first index along what is y for the image and the
            # second index along what is x for the image.  For the image,
            # x is the horizontal axis to the right, y is the vertical axis
            # pointing downwards.  In what follows, we treat i and j as the
            # image coordinates.  That is, i increments horizontally to the right
            ...
            for i in range(3,width-3):                 # i is for horizontal axis to the right
                for j in range(3,height-3):            # j is for vertical axis to the bottom
                    ...
                    edge_im.putpixel((i,j), edge_strength)
    ...
    def display_array_as_image(self, numpy_arr):
        ...
        for i in range(3,width-3):
            for j in range(3,height-3):
                display_im.putpixel((i,j), abs(numpy_arr[(j,i)]))
        display_im.show()
    ...
    def save_array_as_image(self, numpy_arr, label):
        ...
        for i in range(3,width-3):
            for j in range(3,height-3):
                save_im.putpixel((i,j), abs(numpy_arr[(j,i)]))
    ...
    def display_results_as_movie(self):
        ...
        for i in range(0,self.iterations):
            result_im = Image.open("__result" + str(i) + ".jpg")
            ...
            result_color_im.save("__result_color" + str(i) + ".jpg")
        while self.iteration_control_flag:
            for i in range(0,self.iterations):
                try:
                    tkim[i] = Image.open("__result_color" + str(i) + ".jpg")
    ...
    def display_images_used_for_edge_based_icp(self):
        ...
        rootWindow.geometry( str(int(0.8 * screen_width)) + "x" + str(int(0.6 * screen_height)) + "+50+50") 
...
#        rootWindow.geometry("1400x750+50+50") 
    ...
    def cleanup_directory(self):
        for filename in glob.glob( '__result*.jpg' ): os.unlink(filename)
