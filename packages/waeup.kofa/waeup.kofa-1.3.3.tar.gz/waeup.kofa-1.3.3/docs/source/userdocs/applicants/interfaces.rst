...
split up and copied into a dictionary for faster processing. In the
base package, `local_roles` is an empty list which means, no local
role can be assigned at applicants container level.
