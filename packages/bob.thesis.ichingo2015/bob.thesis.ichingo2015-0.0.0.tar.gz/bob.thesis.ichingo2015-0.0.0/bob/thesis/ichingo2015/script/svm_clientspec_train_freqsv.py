...
#Tue 28 Jul 15:55:43 CEST 2015

"""
This script creates SVM machines for each client based on the client's enrollment samples (as a positive class) and the cohort attack samples (as a negative class). As negative data, it uses samples that have been previously determined to be most frequently selected as support vectors.
...
def main():
  ...
  parser.add_argument('--freqfile', dest='freqfile', type=str, default=None, help='File containing the frequency counts of the SV')  
  ...
  parser.add_argument('--nssv', type=int, dest='nssv', default=0, help='Only the nssv vectors with highest frequencies will be selected to form the set of negative data. If nssv==0, all of them will be considered (defaults to "%(default)s")')
  ...
  if args.freqfile != None:
    sys.stdout.write("Reading SV frequencies and negative data set...\n")   
    ff = bob.io.base.HDF5File(args.freqfile, 'r')
    negdata_info, negdata = sm.create_full_dataset_frameinfo(featdir, process_negative);
    sorted_ind_freq = ff.get_attribute('sorted_ind_freq')
    #sorted_ind_freq = freq_count.argsort()[::-1] # sorted indices, descending order
    negdata = negdata[sorted_ind_freq[:args.nssv],:]
  else:
