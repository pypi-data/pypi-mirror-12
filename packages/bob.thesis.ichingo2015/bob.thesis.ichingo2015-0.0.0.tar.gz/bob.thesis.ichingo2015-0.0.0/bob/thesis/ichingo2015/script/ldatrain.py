...
#Mon 14 Sep 15:05:52 CEST 2015

"""This script calculates the scores of LDA classification of data into two categories: real accesses and spoofing attacks. There is an option for normalizing and dimensionality reduction of the data prior to the LDA classification.
...
Out of all the features, the script has an option to remove certain features.
"""
...
def main():
  ...
  parser.add_argument('-c', '--cols-to-remove', type=str, dest="cols_to_remove", default=None, help='List of columns to remove', nargs='+')
  ...
  if args.cols_to_remove is not None:
    cols_to_keep = list(set(range(train_real.shape[1])) - set(args.cols_to_remove))
    train_real = train_real[:,cols_to_keep]
    devel_real = devel_real[:,cols_to_keep]
    test_real = test_real[:,cols_to_keep]
