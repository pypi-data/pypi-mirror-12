...
def setAllCrossCoeff(comps,table,keep="All"):
    ...
    if keep == "All":
        ...
        for (A,B) in keep:
            Acomps = subComps(A)
            Bcomps = subComps(B)
            tokeep.extend(product(Acomps,Bcomps))
            # Also keep self-interactions within heteronuclear
            tokeep.extend(combinations(Acomps,2))
            tokeep.extend(combinations(Bcomps,2))
        # Hackish way of making the tokeep list not contain
        keep = tokeep
        print(keep)
...
def gmxWriteAllXVG(system, components):
    ...
    for comp in components:
        gmxWriteXVG(comp.Name,comp.N,comp.M,comp.RMax)
    ...
    copyfile("table_"+components[0].Name+".xvg", "table.xvg")
    ...
    with open(filename, 'w') as itp:
        ...
        for comp in getAllSubComponents(components):
            ...
            if comp.Segments > 2:
                ...
                bond_type = 'H'
            mass = comp.Mass
            itp.write('{0:^10} {1:<10} {2:2.5f}   0.000    A      {3:e} {4:e}\n'.format(comp.Name[0:5], bond_type, mass, comp_C, comp_A))
        ...
        for (A, B) in combinations(getAllSubComponents(components), 2):
            ...
            itp.write('{0:<7} {1:<7}  1        {2:e}  {3:e}\n'.format(A.Name[0:5], B.Name[0:5], cross_C, cross_A))
        ...
        for comp in components:
            ...
            for t in comp.Types:
                ...
                if lasttype != t:
                    lasttype = t
                    atom += 1
                itp.write('{0:>3}  {1:<7}  1  {2:<7}  {3:<7}  {4:>3}      \n'.format(tcount, t[0:5], comp.Name[0:5], t[0:5], atom))
...
def gmxWriteTOP(system,components):
    ...
    with open(filename, 'w') as top:
        ...
        for comp in components:
            ...
            top.write("{0:<} {1:>6}\n".format(name,comp.Count))
    # End of file

def gmxWriteMDP(system,components):
    filename = 'NVT.mdp'
    # Create group string and (cross-)interaction strings
    groups = " ".join( c.Name[0:5] for c in getAllSubComponents(components) )
    Ngroups = len(getAllSubComponents(components))
    interactions = " ".join( a.Name[0:5]+" "+b.Name[0:5] for a,b in combinations(getAllSubComponents(components),2) )
