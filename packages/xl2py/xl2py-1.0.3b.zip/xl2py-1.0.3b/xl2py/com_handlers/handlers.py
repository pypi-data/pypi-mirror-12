...
class xlcom(object):
    ...
    def get_formulas(self,xlranges):
        ...
        for i in range(0,len(xlranges)):
            ...
            if self.Worksheet.Range(xlranges[i]).HasArray:
                formulas.append(np.array(self.Worksheet.Range(xlranges[i]).FormulaArray,ndmin=2))
            else:
                innerformulas = []
                if type(xlranges[i]) is not list:
                    xlranges[i] = [xlranges[i]]
                for cell in xlranges[i]:
                    innerformulas.append(np.array(self.Worksheet.Range(cell).Formula,ndmin=2))
    ...
    def convert_r1c1A1(self,xlranges):
        ...
        if type(xlranges) is not list:
            xlranges = [xlranges]
        for i in range(0,len(xlranges)):
            if re.compile('R\d+C\d+').search(str(xlranges[0])):
                try:
                    xlranges[i] = str(self.ExcelObject.ConvertFormula(Formula=xlranges[i],FromReferenceStyle=0,ToReferenceStyle=1,ToAbsolute=1))
                except:
                    pass
            else:
                try:
                    xlranges[i] = str(self.ExcelObject.ConvertFormula(Formula=xlranges[i],FromReferenceStyle=1,ToReferenceStyle=0,ToAbsolute=1))
                except:
                    pass
        return xlranges
    ...
    def change_path(self,WB=[],WS=[],bkp=[]):
        ...
        if WS != ([] and ''):
            ...
            if all([WB != i for i in (self.Workbook.Name, [], '')]):
                ExcelObject = x32.Dispatch('Excel.Application')
                Workbook = ExcelObject.Workbooks.xlopen(WB)
