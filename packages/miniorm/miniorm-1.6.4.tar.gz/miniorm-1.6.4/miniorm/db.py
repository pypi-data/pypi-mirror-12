...
class Model(object):
    ...
    def make(name, dbargkws=None, tablename=None, pool_size=1, external_pool = None):
        pool = external_pool and external_pool or MySQLConnectionPool(pool_name="miniorm_pooling", pool_size=pool_size, pool_reset_session=True, **dbargkws)
        return type(tablename or name.lower(), (Model,), dict(tablename=tablename or name.lower(), dbargkws=dbargkws, pool=pool))
