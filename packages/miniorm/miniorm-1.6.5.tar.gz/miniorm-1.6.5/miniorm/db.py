...
class Model(object):
    ...
    @staticmethod
    def make(name, dbargkws=None, tablename=None, pool_size=1, external_pool=None):
        return type(tablename or name.lower(), (Model,), dict(tablename=tablename or name.lower(), dbargkws=dbargkws, pool_size=pool_size, external_pool=external_pool))

    @classmethod
    def close(clz):
        return clz.pool._remove_connections()

    @classmethod
    def get_conn(clz):

        if not clz.pool:
            clz.pool = clz.external_pool and clz.external_pool or MySQLConnectionPool(pool_name="miniorm_pooling", pool_size=clz.pool_size, pool_reset_session=True, **clz.dbargkws)

        while 1:
            try:
                return clz.pool.get_connection()
            except PoolError:
                time.sleep(0.01)
