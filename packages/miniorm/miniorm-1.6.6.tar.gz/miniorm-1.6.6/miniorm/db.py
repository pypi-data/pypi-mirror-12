...
class Model(object):
    ...
    def get_conn(dbargkws):
        ...
        if ident not in Model.connections:
            conn = connection.MySQLConnection(**dbargkws)
            conn.set_autocommit(False)
            Model.connections[ident] = conn
        return Model.connections[ident]

    @classmethod
    def close(clz):
        ident = threading.currentThread().ident
        ident in Model.connections and Model.connections[ident].close()
