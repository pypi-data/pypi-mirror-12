...
# ############################################################################
# [2015-12-15 12:35:24.841000] THIS FILE IS AUTOGENERATED - DO NOT EDIT!
# ############################################################################
import types
import pandas as pd
import numpy as np
...
class Proxy(ProxyBase):
    ...
    _CMD_SPI_BEGIN = 0x4fb
    _CMD_SET_SPI_BIT_ORDER = 0x4fc
    _CMD_SET_SPI_CLOCK_DIVIDER = 0x4fd
    _CMD_SET_SPI_DATA_MODE = 0x4fe
    _CMD_SPI_TRANSFER = 0x4ff
    _CMD_SPI_BEGIN_TRANSACTION = 0x500
    _CMD_SPI_END_TRANSACTION = 0x501
    _CMD_SPI_TRANSFER_ARRAY = 0x502
    ...
    def spi_begin(self):
        command = np.dtype('uint16').type(self._CMD_SPI_BEGIN)
        payload_size = 0
        payload_data = ''
    ...
    def spi_begin_transaction(self, clock, bit_order, data_mode):
        command = np.dtype('uint16').type(self._CMD_SPI_BEGIN_TRANSACTION)
        ARG_STRUCT_SIZE = 6
        array_data = ''
        payload_size = ARG_STRUCT_SIZE + len(array_data)
        struct_data = np.array([(clock, bit_order, data_mode, )],
                               dtype=[('clock', 'uint32'), ('bit_order', 'uint8'), ('data_mode', 'uint8'), ])
        payload_data = struct_data.tostring() + array_data
    ...
    def spi_end_transaction(self):
        command = np.dtype('uint16').type(self._CMD_SPI_END_TRANSACTION)
        payload_size = 0
        payload_data = ''
    ...
    def spi_transfer_array(self, slave_select_pin, clock, bit_order, data_mode, data):
        command = np.dtype('uint16').type(self._CMD_SPI_TRANSFER_ARRAY)
        ARG_STRUCT_SIZE = 13
        ...
        array_data = ''.join([data.tostring(), ])
        payload_size = ARG_STRUCT_SIZE + len(array_data)
        struct_data = np.array([(slave_select_pin, clock, bit_order, data_mode, array_info.length['data'], ARG_STRUCT_SIZE + array_info.start['data'], )],
                               dtype=[('slave_select_pin', 'uint8'), ('clock', 'uint32'), ('bit_order', 'uint8'), ('data_mode', 'uint8'), ('data_length', 'uint32'), ('data_data', 'uint16'), ])
        payload_data = struct_data.tostring() + array_data
