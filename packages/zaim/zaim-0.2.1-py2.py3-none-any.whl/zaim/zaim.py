...
class Api(object):
    ...
    def genre_from_name(self, genre_name, category_name=None):
        if category_name is not None:
            category_id = self.category_from_name(category_name)
        
        response = self.genre()
        for genre in response['genres'][:]:
            if categoy_name is not None:
                if genre['name'] != genre_name or genre['category_id'] != category_id:
