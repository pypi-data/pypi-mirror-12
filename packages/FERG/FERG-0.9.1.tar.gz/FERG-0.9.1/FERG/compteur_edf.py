...
class compteur_edf(object):
	...
	def __init__(self, capteur = None, micro_controleur = None, capteur_hc = None, led = led_io(), intensity_max=45, energy_tr=2.5, nb_tours_mesure = 1, counter_hc = 0, counter_hp = 0, led_hc = led_io()):
		'''Initialisation
	...
	def record_micro_controleur(self, rec_function):
		...
		while nb_tours_enregistres < self.nb_tours_mesure and not self.terminated:
			...
			self.led.off()
		if not self.terminated:
			logging.debug('%s tour enregistres.' % nb_tours_enregistres)
			# Calcul Energie et puissance
			duree = time.time() - now
			energy = self.energy_tr * nb_tours_enregistres
			power = energy * 3600 / duree
			# Mise à jours des compteurs (HC et HP)
			if self.type_horaire() == 'HC':
				self.counter_hc += energy
				logging.info('Compteur HC = %s.' % self.counter_hc)
			else:
				self.counter_hp += energy
				logging.info('Compteur HP = %s.' % self.counter_hp)
			# Enregistrement
			rec_function( \
				date_time=datetime.datetime.now(), \
