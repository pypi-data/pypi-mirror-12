...
class AWSRegionParamType(click.ParamType):
    ...
    def convert(self, value, param, ctx):
        ...
        if value and isinstance(value, str):
            ...
            import boto3
            ctx.obj['ec2'] = boto3.client('ec2', region_name=value)
...
class VpcPrefixParamType(click.ParamType):
    ...
    def convert(self, value, param, ctx):
        ...
        if value:
            res = ctx.obj['ec2'].describe_vpcs(
                Filters=[{'Name': 'tag:Name', 'Values': [value]}]
            )

            if res.get("Vpcs"):
                self.fail("VPC '{}' already exists!".format(value), param, ctx)
...
class VpcNumOfAZsParamType(click.IntRange):
    ...
    def convert(self, value, param, ctx):
        azs = ctx.obj['ec2'].describe_availability_zones()
