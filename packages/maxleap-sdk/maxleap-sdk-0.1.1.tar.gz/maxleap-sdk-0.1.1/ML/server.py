...
def get_principal():
    app_id = request.headers.get('X-LAS-AppId',None)
    session_token = request.headers.get('X-LAS-Session-Token',None)
    master_key = request.headers.get('X-LAS-MasterKey',None)
    client_key = request.headers.get('X-LAS-APIKey',None)
    return ML.UserPrincipal(app_id, client_key=client_key, master_key=master_key, session_token=session_token)
