...
class Worker(object):
    ...
    def run_cmd(self, cmd, wd):
        ...
        logger.info("run_cmd:" + str(cmd))
        try:
            p = sp.Popen(cmd, stdout=open(file_name, 'wb'), stderr=sp.PIPE, cwd=wd)
            stdout, stderr = p.communicate()
            if stderr:
                logger.error("Error:\n\t"+str(stderr))
                logger.info("Additional output:\n\t"+str(stdout))
                print "Error in Worker running "+str(cmd)+" in "+str(wd)+". \nError message is:\n\t"+str(stderr) + "\nProcess output:\n\t" +str(stdout)+"\n"
                #sys.exit()
                #raise Exception #  TODO: On windows this throw an error even in a normal run
            logger.info("Done run_cmd:\n" + str(" ".join(cmd)) + ", in wd:" + str(wd))
