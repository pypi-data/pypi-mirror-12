...
class CannotCrossSimulationsError(GenericError):
    """An attempt was made to relate two objects from different simulations. This could be assigning to buffer properties, connecting nodes, or any other such condition."""
    pass
    ...
    def init_with_handle(self, handle):
        with _object_states_lock:
            ...
            if should_add_properties:
                ...
                self._state['properties']["delay_samples"] = _libaudioverse.Lav_DELAY_DELAY_SAMPLES
            ...
            self._property_instances[_libaudioverse.Lav_DELAY_DELAY_SAMPLES] = DoubleProperty(handle = self.handle, slot = _libaudioverse.Lav_DELAY_DELAY_SAMPLES)
...
"""
        return self._property_instances[_libaudioverse.Lav_DELAY_DELAY_SAMPLES]
...
"""
        evt = self._state['events'].get(_libaudioverse.Lav_PUSH_AUDIO_EVENT, None)
        if evt is None:
            return
        return (evt.callback, evt.extra_arguments)

    @audio_event.setter
    def audio_event(self, val):
        global _global_events
        val_tuple = tuple(val) if isinstance(val, collections.Iterable) else (val, )
        if len(val_tuple) == 1:
            val_tuple = (val, ())
        cb, extra_args = val_tuple
        event_obj = _EventCallbackWrapper(self, _libaudioverse.Lav_PUSH_AUDIO_EVENT, cb, extra_args)
        self._state['events'][_libaudioverse.Lav_PUSH_AUDIO_EVENT] = event_obj
...
"""
        evt = self._state['events'].get(_libaudioverse.Lav_PUSH_OUT_EVENT, None)
        if evt is None:
            return
        return (evt.callback, evt.extra_arguments)

    @out_event.setter
    def out_event(self, val):
        global _global_events
        val_tuple = tuple(val) if isinstance(val, collections.Iterable) else (val, )
        if len(val_tuple) == 1:
            val_tuple = (val, ())
        cb, extra_args = val_tuple
        event_obj = _EventCallbackWrapper(self, _libaudioverse.Lav_PUSH_OUT_EVENT, cb, extra_args)
        self._state['events'][_libaudioverse.Lav_PUSH_OUT_EVENT] = event_obj
...
Each input and output pair represent the input and output of a specific delay line, respectively."""
    ...
    def init_with_handle(self, handle):
        with _object_states_lock:
            ...
            if should_add_properties:
                self._state['properties']["delay_max"] = _libaudioverse.Lav_FDN_MAX_DELAY
...
This node, coupled with the :class:`ObjtypeChannelMergerNode`, allows advanced applications to manipulate the individual audio channels directly.
The usual workflow is as follows: feed the output of a node through this node,
modify the channels individually, and then merge them with a :class:`ObjtypeChannelMergerNode`."""
...
class ChannelMergerNode(GenericNode):
    """Libaudioverse inputs and outputs transport multiple channels of audio, which is usually the desired behavior.
This node, coupled with the :class:`ObjtypeChannelSplitterNode` , allows advanced applications to manipulate the individual audio channels directly.
The usual workflow is as follows: feed the output of a node through a :class:`ObjtypeChannelSplitterNode`,
...
"""
        evt = self._state['events'].get(_libaudioverse.Lav_BUFFER_END_EVENT, None)
        if evt is None:
            return
        return (evt.callback, evt.extra_arguments)

    @end_event.setter
    def end_event(self, val):
        global _global_events
        val_tuple = tuple(val) if isinstance(val, collections.Iterable) else (val, )
        if len(val_tuple) == 1:
            val_tuple = (val, ())
        cb, extra_args = val_tuple
        event_obj = _EventCallbackWrapper(self, _libaudioverse.Lav_BUFFER_END_EVENT, cb, extra_args)
        self._state['events'][_libaudioverse.Lav_BUFFER_END_EVENT] = event_obj
...
It is slower than the :class:`ObjtypeConvolverNode` for small impulse responses.

The difference between this node and the :class:`ObjtypeConvolverNode` is the complexity of the algorithm.
This node is capable of handling impulses longer than a second, a case for which the :class:`ObjtypeConvolverNode` will fail to run in realtime.
...
"""
        evt = self._state['events'].get(_libaudioverse.Lav_CROSSFADER_FINISHED_EVENT, None)
        if evt is None:
            return
        return (evt.callback, evt.extra_arguments)

    @finished_event.setter
    def finished_event(self, val):
        global _global_events
        val_tuple = tuple(val) if isinstance(val, collections.Iterable) else (val, )
        if len(val_tuple) == 1:
            val_tuple = (val, ())
        cb, extra_args = val_tuple
        event_obj = _EventCallbackWrapper(self, _libaudioverse.Lav_CROSSFADER_FINISHED_EVENT, cb, extra_args)
        self._state['events'][_libaudioverse.Lav_CROSSFADER_FINISHED_EVENT] = event_obj
...
class FirstOrderFilterNode(GenericNode):
    """A first order filter section, implementing the transfer function H(Z) = rac{B_0 + B_1 Z^{-1} }{1+A_0 Z^{-1} }

This filter is not your friend unless you know DSP or have a specific goal in mind.
Most applications will want  a :class:`ObjtypeBiquadNode` or a :class:`ObjtypeOnePoleFilterNode` instead.
...
The major advantage for this filter type is that it is incredibly inexpensive as compared to the :class:`ObjtypeIirNode` and supports automation of the pole and zero's position."""
...
This is equivalent to the :class:`ObjtypeOnePoleFilterNode` lowpass configuration."""
...
You specify the rac{\pi}{2} frequency.
You get a filter with a phase of \pi at DC and 0 at Nyquist."""
        return _lav.first_order_filter_node_configure_allpass(node, frequency)

    def configure_highpass(node, frequency):
        """Configure the filter as a highpass with a roll-off of ``6 DB`` per octave.
This is identical to the :class:`ObjtypeOnePoleFilterNode` highpass configuration."""
...
class AllpassNode(GenericNode):
    """Implements a first-order allpass filter whose transfer function is rac{c+Z^{-d} }{1 + cZ^{-d} } where ``c`` is the coefficient and ``d`` the delay in samples.

This filter is useful in various reverb designs.
It can be thought of as a series of decaying echoes recurring every ``delay_samples`` samples, whose starting amplitude and overall decay time are controlled by the ``delay_samples`` property."""
