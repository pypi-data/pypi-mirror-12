...
def error_get_message():
    ...
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return     destination.value.decode('utf8') #All strings are contractually UTF8 when returned to us.
def error_get_file():
...
def simulation_get_block_size(simulationHandle):
    ...
    destination = ctypes.c_int()
    err = _libaudioverse.Lav_simulationGetBlockSize(simulationHandle,         ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return getattr(destination, 'value', destination)
...
def simulation_get_sr(simulationHandle):
    ...
    destination = ctypes.c_int()
    err = _libaudioverse.Lav_simulationGetSr(simulationHandle,         ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return getattr(destination, 'value', destination)
...
def simulation_get_threads(simulationHandle):
    ...
    destination = ctypes.c_int()
    err = _libaudioverse.Lav_simulationGetThreads(simulationHandle,         ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return getattr(destination, 'value', destination)
...
def buffer_get_simulation(bufferHandle):
    ...
    destination = _libaudioverse.LavHandle()
    err = _libaudioverse.Lav_bufferGetSimulation(bufferHandle,         ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
...
def node_get_event_handler(nodeHandle, event):
    #Drill down up to twice, otherwise assume we passed in something safe.
    nodeHandle = getattr(nodeHandle, 'handle', nodeHandle)
    nodeHandle = getattr(nodeHandle, 'handle', nodeHandle)

    destination = _libaudioverse.LavEventCallback()
    err = _libaudioverse.Lav_nodeGetEventHandler(nodeHandle, event,         ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return getattr(destination, 'value', destination)
def node_get_event_user_data_pointer(nodeHandle, event):
    #Drill down up to twice, otherwise assume we passed in something safe.
    nodeHandle = getattr(nodeHandle, 'handle', nodeHandle)
    nodeHandle = getattr(nodeHandle, 'handle', nodeHandle)

    destination = ctypes.c_void_p()
    err = _libaudioverse.Lav_nodeGetEventUserDataPointer(nodeHandle, event,         ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return getattr(destination, 'value', destination)
def node_set_event(nodeHandle, event, handler, userData):
    #Drill down up to twice, otherwise assume we passed in something safe.
    nodeHandle = getattr(nodeHandle, 'handle', nodeHandle)
    nodeHandle = getattr(nodeHandle, 'handle', nodeHandle)
    if isinstance(userData, collections.Sized):
    ...
    err = _libaudioverse.Lav_nodeSetEvent(nodeHandle, event, handler, userData)
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
...
def create_additive_square_node(simulationHandle):
    ...
    destination = _libaudioverse.LavHandle()
    err = _libaudioverse.Lav_createAdditiveSquareNode(simulationHandle,         ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
def create_additive_triangle_node(simulationHandle):
    ...
    destination = _libaudioverse.LavHandle()
    err = _libaudioverse.Lav_createAdditiveTriangleNode(simulationHandle,         ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
def create_additive_saw_node(simulationHandle):
    ...
    destination = _libaudioverse.LavHandle()
    err = _libaudioverse.Lav_createAdditiveSawNode(simulationHandle,         ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
def create_noise_node(simulationHandle):
    ...
    destination = _libaudioverse.LavHandle()
    err = _libaudioverse.Lav_createNoiseNode(simulationHandle,         ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
def create_hrtf_node(simulationHandle, hrtfPath):
    ...
    destination = _libaudioverse.LavHandle()
    err = _libaudioverse.Lav_createHrtfNode(simulationHandle, hrtfPath,         ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
def create_hard_limiter_node(simulationHandle, channels):
    ...
    destination = _libaudioverse.LavHandle()
    err = _libaudioverse.Lav_createHardLimiterNode(simulationHandle, channels,         ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
def create_crossfading_delay_node(simulationHandle, maxDelay, channels):
    ...
    destination = _libaudioverse.LavHandle()
    err = _libaudioverse.Lav_createCrossfadingDelayNode(simulationHandle, maxDelay, channels,         ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
def create_dopplering_delay_node(simulationHandle, maxDelay, channels):
    ...
    destination = _libaudioverse.LavHandle()
    err = _libaudioverse.Lav_createDoppleringDelayNode(simulationHandle, maxDelay, channels,         ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
def create_amplitude_panner_node(simulationHandle):
    ...
    destination = _libaudioverse.LavHandle()
    err = _libaudioverse.Lav_createAmplitudePannerNode(simulationHandle,         ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
...
def create_multipanner_node(simulationHandle, hrtfPath):
    ...
    destination = _libaudioverse.LavHandle()
    err = _libaudioverse.Lav_createMultipannerNode(simulationHandle, hrtfPath,         ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
def create_push_node(simulationHandle, sr, channels):
    ...
    destination = _libaudioverse.LavHandle()
    err = _libaudioverse.Lav_createPushNode(simulationHandle, sr, channels,         ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
...
def create_biquad_node(simulationHandle, channels):
    ...
    destination = _libaudioverse.LavHandle()
    err = _libaudioverse.Lav_createBiquadNode(simulationHandle, channels,         ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
def create_pull_node(simulationHandle, sr, channels):
    ...
    destination = _libaudioverse.LavHandle()
    err = _libaudioverse.Lav_createPullNode(simulationHandle, sr, channels,         ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
...
def create_graph_listener_node(simulationHandle, channels):
    ...
    destination = _libaudioverse.LavHandle()
    err = _libaudioverse.Lav_createGraphListenerNode(simulationHandle, channels,         ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
...
def create_custom_node(simulationHandle, inputs, channelsPerInput, outputs, channelsPerOutput):
    ...
    destination = _libaudioverse.LavHandle()
    err = _libaudioverse.Lav_createCustomNode(simulationHandle, inputs, channelsPerInput, outputs, channelsPerOutput,         ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
...
def create_ringmod_node(simulationHandle):
    ...
    destination = _libaudioverse.LavHandle()
    err = _libaudioverse.Lav_createRingmodNode(simulationHandle,         ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
def create_feedback_delay_network_node(simulationHandle, maxDelay, channels):
    ...
    destination = _libaudioverse.LavHandle()
    err = _libaudioverse.Lav_createFeedbackDelayNetworkNode(simulationHandle, maxDelay, channels,         ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
def create_iir_node(simulationHandle, channels):
    ...
    destination = _libaudioverse.LavHandle()
    err = _libaudioverse.Lav_createIirNode(simulationHandle, channels,         ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
...
def create_gain_node(simulationHandle, channels):
    ...
    destination = _libaudioverse.LavHandle()
    err = _libaudioverse.Lav_createGainNode(simulationHandle, channels,         ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
def create_channel_splitter_node(simulationHandle, channels):
    ...
    destination = _libaudioverse.LavHandle()
    err = _libaudioverse.Lav_createChannelSplitterNode(simulationHandle, channels,         ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
def create_channel_merger_node(simulationHandle, channels):
    ...
    destination = _libaudioverse.LavHandle()
    err = _libaudioverse.Lav_createChannelMergerNode(simulationHandle, channels,         ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
def create_buffer_node(simulationHandle):
    ...
    destination = _libaudioverse.LavHandle()
    err = _libaudioverse.Lav_createBufferNode(simulationHandle,         ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
def create_buffer_timeline_node(simulationHandle, channels):
    ...
    destination = _libaudioverse.LavHandle()
    err = _libaudioverse.Lav_createBufferTimelineNode(simulationHandle, channels,         ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
...
def create_recorder_node(simulationHandle, channels):
    ...
    destination = _libaudioverse.LavHandle()
    err = _libaudioverse.Lav_createRecorderNode(simulationHandle, channels,         ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
...
def create_convolver_node(simulationHandle, channels):
    ...
    destination = _libaudioverse.LavHandle()
    err = _libaudioverse.Lav_createConvolverNode(simulationHandle, channels,         ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
def create_fft_convolver_node(simulationHandle, channels):
    ...
    destination = _libaudioverse.LavHandle()
    err = _libaudioverse.Lav_createFftConvolverNode(simulationHandle, channels,         ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
...
def create_three_band_eq_node(simulationHandle, channels):
    ...
    destination = _libaudioverse.LavHandle()
    err = _libaudioverse.Lav_createThreeBandEqNode(simulationHandle, channels,         ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
def create_filtered_delay_node(simulationHandle, maxDelay, channels):
    ...
    destination = _libaudioverse.LavHandle()
    err = _libaudioverse.Lav_createFilteredDelayNode(simulationHandle, maxDelay, channels,         ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
def create_panner_bank_node(simulationHandle, pannerCount, hrtfPath):
    ...
    destination = _libaudioverse.LavHandle()
    err = _libaudioverse.Lav_createPannerBankNode(simulationHandle, pannerCount, hrtfPath,         ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
def create_crossfader_node(simulationHandle, channels, inputs):
    ...
    destination = _libaudioverse.LavHandle()
    err = _libaudioverse.Lav_createCrossfaderNode(simulationHandle, channels, inputs,         ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
...
def create_one_pole_filter_node(simulationHandle, channels):
    ...
    destination = _libaudioverse.LavHandle()
    err = _libaudioverse.Lav_createOnePoleFilterNode(simulationHandle, channels,         ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
def create_first_order_filter_node(simulationHandle, channels):
    ...
    destination = _libaudioverse.LavHandle()
    err = _libaudioverse.Lav_createFirstOrderFilterNode(simulationHandle, channels,         ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
...
def create_allpass_node(simulationHandle, channels, maxDelay):
    ...
    destination = _libaudioverse.LavHandle()
    err = _libaudioverse.Lav_createAllpassNode(simulationHandle, channels, maxDelay,         ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
def create_nested_allpass_network_node(simulationHandle, channels):
    ...
    destination = _libaudioverse.LavHandle()
    err = _libaudioverse.Lav_createNestedAllpassNetworkNode(simulationHandle, channels,         ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
...
def create_fdn_reverb_node(simulationHandle):
    ...
    destination = _libaudioverse.LavHandle()
    err = _libaudioverse.Lav_createFdnReverbNode(simulationHandle,         ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
...
def create_dc_blocker_node(simulationHandle, channels):
    ...
    destination = _libaudioverse.LavHandle()
    err = _libaudioverse.Lav_createDcBlockerNode(simulationHandle, channels,         ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
def create_leaky_integrator_node(simulationHandle, channels):
    ...
    destination = _libaudioverse.LavHandle()
    err = _libaudioverse.Lav_createLeakyIntegratorNode(simulationHandle, channels,         ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
def create_environment_node(simulationHandle, hrtfPath):
    ...
    destination = _libaudioverse.LavHandle()
    err = _libaudioverse.Lav_createEnvironmentNode(simulationHandle, hrtfPath,         ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
...
def create_source_node(simulationHandle, environmentHandle):
    ...
    destination = _libaudioverse.LavHandle()
    err = _libaudioverse.Lav_createSourceNode(simulationHandle, environmentHandle,         ctypes.byref(destination))
    if err != _libaudioverse.Lav_ERROR_NONE:
        raise make_error_from_code(err)
    return reverse_handle(destination.value)
