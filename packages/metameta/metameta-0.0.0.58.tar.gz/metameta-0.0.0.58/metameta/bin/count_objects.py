...
def compute_gene_abundance_from_blast(blast_file, out_file):
    with open(out_file) as out_handle, open(blast_file) as blast_handle:
        genes = {}
        for entry in m8_iter(blast_handle):
            if entry['subjectID'] in genes:
                genes[entry['subjectID']] += 1
            else:
                genes[entry['subjectID']] = 1
...
def normalization_method(method):
    acceptable_methods = ['rpkt', 'rpkm', 'rpkb', ' rpb', 'arpb']
...
if __name__ == '__main__':
    ...
    if args.version:
        print(__version__)
    elif args.fastr is None and args.bam is None and args.blast:
        print(__doc__)
    else:
        if args.verify:
