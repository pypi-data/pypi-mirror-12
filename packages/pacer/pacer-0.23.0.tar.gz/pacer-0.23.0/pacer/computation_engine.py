...
class RegularWorkflowAbortion(DelayedException):
...
def delayed_exception(e):
    ...
    if isinstance(e, Warning):
        return RegularWorkflowAbortion(e)
        ...
        for _ in range(self.size()):
            ...
            if not do_not_reraise_exceptions:
                if isinstance(item.value, DelayedException):
                    if not isinstance(item.value, RegularWorkflowAbortion):
                        item.value.re_raise()
            results.append(item)
        ...
        if re_raise_exceptions:
            for r in self.results:
                if isinstance(r, DelayedException):
                    if not isinstance(r, RegularWorkflowAbortion):
                        r.re_raise()
        return self.results
