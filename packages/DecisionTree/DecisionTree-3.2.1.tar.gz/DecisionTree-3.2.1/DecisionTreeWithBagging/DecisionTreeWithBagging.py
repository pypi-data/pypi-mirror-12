...
class DecisionTreeWithBagging(object):
    ...
    def __init__(self, *args, **kwargs ):
        ...
        if not args and training_datafile:
            ...
            if args[0] != 'evalmode':
                raise SyntaxError("""When supplying non-keyword arg, it can only be 'evalmode'""")
    ...
    def get_training_data_for_bagging(self):
        ...
        if '""' not in data_dict:
            TypeError('''Aborted. The first row of CSV file must begin with "", must have a header '''
    ...
    def display_classification_results_for_each_bag(self):         
        ...
        if classifications is None:
            raise SyntaxError('''You must first call "classify_with_bagging()" before invoking "display_classification_results_for_each_bag()" ''')
    ...
    def get_majority_vote_classification(self):
        classifications = self._classifications
        if classifications is None:
            raise SyntaxError('''You must first call "classify_with_bagging()" before invoking "get_majority_vote_classification()" ''')
