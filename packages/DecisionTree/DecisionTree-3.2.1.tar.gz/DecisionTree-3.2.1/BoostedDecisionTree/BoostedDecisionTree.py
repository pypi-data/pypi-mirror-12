...
class BoostedDecisionTree(DecisionTree):
    ...
    def get_training_data_for_base_tree(self):
        ...
        if '""' not in data_dict:
            SyntaxError('''Aborted. The first row of CSV file must begin with "", must have a header '''
...
#            if stagedebug: dt_this_stage._debug2 = 1
    ...
    def trust_weighted_majority_vote_classifier(self):
        classifications = self._classifications
        if classifications is None:
            raise AttributeError('''You must first call "classify_with_boosting()" before invoking "trust_weighted_majority_vote_classifier()" ''')
