...
__date__    = '2015-June-10'
__url__     = 'https://engineering.purdue.edu/kak/distDT/DecisionTree-3.2.0.html'
...
@tag2
    show_training_data_for_base_tree()
        ...
        Shows the training data samples and some relevant properties of the
        features used for the training dataset.
...
@tag2
    display_base_decision_tree()
        ...
        Displays the base decision tree.
...
@tag2
    display_decision_trees_for_different_stages()
        ...
        Displays separately the decision tree constructed for each stage of
        the cascade.
...
@tag2
    display_classification_results_for_each_stage()
        ...
        This method shows you the classification decisions made by each
        decision tree in the cascade.  The method also prints out the
        trust factor associated with each decision tree.  It is important
        to look simultaneously at the classification decision and the
        trust factor for each tree --- since a classification decision made
        by a specific tree may appear bizarre for a given test sample.
        This method is useful primarily for debugging purposes.

@tag2
    show_class_labels_for_misclassified_samples_in_stage( stage_index )

        As for the previous method, this method is useful mostly for
...
@tag2
    trust_weighted_majority_vote_classifier()
        ...
        trees taking into account the trust factors associated with the
        trees.  As mentioned earlier, we associate with each tree of the
        cascade a trust factor that depends on the overall
...
THE BoostingExamples DIRECTORY:
    ...
    in a single file.  The demonstration script displays for each test
