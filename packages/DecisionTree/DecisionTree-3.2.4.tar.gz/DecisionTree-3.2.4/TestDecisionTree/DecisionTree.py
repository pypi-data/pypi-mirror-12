...
__date__    = '2015-November-23'
__url__     = 'https://engineering.purdue.edu/kak/distDT/DecisionTree-3.2.4.html'
...
CHANGES:
  ...
  Version 3.2.4
    ...
    fields. Previously, this entry was required to be the empty
...
@tag2
    The constructor parameters:
        ...
        number_of_histogram_bins:
            ...
            parameter is also important when you have a very large training
            dataset.  In general, the larger a dataset, the smaller the
            smallest difference between any two values for a numeric
            variable in relation to the overall range of values for that
            variable. In such cases, the value used for the number of bins
            used for estimating the probabilities could become much too
            large and slow down the calculation of the decision tree.  You
            can get around this problem by explicitly giving a value to the
            'number_of_histogram_bins' parameter.
...
@endcode
    ...
    are dealing with CSV training files with tens of thousands of records.
...
class DecisionTree(object):
    def __init__(self, *args, **kwargs ):
        ...
        allowed_keys = 'training_datafile','entropy_threshold','max_depth_desired','csv_class_column_index',\
                       ...
                       'number_of_histogram_bins','csv_cleanup_needed','first_field_in_first_record',\
        ...
        symbolic_to_numeric_cardinality_threshold=csv_columns_for_features=csv_cleanup_needed=None
        first_field_in_first_record=debug1=debug2=debug3=None
        if kwargs and not args:
            ...
            if 'first_field_in_first_record' in kwargs: \
                                          first_field_in_first_record = kwargs.pop('first_field_in_first_record')
        ...
        if first_field_in_first_record:
            self._first_field_in_first_record               =      first_field_in_first_record
        else:
            self._first_field_in_first_record               =      '""'
    ...
    def get_training_data_from_csv(self):
        class_name_in_column = self._csv_class_column_index - 1  # subtract 1 because first col has record IDs
        if not self._training_datafile.endswith('.csv'): 
            raise TypeError("Aborted. get_training_data_from_csv() is only for CSV files")
        filein = open(self._training_datafile,"rU")
        firstline = filein.readline()        
        filein.close()
        self._bigdata = 1 if os.path.getsize(self._training_datafile) / len(firstline) > 5000 else 0
        filein = open(self._training_datafile,"rU")
        records_read = 0
        all_data = []
        while 1:
            ...
            if line == '': break
            records_read += 1
            # The self._bigdata predicate testing is not necessary. It is used as a reminder for future plans.
            if self._bigdata: 
                if records_read%10000 == 0:
                    sys.stdout.write('. ')
                    sys.stdout.flush()
            if self._csv_cleanup_needed: line = cleanup_csv(line)
            all_data.append(line.rstrip().split(','))
        data_dict = {line[0] : line[1:] for line in all_data}
        feature_names = [item.strip('"') for item in data_dict[self._first_field_in_first_record]]
        ...
        class_for_sample_dict = { "sample_" + key.strip('"') : 
               class_column_heading + "=" + data_dict[key][class_name_in_column] 
                                              for key in data_dict if key != self._first_field_in_first_record}
        feature_values_for_samples_dict = {"sample_" + key.strip('"') : 
              list(map(operator.add, list(map(operator.add, feature_names, "=" * len(feature_names))),  
                    [str(convert(data_dict[key][i-1].strip('"'))) for i in self._csv_columns_for_features])) 
                                               for key in data_dict if key != self._first_field_in_first_record}
        features_and_values_dict = {data_dict[self._first_field_in_first_record][i-1].strip('"') : 
          [convert(data_dict[key][i-1].strip('"')) for key in data_dict if key != self._first_field_in_first_record]  
    ...
    def get_training_data(self):
        ...
        if self._training_datafile.endswith('.csv'):
            ...
            raise SyntaxError("Your training datafile must either be a csv file or a dat file")
    ...
    def get_training_data_from_dat(self):
        ...
        for line in FILE:
            ...
            if re.search(r'\s*class', line, re.IGNORECASE) and not recording_training_data \
                ...
                if not m: 
                    raise Exception('''No class names in training file''')
                ...
                if not table_header:
                    ...
                    if (len(self._features_and_values_dict) != len(table_header)-2):
                        raise Exception('''Incorrect number of feature names in the header '''
                                         '''line that is just below the "Training Data:" line''')
                    continue
                record = line.split()
                if record[1] not in class_names:
                    print("Error in data record: " + record[0])
                    raise Exception('''For the data record named above, the class name '''
                ...
                if (len(self._features_and_values_dict) != len(record) - 2):
                    print("Error in data record: " + record[0])
                    raise Exception("Number of values supplied for the above data record is wrong")
                for i in range(2, len(record)):
                    feature_name_for_i = column_labels_dict[i]
                    if record[i] not in self._features_and_values_dict[feature_name_for_i]:
                        print("Error in data record: " + record[0])
                        raise Exception('''For the data record named above, one of the feature '''
...
class TrainingDataGeneratorSymbolic(object):
    ...
    def write_training_data_to_file( self ):
        ...
        if len(features) == 0:
            raise Exception("You probably forgot to call gen_training_data() before calling write_training_data_to_file()") 
...
class TestDataGeneratorSymbolic(object):
    ...
    def __init__(self, *args, **kwargs ):
        if args:
            raise SyntaxError('''TestDataGenerator can only be called with keyword arguments for the following '''
        ...
        if output_test_datafile:
            ...
            raise SyntaxError('''You must specify an output test datafile''')
        if output_class_labels_file:
            self._output_class_labels_file = output_class_labels_file
        else:
            raise SyntaxError('''You must specify an output file for class labels''')
        if parameter_file: 
            self._parameter_file =  parameter_file
        else:
            raise SyntaxError('''You must specify a parameter file''')
        if number_of_test_samples:
            ...
            raise SyntaxError('''You forgot to specify the number of test samples needed''')
        if write_to_file:
            self._write_to_file = write_to_file
        else:
            self._write_to_file = 0          
