...
def repeated(first_value, *values):
    ...
    Repeated values preserve order and allow the same value to appear more
    than once.
    """
...
@dispatch.multimethod
def value_eq(x, y):
    """Are the values of x and y exactly the same?"""
    raise NotImplementedError()
...
IRepeated.implement(
    ...
    implementations={
        getvalues: lambda x: (x,) if x is not None else None,
