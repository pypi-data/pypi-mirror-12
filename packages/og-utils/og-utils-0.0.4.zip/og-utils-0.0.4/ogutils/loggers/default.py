...
class DefaultLogger(ExtraAttributeLogger, DebugEvalLogger):
    def apply_default_handlers(self, log_dir=None, console_enabled=True, max_log_size=5*1024*1024, max_backup_logs=5, formatter=DEFAULT_LOGGER_FORMAT):
        if console_enabled:
        ...
        if log_dir:
            ...
            error_file_handler = logging.handlers.RotatingFileHandler(os.path.join(log_dir, 'errors.log'),
                maxBytes=max_log_size, backupCount=max_backup_logs)
...
def build_default_logger(logger_name='logger', log_level=None, log_dir=None, console_enabled=True, max_log_size=5*1024*1024, max_backup_logs=5):
    '''
