...
def plot(data, width, height):
    ...
    for column, datum in enumerate(data_slice, 2):
        if datum is None:
            canvas.point(P(column, 2), hidden, Back.RED)
            continue
        elif datum is 0:
            continue
        # bar percentage
    ...
    if stats_data:
        ...
        if False:
            ...
            canvas.box(P(width - max_stats_len - 4, height - 2 - len(stats_box)), P(width - 1, height - 1))
        else:
            midpoint = P(
                round(width / 2),
                round(height / 2)
            ...
            for idx, stat in enumerate(stats_box):
                canvas.horizontal_line(stat, midpoint.y + idx, midpoint.x - max_stats_len)

            canvas.box(
                P(midpoint.x - max_stats_len - 1, midpoint.y + len(stats_box)),
                P(midpoint.x, midpoint.y),
                blank=True
            )
...
def _windows():
    ping = subprocess.Popen(["ping", "-t", "google.com"], stdout=subprocess.PIPE)
    while True:
...
def run():
    ...
    for ping in it:
        ...
        if winterm:
            winterm.erase_screen(2)
        else:
            os.system("cls" if platform.system() == "Windows" else "clear")
        width, height = get_terminal_size()
        c = plot(buff, width - 2, height - 2)
