...
############################################
 ...
 Changes:
    Jul 30 2013: findnext_keyword function  and modification to find function
...
class UseExcel(object): 
    ...
    def find_keyword(self,sheetname,item, whole_cell = 0):
        '''
        ...
        if whole_cell:
            Search = self.xlbook.Worksheets(sheetname).Cells.Find(What = item,MatchCase = True, LookAt = 1)#one is xlwhole
        else:
            Search = self.xlbook.Worksheets(sheetname).Cells.Find(What = item,MatchCase = True, LookAt =2 )#two is xlpart
        activecell = Search.Activate()
        return self.get_active_select_add()

    def findnext_keyword(self,sheetname,item, whole_cell = 0, aftercell = None):
        '''
            Designed to take in multiple search time using aftercell (xlapp.ActiveCell object,
            May replace the find_keyword function
            whole cell  =  1,will consider item as the whole in a cell. need owrd together with LookIn= xlValues
                       
        '''

        if aftercell ==None:
            self.activate_sel(sheetname,'A1:A1')
            aftercell  = self.xlapp.ActiveCell
        
        if whole_cell:
            Search = self.xlbook.Worksheets(sheetname).Cells.Find(What = item,MatchCase = True, LookAt = 1, After = aftercell, LookIn = -4163)#one is xlwhole
        else:
            Search = self.xlbook.Worksheets(sheetname).Cells.Find(What = item,MatchCase = True, LookAt =2, After = aftercell )#two is xlpart
        activecell = Search.Activate()
    ...
    def copy_data_fr_csv(self, filename, sheet_to_paste, do_transpose = 0):
        '''
            ...
            need the copy range?? --> need to add this as an option
        ...
        try:
            ...
            #src_xlbook.ActiveSheet.Cells.Copy()             #can select range of the copy function
            src_xlbook.ActiveSheet.Range("A1:IF500").Copy() 
...
##>>> a.MarkerForegroundColor
    ...
    def paste_pic_to_excel(self, sheetname, celladdress =(1,1), pic_file = r'' ):
        """Method to paste the pic file into target sheetname and target cell address.
            Args:
                sheetname (str): excel sheet name
                celladdress (str): cell address
                pic_file (str): filepath of pic file.

        """
        #self.activate_sel(sheetname,celladdress)
        sht = self.xlbook.Worksheets(sheetname)
        sht.Activate()
        sht.Cells(celladdress[0], celladdress[1]).Select()
        sht.Pictures().Insert(pic_file).Select()
...
##        xlapp.Application.DisplayAlerts =False
    ...
    if test == 11:
        xlFile =  UseExcel(r'c:\data\temp\temp\aas.xls')
        xlFile.show()
        #xlbook = xlapp.Workbooks.Add()
        xlFile.activate_sel(sheetname,'A1:A1')

    if test == 12:
        xlFile =  UseExcel()
        xlFile.show()
        xlFile.paste_pic_to_excel('Sheet1', r'C:\data\temp\cdajmpplots\_26_Jun14\ACTUAL_CMR_ADC_AVGvsWPE_UIN_bivar.png')
...
if (__name__ == "__main__"):
    ...
    xlFile =  UseExcel()
    xlFile.show()
    xlFile.paste_pic_to_excel('Sheet1', (5,5), r'C:\data\temp\cdajmpplots\_26_Jun14\ACTUAL_CMR_ADC_AVGvsWPE_UIN_bivar.png')
