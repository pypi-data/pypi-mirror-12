...
line_break = '\N{ZERO WIDTH JOINER}\N{PILCROW SIGN}'
...
class Paragraph(Text):
    ...
    def text(self):
        text = super().text()
        if self.wrap:
            text = line_break.join(textwrap.fill(chunk, self.wrap)
                                   for chunk in line_break_re.split(text))
...
class Anchor(Text):
    ...
    def append(self, item):
        # Eliminate font style from the spans, since reST does not handle nested inline markup
        # like `*Foo*`_.
        # FIXME: this could be handled using a substitution, see
...
class Substitution(Directive):
    has_footer_content = True

    def __init__(self, term, type, data=None, content=None, **options):
        self.term = term
        super().__init__('%s %s' % (self, type), data, content, **options)
...
def convert_mf_to_png(content, kind):
    ...
    try:
        try:
            subprocess.check_output(['soffice', '--convert-to', 'png',
                                     '--outdir', str(mf.parent), str(mf)])
        except FileNotFoundError as e:
            raise RuntimeError("Could not convert a %s image to PNG (maybe"
                               " {Libre|Open}Office is not installed on the system?):"
                               " %s" % (kind.upper(), e))
    finally:
        mf.unlink()
    ...
    if not png.exists():
        raise RuntimeError("Could not convert a %s image to PNG: most probably you"
                           " have an instance of {Libre|Open}Office already running,"
                           " close it and retry please" % kind.upper())
...
class Image(Substitution):
    def __init__(self, basedir, name, content, width=None, height=None):
        ...
        super().__init__(make_filename(str(self.path), False), 'image',
                         self.path, width=width, height=height)

    def footer_emit(self, output):
        super().footer_emit(output)
