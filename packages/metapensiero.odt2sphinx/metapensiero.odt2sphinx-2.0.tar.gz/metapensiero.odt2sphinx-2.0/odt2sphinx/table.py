...
class AsciiTable(object):
    ...
    def render(self):
        ...
        # First pass, ignore multi-col cells
        for row in self.rows:
            for c, cell in enumerate(row.cells):
                if cell.columns_span == 1 and cell.width:
                    col_widths[c] = max(col_widths[c], cell.width)

        # First pass, consider only multi-col cells
        for row in self.rows:
            for c, cell in enumerate(row.cells):
                if cell.columns_span == 1 or not cell.width:
                    continue
        ...
        # TODO: compute the row_heights in a similar way and pass it down to the render
        # method, so it can produce shorter tables in presence of spanned rows
        ...
        for row in self.rows:
            ...
            rows.append(row.render(col_widths))
            hrows -= 1
...
class AsciiRow(object):
    ...
    def render(self, col_widths):
        height = max(c.height for c in self.cells)
        rows = []
            ...
            for c, cell in enumerate(self.cells):
                if cell.columns_span == 1:
                    ...
                    width = sum(col_widths[c]
                                for c in range(cell.column, cell.column+cell.columns_span))
                    width += 3 * (cell.columns_span - 1)
                if cell.content and r < cell.height:
                    content = cell.content.pop(0)
                    row.append(' %-*s ' % (width, content))
                else:
                    if cell.master is None or cell.master.columns_span == 1:
                        row.append(' ' * (width + 2))
                if cell.columns_span == 1:
                    row.append('|')
