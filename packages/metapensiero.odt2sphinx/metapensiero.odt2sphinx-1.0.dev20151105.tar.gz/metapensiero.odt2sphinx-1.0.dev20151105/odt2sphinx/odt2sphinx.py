...
# to either remove them or inject a space. Even better could be injecting a
# backslashed-space “\ ”...
...
class RstFile(object):
    ...
    def _addline(self, line, indent=None):
        if line is None:
            line = ''
        if self.paragraph is not None:
            self.paragraph[0].append(line)
        else:
            if line:
                if indent is None:
                    indent = self.current_indent
                line = textwrap.indent(textwrap.fill(line, 90), '   ' * indent)
            self.lines.append(line)

    def start_paragraph(self):
        if self.paragraph is None:
            self.paragraph = [], self.current_indent

    def end_paragraph(self):
        if self.paragraph:
            lines, indent = self.paragraph
            paragraph = ''.join(lines)
            self.paragraph = None
            self.append(paragraph, indent)
        else:
            self.append()
        self.append()

    def append(self, line='', indent=None):
        self.current._append(line, indent)

    def _append(self, line='', indent=None):
        line = line.strip(' \n')
        self._addline(line, indent)
    ...
    def start_cell(self, columns_span, rows_span):
        self.current = RstFile(self.encoding)
...
class SphinxWriter(object):
    def __init__(self, target, encoding, download_source_link=False):
        ...
        self.files = {}
        self.current_file = RstFile(encoding)

        if target.suffix == '.rst':
            self.monolithic_output = True
            self.dir = target.parent
            self.files[target] = self.current_file
        else:
    ...
    def p_start(self):
        self.current_file.start_paragraph()

    def p_end(self):
        self.current_file.end_paragraph()
    ...
    def list_item(self, text=None, numbered=False):
        if numbered:
            ...
            self.current_file.append('*')
        within_paragraph = self.current_file.paragraph is not None
        if within_paragraph:
            self.current_file.end_paragraph()
        else:
            self.current_file.append()
        self.current_file.start_block()
        if within_paragraph:
            self.current_file.start_paragraph()
        if text is not None:
            self.current_file.append(text)
    ...
    def end_block(self):
        self.current_file.end_block()

    def finalise(self):
        if len(self.files) > 1:
            self.files['index'].end_block()

    def writeout(self):
        if not self.monolithic_output:
            if not self.dir.is_dir():
                self.dir.mkdir()
        if not self.dir.joinpath('images').exists():
            self.dir.joinpath('images').mkdir()
        for fname, rstfile in self.files.items():
            if not self.monolithic_output:
                fname = self.dir.joinpath(fname).with_suffix('.rst')
            print("Generating %s... " % fname, end='', flush=True)
            lastcontent = None
            if fname.exists():
                with fname.open(encoding=self.encoding) as f:
                    lastcontent = f.read()
            content = rstfile.getvalue()
            if self.download_source_link:
                content += """
.. only:: html

    OpenOffice source: :download:`%s <%s>`.

""" % (self.download_source_link.name, self.download_source_link)
            if content != lastcontent:
                with fname.open('w', encoding=self.encoding) as f:
                    f.write(content)
                print("Done")
            else:
                print("No change")
            ...
            for fname, attrs in rstfile.images.items():
                fname = self.dir.joinpath('images', fname)
...
class ODTReader(object):
    ...
    def handle_node(self, node):
        if node.tag in ('{%(text)s}span' % self.ns, '{%(text)s}s' % self.ns):
            ...
            self.call_visitor('on_text', ' ')

        else:
            if self.debug:
                print("Unsupported node tag: %r" % node.tag)
...
class Converter(object):
    def __init__(self, source, target, encoding, debug=False, download_source_link=False):
        from os.path import relpath
        self.reader = ODTReader(source, self, debug)
        if download_source_link:
            download_source_link = pathlib.Path(relpath(str(source), str(target)))
        self.writer = SphinxWriter(target, encoding, download_source_link)
        self.buffer = None
    ...
    def set_last_block(self, name=None):
        changed = name != self.last_block
        self.last_block = name
        return changed

    def hightlight_start(self, name):
        if self.last_block == name:
            self.writer.current_file.start_block()
        else:
...
def convert_odt(source, target, encoding, debug=False, download_source_link=False):
    converter = Converter(source.resolve(), target.absolute(), encoding, debug=debug,
...
def main():
    ...
    parser.add_argument('source', type=pathlib.Path,
                        help='Source ODT file to be converted')
    parser.add_argument('target', type=pathlib.Path, nargs='?',
                        help='Either destination directory or single .rst target filename')

    args = parser.parse_args()

    if not args.target:
        args.target = args.source.with_suffix('')

    convert_odt(**args.__dict__)
