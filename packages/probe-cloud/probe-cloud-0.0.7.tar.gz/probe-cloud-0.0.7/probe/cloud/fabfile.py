import os
from fabric.api import run, cd, local, env, put, abort, get, settings, prefix, task
...
@task
def sim_kill(tempstr, dbfile='~/.simulations.sqlite'):
    db, simulations = get_db_and_simulations_table(dbfile)
    tempdir = simulations.find_one(tempstr=tempstr)['tempdir']
...
@task
def install_python_packages(package, virtualev='probe'):
