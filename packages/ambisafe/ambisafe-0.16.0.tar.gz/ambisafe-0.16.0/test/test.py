...
class ClientTestCase(unittest.TestCase):
    ...
    def test_get_hex_balance(self):
        url = 'http://localhost:8080/balances/ETH/{external_id}'.format(external_id=self.id)
        httpretty.register_uri(httpretty.GET, url,
                               body=json.dumps({u'address': u'38gP2E6Fj2s3sx63urYPpSrgTwupHGvKsK',
                                                u'balance': u'0x',
                                                u'balanceInSatoshis': None,
                                                u'currencySymbol': u'ETH'}))
        balance = self.client.get_balance(self.id, 'ETH')
...
class ContainerTestCase(unittest.TestCase):
    ...
    def test_generate(self):
        container = Container.generate('1')
        private_key = container.get_private_key('1')
