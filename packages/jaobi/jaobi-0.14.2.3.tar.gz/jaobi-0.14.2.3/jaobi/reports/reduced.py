...
class AggregatedReportMixin(ReportMixin):
    ...
    def do_aggregate(cls, **kwargs):
        ...
        field. It does an 'incremental' aggregation by sending the
        aggregation result to a temp collection and map reduce it to
        the final collection (read the code!)
...
class ReferrerReportMixin(ReportMixin):
    ...
    @classmethod
    @gen.coroutine
    def get_referrer_report(cls, **kwargs):
...
class ReferrerConsumption(MapReduceDocument, ReferrerReportMixin,
    ...
    aggregation_fields = {'referrer': '$referrer'}
...
class IncrementalMapReduce(Document):
    ...
    _reduced = []
    _incr_lock = None
    _today_incr_lock = None
    _yesterday_incr_lock = None
    ...
    def do_incremental_map_reduce(cls):
        ...
        if cls._incr_lock is not None:
            return []

        cls._incr_lock = 'lóqui'
        futures = []
    ...
    def do_today_map_reduce(cls):
        ...
        if cls._today_incr_lock is not None:
            return []

        cls._today_incr_lock = 'lóqui'
        futures = []
        now = datetime.datetime.now()
        today = datetime.datetime(now.year, now.month, now.day)
        kw = {'inclusion_date__gte': today}
        for reduced_cls in cls._reduced:
            log('today map reduce for {}'.format(fqualname(reduced_cls)),
                level='warning')
            future = reduced_cls.do_aggregate(**kw)
            futures.append(future)

        cls._today_incr_lock = None
        return futures
    ...
    def do_yesterday_map_reduce(cls):
        ...
        if cls._yesterday_incr_lock is not None:
            return []

        cls._yesterday_incr_lock = 'lóqui'
        futures = []
        now = datetime.datetime.now()
        today = datetime.datetime(now.year, now.month, now.day)
        ...
        for reduced_cls in cls._reduced:
            log('yesterday map reduce for {}'.format(fqualname(reduced_cls)),
                level='warning')
            future = reduced_cls.do_aggregate(**kw)
            futures.append(future)

        cls._yesterday_incr_lock = None
        return futures
...
def _hack_clean_bad_collections():
    ...
    db = coll.database
    collections = yield db.eval('db.getCollectionNames()')
