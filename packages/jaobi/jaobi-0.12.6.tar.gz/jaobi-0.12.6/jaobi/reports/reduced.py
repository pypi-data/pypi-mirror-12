...
from jaobi.reports.exceptions import BadReport
...
class ContentReportMixin(ReducedReportMixin):
    ...
    def _format_content_result(cls, result):
        ...
        contents = yield models.Content.objects.filter(id__in=contents).to_list()
...
class SiteThemeConsumption(MapReduceDocument, ThemeReportMixin,
                           SiteReportMixin):
    ...
    This document is different from SiteConsumption and ThemeConsumption
    because here you can filter the reports by theme/site but in other
    hand you don't have history reports.
    """

    map_reduce_js_file = os.path.join('js', 'site_theme_consumption.js')

    @classmethod
    @gen.coroutine
    def get_sites_report_history(cls, **kwargs):
        msg = 'SiteThemeConsumption does not have sites history reports. '
        msg += 'Please use the SiteConsumption class.'
        raise BadReport(msg)

    @classmethod
    @gen.coroutine
    def get_themes_report_history(cls, **kwargs):
        msg = 'SiteThemeConsumption does not have themes history reports. '
        msg += 'Please use the ThemesConsumption class.'
        raise BadReport(msg)
...
class IncrementalMapReduce(Document):
    ...
    @classmethod
    @gen.coroutine
    def do_daily_incremental_map_reduce(cls):
        """Do an incremental map reduce for the last 24 hours."""

        futures = []
        today = datetime.datetime.today()
        last_day = today - datetime.timedelta(days=1)
        for reduced_cls in cls._reduced:
            future = reduced_cls.do_map_reduce(inclusion_date__gte=last_day)
            futures.append(future)
...
if hasattr(settings, 'USE_REDUCED_REPORTS') and settings.USE_REDUCED_REPORTS:
    ...
    scheduler.add(IncrementalMapReduce.do_daily_incremental_map_reduce, secs)
