...
def update_themes_consolidation(cls, document):
    ...
    log('updating {} themes'.format(len(themes)), level='warning')
    for theme in themes:
        ...
        yield consolidated.update(inc__total=1)
...
class ThemeDailyConsumers(Document):
    ...
    def update_consumers(cls, sender, document, **kwargs):
        ...
        if consumer is None:  # pragma: no cover
            log('no consumer for consumption {}'.format(str(document.id)),
                level='warn')
            return
        ...
        for theme in themes:
            ...
            if str(consumer.id) in consolidated.consumers:
                log('consumer {} already on theme {} {}'.format(
                    consumer.id, theme, today), level='warning')
                continue

            consumers = consolidated.consumers
            consumers.append(str(consumer.id))
            yield consolidated.update(set__consumers=consumers, inc__total=1)
...
class ReferrerDailyConsumption(Document):
    ...
    def update_consumption(cls, sender, document, **kwargs):
        if not document.referrer:
            log('no referrer for consumption')
            return
        today = datetime.date.today()
    ...
    @gen.coroutine
    def get_consumption_history(cls, **kwargs):
        consumption = yield get_consolidation_history(cls, 'referrer',
                                                      **kwargs)
        return consumption
...
class ContentTotalConsumption(Document):
    content = URLField(required=True, unique=True)
    total = IntField(default=0, required=True)
    ...
    def update_consumption(cls, sender, document, **kwargs):
        ...
        if not content:  # pragma: no cover
            msg = 'no content for content consumption {}'.format(
                str(document.id))
            log(msg, level='error')
            return

        kw = {'content': content.url}
        try:
            consolidated = yield cls.objects.get(**kw)
        except cls.DoesNotExist:
            consolidated = cls(**kw)
    ...
    @gen.coroutine
    def get_consumption(cls, **kwargs):
        consumption = yield get_consolidation(cls, 'content', **kwargs)
        return consumption
...
class ConsumptionAverageTime(Document):
    ...
    def update_consumption(cls, sender, document, **kwargs):
        ...
        for theme in themes:
            ...
            secs = consumption_time.seconds
            avg = ((consolidated.total + secs) /
                   (consolidated.access_count + 1))
            yield consolidated.update(inc__total=secs, inc__access_count=1,
                                      set__average=avg)
    ...
    def get_average_consumption(cls, **kwargs):
        ...
        for consumption in (yield qs.to_list()):
            if not consumption:  # pragma: no cover
                continue
...
signals.post_save.connect(ContentTotalConsumption.update_consumption,
                          sender=ContentConsumption)

signals.post_bulk_insert.connect(
    ContentTotalConsumption.bulk_update_consumption,
    sender=ContentConsumption)
