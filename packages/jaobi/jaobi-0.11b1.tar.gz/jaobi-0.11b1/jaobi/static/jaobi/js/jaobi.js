...
(function(o){
  ...
  o.is_accessed = function(url) {
    if (interphone_get_local(o.confs.ACCESS_COOKIE,"cookie")) {
      var urls = interphone_get_local(o.confs.ACCESS_COOKIE,"cookie").split("|");
  ...
  o.set_access = function(url) {
    var urls = interphone_get_local(o.confs.ACCESS_COOKIE,"cookie");
    urls = (urls || "").split("|");
    urls.unshift(url);
    urls.splice(o.confs.MAX_ACCESS_DATA,urls.length);
    urls = urls.join("|");
    interphone_set_local(o.confs.ACCESS_COOKIE,urls,"cookie")
    return url;
  }
...
(function(o){
  ...
  function retrieve_id(id, callback) {
    ...
    if (o.confs.JAOBI_CONSUMER_URL) {
      ...
      server.set(o.confs.COOKIE_NAME,o.confs.consumerID,"cookie")
    } else {
      interphone_set_local(o.confs.COOKIE_NAME,o.confs.consumerID,"cookie");
    };
  ...
  o.consumer.getID = function(callback) {
    var id = interphone_get_local(o.confs.COOKIE_NAME,"cookie");
    if (id && id != 'undefined' && id !="!storage" && id != "storage") {
      callback(id);
      ...
      if (o.confs.JAOBI_CONSUMER_URL) {
	    ...
	    console.log('is getting');
            server.get(o.confs.COOKIE_NAME,"cookie");
          },
          on_data: function(key,val,type){
	    console.log('tô no on data');
    ...
    function send_URL(contentID) {
      o.consumer.getID(function(consumerID){
	    ...
	    if (response.consumer){
	      o.confs.consumerID = response.consumer.id;
	    }
      ...
      utils.ajax({
		...
		success: function(response){
		  interphone_set_local(
			o.confs.THEMES_COOKIE, JSON.stringify(response.items), "cookie");
		}
      });
    });
  };
  ...
  o.consumer.preferredThemes = function(){
    ...
    var themes = interphone_get_local(o.confs.THEMES_COOKIE, "cookie");
    if (themes){
...
(function(o){
  ...
  o.content.getLastConsumers = function($quantity, callback){
    ...
    var recorded = interphone_get_local(o.confs.CONSUMPTION_COOKIE,"cookie");
    if (recorded) recorded = JSON.parse(recorded)
    var consumers = (recorded || {});
    ...
    function classifyConsumers(last_consumers){
      ...
      interphone_set_local(o.confs.CONSUMPTION_COOKIE,JSON.stringify(consumers),"cookie");
      return consumers;
    };
