...
class TestShipping:
    ...
    def test_fedex_labels_single_package_international(
            self, dataset, transaction
    ):
        """Generate fedex label if there is single package.
        """
        Sale = self.POOL.get('sale.sale')
        Attachment = self.POOL.get('ir.attachment')
        Address = self.POOL.get('party.address')
        Country = self.POOL.get('country.country')
        Subdivision = self.POOL.get('country.subdivision')
        GenerateLabel = self.POOL.get('shipping.label', type="wizard")
        ...
        sale, = Sale.create([{
            ...
            'fedex_drop_off_type':
                data.get_fedex_drop_off_type('REGULAR_PICKUP'),
            'fedex_packaging_type':
                data.get_fedex_packaging_type('YOUR_PACKAGING'),
            'fedex_service_type':
                data.get_fedex_service_type('INTERNATIONAL_PRIORITY'),
            'lines': [('create', [{
                'type': 'line',
                'quantity': 1,
                'product': data.product1.id,
        ...
        country_in, = Country.create([{
            'name': 'India',
            'code': 'IN',
        }])

        subdivision_del, = Subdivision.create([{
            'name': 'Delhi',
            'code': 'IN-DL',
