...
def has_permission_level(graph, level):
    ...
    if current_app.login_manager._login_disabled: # pylint: disable-msg=protected-access
        return True
...
def get_rel_types(graph):
    from .models import RelationshipType
    rel_types = g.db.query(RelationshipType.name).filter_by(
        graph_id=graph.id).order_by(RelationshipType.name)
    return [ rt.name for rt in rel_types ]
