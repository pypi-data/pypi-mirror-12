...
from flask import redirect, request, url_for, session, abort, jsonify
from webportfolio import (WebPortfolio, route, flash_error, flash_success,
                          flash_info, flash_data, get_flashed_data, register_package,
                          ModelError, ViewError)
from webportfolio.decorators import (nav_menu, with_user_roles)
...
def post(view, **kwargs):
    ...
    # templates directory
    templates_dir = kwargs.get("templates_dir", "WebPortfolio/Package/Cms/Post")
    ...
    nav_menu_context = view.nav_menu_context()
    ...
    class Postpage(object):
        ...
        def post_index(self):
            ...
            app_per_page = self.config_("APPLICATION_PAGINATION_PER_PAGE", 10)
            per_page = opt_endpoints.get("index.per_page", app_per_page)

            self.meta_(title=opt_endpoints.get("index.title", "All Posts"))
            ...
            return self.render_(posts=posts,
                               view_template_="%s/post_index.html" % templates_dir,
                               **_kwargs)

        @nav_menu("Post", show=False, **nav_menu_context)  # No need to show the read in the menu
        ...
        def post_authors(self):
            ...
            self.meta_(title=opt_endpoints.get("authors.title", "Authors"))

            authors = []
            return self.render_(authors=authors,
                               view_template_="%s/post_authors.html" % templates_dir)
        ...
        def post_author(self, id, name=None):
            ...
            self.meta_(title=opt_endpoints.get("author.title", "Author"))

            author = []
            return self.render_(author=author,
                               view_template_="%s/post_author.html" % templates_dir)
        ...
        def post_archive(self):
            ...
            Endpoints options
                - archive
                    ...
                    title
            """
            self.meta_(title=opt_endpoints.get("archive.title", "Archive"))
            ...
            return self.render_(posts=posts,
                               view_template_="%s/post_archive.html" % templates_dir)
...
def admin(view, **kwargs):
    ...
    model = kwargs.get("model")
    templates_dir = kwargs.get("templates_dir", "WebPortfolio/Package/Cms/Admin")
    ...
    class Admin(object):
        ...
        decorators = view.decorators + [login_required, with_user_roles('superadmin', 'admin', 'manager', 'editor')]
        ...
        def cms_admin_preview(self, id):
            ...
            self.meta_(title="Read: %s " % post.title)

            return self.render_(post=post,
                               view_template_=template_page % "preview")
        ...
        def cms_admin_edit(self, id):
            ...
            return self.render_(post=post,
                               ...
                               view_template_=template_page % "edit",
        ...
        def cms_admin_categories(self):
            self.meta_(title="Post Categories")
            if request.method == "POST":
                ...
                return self.render_(categories=cats,
                                   view_template_=template_page % "categories")
