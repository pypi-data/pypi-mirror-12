...
It allows you quickly switch the rendenring view of a method.
...
def render_as_template(func):
    """
    Decorator to render as normal template
    :param func:
    :return:
    """
    @functools.wraps(func)
    def decorated_view(self, *args, **kwargs):
        ...
        def_v_t = "%s/%s.html" % (self.__class__.__name__, func.__name__)
        view_template_ = data.pop("view_template_", def_v_t)
        layout_ = data.pop("layout_", None)
        return self.render(data, view_template_=view_template_, layout_=layout_), status
    return decorated_view
