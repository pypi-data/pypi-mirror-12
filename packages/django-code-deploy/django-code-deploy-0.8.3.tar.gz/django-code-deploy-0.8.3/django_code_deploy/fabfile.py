...
@task
def restart_nginx():
    run("/usr/local/opt/python/bin/supervisorctl restart nginx")

@task
def restart_uwsgi():
    run("/usr/local/opt/python/bin/supervisorctl restart uwsgi")

@task
def restart_celery():
    run("/usr/local/opt/python/bin/supervisorctl restart celery")
