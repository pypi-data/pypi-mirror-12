...
class TestCephQaSuite(object):
    ...
    def test_argv_override(self):
        c = ceph_qa_suite.CephQaSuite.factory([
            'ceph-qa-suite',
            '--key-name', 'NAME',
        ])
        assert 'NAME' == c.args.key_name
        c.argv_override('--key-name', 'OTHER')
        assert 'OTHER' == c.args.key_name
    ...
    def test_verify_keys(self):
        ...
        with mock.patch.multiple(
            ...
            assert openstack.OpenStack.get_value(
                keypair, 'fingerprint') == fingerprint.strip()
            #
            # If the --key-name and --key-filename arguments are set,
            # do nothing
            #
