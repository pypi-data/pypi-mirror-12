...
class TestWBRepo(testtools.TestCase):
    ...
    def test_update_release2ranges(self):
        ...
        self.assertEqual({'firefly': [('v0.80..v0.80.1', 'v0.80.1'),
    ...
    def test_remember_picked(self):
        ...
        self.assertEqual(set(['B']), r.commit2picked_by['A'])
        self.assertEqual(set(['A']), r.picked_by2commits['B'])
        r.remember_picked(Commit('A', 'C'))
        self.assertEqual(set(['B', 'C']), r.commit2picked_by['A'])
        r.remember_picked(Commit('D', 'B'))
        self.assertEqual(set(['A', 'D']), r.picked_by2commits['B'])
    ...
    def test_remember_commits(self, m_remember_picked):
        ...
        self.assertEqual('tag', r.commit2tag[c.hexsha])
        self.assertEqual('release', r.commit2release[c.hexsha])
    ...
    def test_update_commits(self, m_remember_commits):
        ...
        self.assertEqual(m_remember_commits.call_args_list, e)
    ...
    def test_update_head2merge(self):
        ...
        r = wbgit.WBRepo.factory(['--git-directory', self.d,
                                  '--releases=firefly']).open()
        r.update_head2merge()
        self.assertEqual(1, len(r.head2merge))
    ...
    def test_update_release2last_tag(self):
        ...
        self.assertEqual('v0.80.1', r.release2last_tag['firefly'])
