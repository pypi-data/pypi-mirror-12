...
class ResourceMapGenerator():
  ...
  def simple_generate_resource_map(self, resource_map_pid, science_metadata_pid,
                                   science_data_pids):
  ...
  def generate_system_metadata_for_resource_map(self, resource_map_pid,
    resource_map, rights_holder,
    ...
    checksum = d1_common.checksum.create_checksum_object(resource_map,
      checksum_algorithm)
    return self._generate_sys_meta(resource_map_pid, RDFXML_FORMATID, size,
                                   checksum, rights_holder)
  ...
  def _generate_resource_map(self, aggregation_id, resource_map_id, relations):
    ...
    resmap = foresite.ResourceMap(self._resolvable_uri_from_pid(
      resource_map_id))
  ...
  def _generate_sys_meta(self, pid, format_id, size, checksum, rights_holder,
                         modified=None):
    if modified is None:
