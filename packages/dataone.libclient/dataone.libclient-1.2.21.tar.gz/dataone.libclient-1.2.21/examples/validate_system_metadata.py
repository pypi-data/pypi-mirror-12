...
def main():
  mn_client = d1_client.mnclient.MemberNodeClient(BASE_URL,
    cert_path=CERTIFICATE_PATH, key_path=CERTIFICATE_KEY_PATH)
...
class DataONENodeObjectValidator(object):
  ...
  def _validate_pid(self, pid):
    ...
    if errors:
      self._write_csv_row(pid, '/'.join(errors), sys_meta.size,
                          sys_meta.checksum.value(), len(sci_obj), checksum)
      raise ValidationError()
    else:
      self._write_csv_row(pid, 'ok', sys_meta.size,
                          sys_meta.checksum.value(), len(sci_obj), checksum)
  ...
  def _read_sys_meta(self, pid):
    ...
    except (d1_common.types.exceptions.DataONEException, pyxb.UnrecognizedDOMRootNodeError):
  ...
  def _write_csv_row(self, pid, status, sys_meta_size=None, sys_meta_checksum=None, actual_size=None, actual_checksum=None):
    self._csv_file.writerow([status, pid, sys_meta_size, sys_meta_checksum, actual_size, actual_checksum])
