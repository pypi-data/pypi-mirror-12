...
class CommandNotAvailableException(KempTechApiException):
    def __init__(self, cmd_name):
        msg = "Command '{}' is not available on the LoadMaster.".format(
            cmd_name)
        super(CommandNotAvailableException, self).__init__(msg,
                                                           is_xml_msg=False)


class InvalidOperationException(KempTechApiException):
    def __init__(self, cmd_name):
        msg = "Command '{}' is not available on the LoadMaster.".format(
            cmd_name)
...
class ConnectionTimeoutException(KempTechApiException):
    def __init__(self, lm):
        ...
        super(ConnectionTimeoutException, self).__init__(msg,
                                                         is_xml_msg=False)
