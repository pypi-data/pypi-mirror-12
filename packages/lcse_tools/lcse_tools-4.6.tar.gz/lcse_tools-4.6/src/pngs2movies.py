...
def make_movie_from_pngs(image_path, **kwargs):
  ...
  if kwargs.get('add_time'):
    rtp = lcse.rtplot_reader(kwargs.get('add_time'))
  ...
  video_filename = '%s_%04i-%04i_%s.avi' % (video_prefix, dmin, dmax, str(fps))
...
def main():
  ...
  parser.add_argument('--add-time', help='Specify RTplot.ppm file to use for timing info')
