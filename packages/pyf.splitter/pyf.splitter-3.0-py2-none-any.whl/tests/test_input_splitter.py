...
def test_tokenize():
    ...
    result = tokenize('', separator='\x00')
    assert result == ([], ''), (
        "Tokenize returned (%s, '%s') "
        "instead of ([], '')" % (result[0], result[1])
    )

    result = tokenize('\x00', separator='\x00')
    assert result == ([''], ''), (
        "Tokenize returned (%s, '%s') "
        "instead of ([''], '')" % (result[0], result[1])
    )

    result = tokenize('s\x00', separator='\x00')
    assert result == (['s'], ''), (
        "Tokenize returned (%s, '%s') "
        "instead of (['s'], '')" % (result[0], result[1])
    )

    result = tokenize('s', separator='\x00')
    assert result == ([], 's'), (
        "Tokenize returned (%s, '%s') "
        "instead of ([], 's')" % (result[0], result[1])
    )

    result = tokenize('\x00s', separator='\x00')
    assert result == ([''], 's'), (
        "Tokenize returned (%s, '%s') "
        "instead of ([''], 's')" % (result[0], result[1])
    )
...
def test_simple_splitter():
    ...
    # only one item with max_items > 1
    result = get_splitter(input_items, 10).split()
    assert len(result) == 1, (
        "The splitter returned %s files instead of 1" % len(result)
    )
    ...
    result = get_splitter(input_items, 3).split()
    assert len(result) == 4, (
    ...
    # list of items with max_items > nb items
    result = get_splitter(input_items, 100).split()
    assert len(result) == 1, (
        "The splitter returned %s files instead of 1" % len(result)
    )
