...
class rprofile_set(object):
  ...
  def __init__(self, **kwargs):
    ...
    `path` is the path to a directory containing profiles
    
    `files` a list of files to open
  ...
  def __iter__(self):
    self._current_ix = 0
    self._current = rprofile(self.files[0], lazy=self.lazy, logging=self._logging)
    return self
  ...
  def next(self):
    ...
    if self._current_ix < len(self.files):

      if self._current_ix < len(self.ray_profiles):
        rp = self.ray_profiles[self._current_ix]
      else:
        rp = rprofile(self.files[self._current_ix], lazy=self.lazy, logging=self._logging)

        if not self.lazy:
          self.ray_profiles.append(rp)
  ...
  def get_dump(self, dump=None):
    ...
    return rprofile(self.dump_map[dump], lazy=self.lazy, logging=self._logging)

  def get_dump_list(self):
    """ Get list of the dumps in this set Use `.dumps` instead... maybe?"""
