...
.. note::
   ...
   Examples below mention `dynamic_username`_ extension, but this is
   just an example, for other extensions things should be done in the
   same way.
...
one may use Mercurial without installing separate Python at all), and
installing into bundled Python path is uneasy. To remedy that,
extensions utilizing this module are able to find it if it is placed
in the same directory as their own code, or in the sibling directory.

The following two methods of installation are available:

1. If you have some Python installed, you may still install the module
   ...
   path, for example by writing in your ``Mercurial.ini``::
     ...
     [extensions]
     dynamic_username = C:/Python27/Lib/site-packages/dynamic_username.py

   .. note:: 

      This works because ``dynamic_username`` checks for
      ``mercurial_extension_utils`` saved in the same directory (and
...
2. If you don't have any Python, clone both the extension(s)
   ...
   by writing in ``Mercurial.ini``::
     ...
     [extensions]
     dynamic_username = C:/MercurialPlugins/mercurial-dynamic_username/dynamic_username.py

   .. note::

      Directory names matter. This works because
      ``dynamic_username.py`` checks for
...
then either symlink system-wide by repeating in every repo::

     sudo python setup.py develop

(this is not polite on multi-user system, but handy on personal laptop), or
activate the extension(s) by full path, by writing in ``~/.hgrc`` 
something like::

     [extensions]
     dynamic_username = ~/sources/mercurial-dynamic_username/dynamic_username.py
