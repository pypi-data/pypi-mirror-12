...
class SCMStore(FragmentStore):
    ...
    def get_developers(self, begin=0, end=None, rid=None, withstamps=False, limit=None, start=None):
        ...
        if len(commits):
            ...
            developers = [tuple(self.db.hmget('frag:devs:-{}-'.format(d_uri), 'id', 'external')) for d_uri in developers]
            return map(lambda (did, external): (did, external == 'True'), filter(lambda x: x is not None, developers))

        return []

    def get_developer_uris(self, *devs):
        for dev in devs:
            yield {'mid': dev, 'uri': self.db.get('frag:members:{}:'.format(dev))}
