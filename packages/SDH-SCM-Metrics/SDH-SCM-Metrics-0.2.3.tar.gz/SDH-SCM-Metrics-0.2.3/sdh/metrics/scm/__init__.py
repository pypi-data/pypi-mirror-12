...
def update_interval_developers(begin, end):
    ...
    if len(devs):
        ...
        [store_calc(st, 'metrics:total-product-devs:{}'.format(x), begin, list(product_devs[x])) for x in products]
        [store_calc(st, 'metrics:total-project-devs:{}'.format(x), begin, list(project_devs[x])) for x in projects]
