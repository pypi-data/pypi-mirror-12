...
class Server(object):
    def __init__(self, name='', prefix='ws', threaded=False, host='localhost', user='guest', password='guest',
                 vhost='/', heartbeat=60, dumper=None):
    ...
    def connect(self):
        self.connection = amqp.Connection(heartbeat=self.heartbeat, **self.credentials)
    ...
    def start(self):
        ...
        try:
            self.connect()
            p = threading.Thread(target=self.schedule_heartbeat)
            p.daemon = True
            p.start()
            while True:
                self.channel.wait()
