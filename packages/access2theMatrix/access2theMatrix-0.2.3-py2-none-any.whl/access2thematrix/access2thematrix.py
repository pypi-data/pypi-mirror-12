# -*- coding: utf-8 -*-
#
#   Copyright © 2014 - 2015 Stephan Zevenhuizen
#   access2theMatrix, (08-12-2015).
#
...
class Im(object):
    ...
    def __init__(self):
        ...
        self.x_offset = 0
        self.angle = 0
        self.channel_name_and_unit = [u'', u'']
...
class Cu(object):
    ...
    def __init__(self):
        ...
        self.x_data_name_and_unit = [u'', u'']
        self.y_data_name_and_unit = [u'', u'']
...
class MtrxData(object):
    ...
    def _scan_raw_param(self, dp, data):
        ...
        if id == 'DPXE':
            ...
            for i in range(n_keys):
                ...
                channel_id = ''.join(['{:02x}'.format(ord(c)) for c in s[::-1]])
                dpb += dlb + 4
                dlb = 4
                key = unpack('<L', data_block[dpb:dpb + dlb])[0]
                dpb += dlb
            ...
            if s1:
                ...
                if s2 and s1.group(2) == u'STS_LOCATION':
    ...
    def _transfer(self, data):
        keys = [k for k in self.param.iterkeys() if re.match('DICT::', k)]
        channel_name = self.channel_name.decode('utf-8')
        key_nr = str(sorted([int(k[6:]) for k in keys
                             if self.param[k][0] == channel_name])[-1])
    ...
    def _cu_data(self):
        if '(V)' in self.channel_name:
            ...
            self.x_data_name_and_unit = [u'Spectroscopy Device 1', unit]
            ...
            self.x_data_name_and_unit = [u'Spectroscopy Device 2', unit]
            ...
            self.x_data_name_and_unit = [u'Frequency/Phase Detector', unit]
        else:
            ...
            self.x_data_name_and_unit = [u'---', unit]
        if (self.curve_mode == _Curve_Mode.SPS_CURVE_V or
            ...
            if self.param.has_key(key):
                ...
                if s1:
                    ...
                    if self.channel_id.has_key(channel_id):
    ...
    def get_experiment_element_parameters(self):
        eepas = sorted([[k[6:]] + v for k, v in self.param.iteritems()
                        if re.match('EEPA::', k)])
        message = u'{0:<55} {1:>20} {2:<12}\n'.format(u'Parameter', u'Value',
                                                      u'Unit')
        message += u'-' * 89 + u'\n'
        for i in eepas:
            message += u'{0:<55} {1!s:>20} {2:<12}\n'.format(*i)
