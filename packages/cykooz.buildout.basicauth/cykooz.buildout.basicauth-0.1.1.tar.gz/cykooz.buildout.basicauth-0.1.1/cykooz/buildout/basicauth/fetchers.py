...
class PromptFetcher(Fetcher):
    ...
    def search(self, uri, realm):
        ...
        for i in range(self.max_tries):
            username = input('Username for %s: ' % realm)
...
class BuildoutFetcher(Fetcher):
    ...
    def __init__(self, mgr):
        ...
        if 'basicauth' not in self.mgr.buildout:
            return

        basicauth = self.mgr.buildout['basicauth']
        if 'credentials' in basicauth:
            credentials = basicauth.get('credentials').split()
            for partname in credentials:
...
if keyring:
    ...
    class KeyringFetcher(Fetcher):
        ...
        name = 'keyring'
        SERVICE = 'cykooz.buildout.basicauth'
        SEP = ':|'
