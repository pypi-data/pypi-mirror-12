...
class TestInjectionDecorator(TestCase):
    ...
    def setUp(self):
        ...
        self.auth_func.return_value = download.addinfourl(BytesIO(b'SUCCESS'), {}, '', 200)

        self.func = download.inject_credentials(self.credentials)(self.auth_func)

    def test_passthru(self):
        res = self.func('http://www.isotoma.com/')
        res = res.read()
        self.assertEquals(res, b'SUCCESS')
        self.assertEquals(self.auth_func.call_count, 1)

    def test_passthru_2(self):
        self.auth_func.side_effect = MockPopper(AuthException('boom'),
                                                download.addinfourl(BytesIO(b'SUCCESS'), {}, '', 200))

        self.credentials.search.return_value.append(('andy', 'penguin55', True))
        self.assertEquals(self.func('http://www.isotoma.com/').read(), b'SUCCESS')
