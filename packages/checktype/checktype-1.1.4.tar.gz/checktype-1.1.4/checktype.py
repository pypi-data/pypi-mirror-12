...
def checktype(obj, spec, bypass=False):
  if checktype_active or bypass: # by default, pass
    _check(obj, _parse_spec(spec), obj, spec) # recursive, baby!
...
class check(object):
  ...
  def __init__(self, spec):
      if '->' in spec: # beware it can be int->int, ->int, int->
          ...
          self.spec_in = spec
      #self.spec_in_parsed = _parse_spec(self.spec_in)
      #print 'self.spec_in', self.spec_in, 'self.spec_out', self.spec_out

  def __call__(self, fn):
      def checktype_wrapping_function(*args, **kwargs):
      ...
      return checktype_wrapping_function
