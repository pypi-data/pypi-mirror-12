...
def get_field_value(root, field_descriptor, full_name, set_default=False):
    ...
    for level in level_fields[:-1]:
        if not set_default and not parent.HasField(level):
            #print '[get_field_value] a) parent.HasField(%s)=%s' % (level, parent.HasField(level))
            return np.NaN
        parent = getattr(parent, level)
    level = level_fields[-1]
    if not set_default and not parent.HasField(level):
        #print '[get_field_value] b) %s.HasField(%s)=%s' % (parent.DESCRIPTOR.name, level, parent.HasField(level))
        #for f, v in parent.ListFields():
            #print '  %s: %s' % (f.name, v)
        return np.NaN
