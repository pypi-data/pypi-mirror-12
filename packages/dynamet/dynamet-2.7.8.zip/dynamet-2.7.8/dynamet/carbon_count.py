...
def estimate_num_c_idms(t_nl, t_idms, isol_width=3*MMU, rttol=10, p_c_source=0.99, max_c=40):
    ...
    potential=potential_idms_pairs(t_m0, t_ul, rttol=rttol_pattern, max_c=max_c)
    if len(potential): # idms_pairs were found
...
def _ident_common_features(ref_m0, t_idms, isol_width=3*emzed.MMU, rttol=10, max_c=40):
...
#        & t_idms.rt.inRange(rtmin, rtmax) & ((t_idms.z==ref_m0.z) | (t_idms.z==0)))
    common=t_idms.join(ref_m0, t_idms.mz.equals(ref_m0.mz, abs_tol=isol_width)\
        ...
        ((t_idms.z.equals(ref_m0.z, abs_tol=None)) | (ref_m0.z==0)))
...
def potential_idms_pairs(t_m0, t_ul, rttol=5, mztol=0.8*MMU):   
    ...
    m0mul.addColumn('c_d_istance',((m0mul.z*m0mul.mz__0-m0mul.mz*m0mul.z)/delta_c).apply(round), 
                                    type_=int, format_='%d')
