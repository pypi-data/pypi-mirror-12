...
def plot_dli_hm_and_curves(mat, curves, sample_order, title=None, path=None):
    if mat!=None:
        ...
        __, unit=smart_time_axis(max(sample_order['time']))
        time_points=_convert(time_points, unit)
        xlabel_hm=[str(v)[:3] for v  in time_points]
        plt.subplot(122)
        dict_to_mat.plot_heatmap(mat, xlabel_hm, ylabel_hm, unit, title=title, 
...
def _convert(time_points, unit):
    conv={'s':1.0, 'min':60.0, 'h': 300.0}
    return [np.round(t/conv[unit],1) for t in time_points]
