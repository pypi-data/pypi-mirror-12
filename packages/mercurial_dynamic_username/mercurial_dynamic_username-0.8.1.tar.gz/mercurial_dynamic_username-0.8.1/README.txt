...
and can be quoted. Repository matches the rule if it belongs to the directory tree(s)
specified here. Tildas (``~/..`` and ``~john/...``) are properly expanded.
...
update to tagged versions and activate by path::
    ...
    [extensions]
    dynamic_username = C:/hgplugins/mercurial-dynamic_username/dynamic_username.py
...
.. note::
   ...
   Directory names matter. If ``dynamic_username.py`` can't find
