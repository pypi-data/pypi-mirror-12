...
def do_fastqc(uuid, fastq_path, cpu_count, engine, logger):
    ...
    if pipe_util.already_step(fastq_dir, 'fastqc_' + fastq_base, logger):
        ...
        cmd = ['fastqc', '--threads', str(cpu_count), '--extract', fastq_path]
...
def fastqc(uuid, fastq_path, engine, logger):
    cpu_count = pipe_util.get_cpu_count()
    do_fastqc(uuid, fastq_path, cpu_count, engine, logger)
    fastqc_to_db(uuid, fastq_path, engine, logger)
    return
