...
from masheryapi.v3.auth import Auth
from masheryapi.v3.base import Base
from masheryapi.v3.apis import Apis
...
class ImportExternalApiDefinitions():
    ...
    def import_definition(self, access_token, external_api_definition_source, public_domain, create_package, create_iodoc):
        ...
        api_create_response = self.mashery_v3.create(access_token, '/services', 'fields=id,name,endpoints', api_definition)

        if api_create_response.status_code != 200:
            raise ValueError("Some api problem.") 

        created_api = api_create_response.json()

        imported_definition['api'] = {'id': created_api['id'], 'name': created_api['name']}

        if api_create_response.status_code == 200 and create_package == 'true':
