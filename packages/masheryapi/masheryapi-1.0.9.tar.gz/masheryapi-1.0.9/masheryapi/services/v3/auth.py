...
class Auth:
    ...
    def authenticate(self, authorization_code, scope, redirect_uri):
        try:
    ...
    def get_access_token(self, authorization_code, scope, redirect_uri ):
        ...
        if access_token == None or access_token['request_time'] - time.time() > 15:
            self.authenticate(authorization_code, scope, redirect_uri)
            access_token = self.access_token_from_list(authorization_code, scope, redirect_uri)
            if access_token == None:
                return None
            else:
