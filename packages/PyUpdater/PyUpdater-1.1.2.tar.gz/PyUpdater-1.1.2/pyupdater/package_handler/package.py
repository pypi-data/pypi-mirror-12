...
def cleanup_old_archives(filename=None, directory=None):
    ...
    if package_info.info['status'] == False:
        log.debug('Not an archive format: {}'.format(package_info.name))
        return

    log.debug('Current version: {}'.format(str(current_version)))
    assert package_info.name is not None
    log.debug('Name to search for: {}'.format(package_info.name))
    with ChDir(directory):
        ...
        for t in temp:
            ...
            if package_info.name not in t:
                ...
                log.debug('Latest name: {}'.format(package_info.name))
                log.debug('Old name: {}'.format(t))
            ...
            if old_version < current_version:
                ...
                log.info('Removing old update: {}'.format(old_path))
                remove_any(old_path)
            else:
...
class Package(object):
    ...
    """
    regex = re.compile('(?P<name>\w+)-[A-Za-z]+-[0-9]\.[0-9]')
    ...
    def extract_info(self, package):
        ...
        log.debug('Got name of update: {}'.format(self.name))
    ...
    def _parse_package_name(self, package):
        ...
        log.debug('Regex name: {}'.format(name))
        return name
