...
def get_highest_version(name, plat, channel, easy_data):
    ...
    if version is not None:
        ...
        if version is not None:
            log.debug('*** Backwards compat *** Highest version: {}'.format(version))
            # End Todo
        else:
...
def get_size_in_bytes(filename):
    ...
    log.debug('File size: {} bytes'.format(size))
    return size
    ...
    def _parse_version_str(self, version):
        ...
        # Convert to number for easy comparison and sorting
            ...
            try:
                ...
                log.error('Cannot parse pre-release info.')
                log.warning('Accepted pre-releases are a, b, alpha or beta')
...
class Restarter(object):
    ...
    def __init__(self, current_app, **kwargs):
        ...
        log.debug('Current App: {}'.format(self.current_app))
        if self.is_win is True:
            log.debug('Restart script dir: {}'.format(self.data_dir))
            log.debug('Update path: {}'.format(self.updated_app))
