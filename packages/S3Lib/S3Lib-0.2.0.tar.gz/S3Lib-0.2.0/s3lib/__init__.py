...
##########################
  ...
  def _s3_retry_request(self, method, bucket, key, args, headers, content, tries, timeout):
    repeater = retry(period = 0, max_tries = tries, max_time = timeout)
    s3_repeater = self._s3_request
    return s3_repeater(method, bucket, key, args, headers, content)
