...
class CircuitBase(VecArray, Shape):
    ...
    def displaced_by_vector_to(self, vec):
        delta = vec -self.pos
        return type(self)([x + delta for x in self]) 
...
class Circuit(CircuitBase, Immutable):
...
class mCircuit(CircuitBase, Mutable):

    '''A mutable polygon'''


class PolyAny(CircuitBase, Solid):
...
class RectangleBase(ConvexPolyAny):
...
class Rectangle(RectangleBase, Immutable):

    '''A rectangle'''


class mRectangle(RectangleBase, Mutable):
