...
from .library import ffi, lib, attach, detach, dummy_callback, is_posix
from .loop import Loop

__all__ = ['FDType', 'close_all_handles', 'Handle']

handles = set()

FDType = int if is_posix else ffi.CData
...
class Handle:
    ...
    def send_buffer_size(self, value: int):
        c_buffer_size = ffi.new('int*', int(value / 2) if is_posix else value)
        code = lib.uv_send_buffer_size(self.uv_handle, c_buffer_size)
        if code < 0: raise UVError(code)
