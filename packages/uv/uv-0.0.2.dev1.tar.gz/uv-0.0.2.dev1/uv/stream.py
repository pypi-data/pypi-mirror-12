...
from .loop import Loop, current_loop
...
def uv_read_cb(uv_stream, length, buffer):
    ...
    with current_loop().callback_context:
...
class WriteRequest(Request):
    __slots__ = ['uv_write', 'buffers', 'callback']

    def __init__(self, buffers, callback=None):
...
@HandleType.STREAM
class Stream(Handle):
    __slots__ = ['stream', 'on_read', 'on_connection', 'ipc', 'requests']

    def __init__(self, stream, loop: Loop=None, ipc=False):
        super().__init__(stream, loop)
        self.stream = ffi.cast('uv_stream_t*', stream)
    ...
    @property
    def readable(self):
        return bool(lib.uv_is_readable(self.stream))

    @property
    def writable(self):
        return bool(lib.uv_is_writable(self.stream))
    ...
    def shutdown(self, callback=None):
        ...
        lib.uv_shutdown(request.uv_request, self.stream, uv_shutdown_cb)
        return request

    def listen(self, backlog: int=5, callback=None):
        self.on_connection = callback or self.on_connection
        lib.uv_listen(self.stream, backlog, uv_connection_cb)

    def accept(self, implementation=None):
        connection = (implementation or type(self))()
        code = lib.uv_accept(self.stream, connection.stream)
        if code < 0: raise UVError(code)
        return connection

    def read_start(self, callback=None):
        self.on_read = callback or self.on_read
        code = lib.uv_read_start(self.stream, self.loop.allocate, uv_read_cb)
        if code < 0: raise UVError(code)

    def read_stop(self):
        code = lib.uv_read_stop(self.stream)
        if code < 0: raise UVError(code)
    ...
    def write(self, data, callback=None, stream=None):
        ...
        self.requests.add(request)
        if stream is None:
            lib.uv_write(request.uv_write, self.stream, buffers, 1, uv_write_cb)
        else:
            lib.uv_write2(request.uv_write, self.stream, buffers, 1,
    ...
    def try_write(self, data):
        ...
        code = lib.uv_try_write(self.stream, buffer, 1)
        if code < 0: raise UVError(code)
        return code
