...
class TTY(Stream):
    ...
    def __init__(self, fd: int, readable: bool=True, loop: Loop=None):
        self.uv_tty = ffi.new('uv_tty_t*')
        super().__init__(self.uv_tty, loop)
        uv_file = lib.file_from_int(fd)
        code = lib.uv_tty_init(self.loop.uv_loop, self.uv_tty, uv_file, int(readable))
        if code < 0: raise UVError(code)
