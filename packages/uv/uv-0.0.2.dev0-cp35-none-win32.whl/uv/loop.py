...
def walk_callback(uv_handle, handles):
...
class Loop:
    ...
    def handles(self):
        handles = []
        lib.uv_walk(self.uv_loop, walk_callback, ffi.new_handle(handles))
        return handles
    ...
    def run(self, mode: RunMode=RunMode.DEFAULT):
        _current_loops.loop = self
        result = bool(lib.uv_run(self.uv_loop, mode))
        _current_loops.loop = None
        return result
...
def current_loop() -> Loop:
    try:
        return _current_loops.loop
