...
void py_ipv6_get_additional(struct sockaddr_in6*, unsigned long[2]);
void py_ipv6_set_additional(struct sockaddr_in6*, unsigned long, unsigned long);
*/
/* Cross-Plattform */
/*uv_uid_t uid_from_int(unsigned int);
uv_gid_t gid_from_int(unsigned int);

unsigned int uid_to_int(uv_uid_t);
unsigned int gid_to_int(uv_gid_t);

uv_file file_from_int(int);
uv_os_sock_t sock_from_int(int);*/
'''
...
void py_ipv6_get_additional(struct sockaddr_in6* addr, unsigned long result[2]) {
    result[0] = (unsigned long) addr->sin6_flowinfo;
    result[1] = (unsigned long) addr->sin6_scope_id;
}

void py_ipv6_set_additional(struct sockaddr_in6* addr, unsigned long flowinfo, unsigned long scope_id) {
...
/* Cross-Plattform */
/*uv_uid_t uid_from_int(unsigned int uid) {
    return (uv_uid_t) uid;
}
uv_gid_t gid_from_int(unsigned int gid) {
    return (uv_gid_t) gid;
}

unsigned int uid_to_int(uv_uid_t uid) {
    return (unsigned int) uid;
}
unsigned int gid_to_int(uv_gid_t gid) {
    return (unsigned int) gid;
}

uv_file file_from_int(int file) {
    return (uv_file) file;
}
uv_os_sock_t sock_from_int(int sock) {
    return (uv_os_sock_t) sock;
}*/
'''
...
def create():
    new_ffi = cffi.FFI()
    new_ffi.set_source('_uv', source, libraries=['uv'])
    new_ffi.cdef(declarations)
    return new_ffi


def verify():
    ffi = create()
    return ffi, ffi.verify(source, libraries=['uv'])
...
def extension():
    return create().distutils_extension()
