        ...
        var util = S.util,
            ...
            loadDependencies = function (functions) {
                ...
                while (functions.length) {
                    ...
                    for (dep in deps) {
                        logger.log("loadDependencies dep loop: loading script: " + dep)
                        if (deps.hasOwnProperty(dep)) {
                            ...
                            if (loaders[deps[dep]]) {
                                ...
                                if (util.startsWith(dep, moduleGooVis)) {
                                    ...
                                    deferreds.push(loaders[deps[dep]]);
                                    logger.log("loadDependencies 1: loading script: " + deps[dep]);
                                } else {
                                    // Assume dependency is a link to a javascript.
                                    loaders[deps[dep]] = $.getScript(deps[dep])
                                        .done(function () { logger.log("loadDependencies 2: loaded: " + deps[dep]); });
                                    deferreds.push(loaders[deps[dep]]);
                                    logger.log("loadDependencies 3: loading script: " + deps[dep]);
                                }
                            } else {
                                logger.log("loadDependencies 4: loading script: " + util.getObjectByPath(dep))
                            }
                        }
                    }
                }
                logger.log("loadDependencies 40: script to load: " + gVizPacks);
                logger.log("loadDependencies 50: script to load: " + loadGLib);
        ...
        var util = S.util,
            ...
            */
            ...
            draw = function (containerId) {
                ...
                logger.log(myChart + " for " + containerId + " on load.");
                $.when(saveDataTable(),
                        ...
                        function () {
                            try {
                                ...
                                if (util.startsWith(myChart, moduleGooVis)) {
                                    ...
                                    logger.log(myChart + " for " + containerId + " is not found.");
                                }
    ...
    S.container = (function () {
        ...
        var util = S.util,
            ...
            draw = function (elementID) {
                ...
                logger.log("finish id: " + elementID);
            },
