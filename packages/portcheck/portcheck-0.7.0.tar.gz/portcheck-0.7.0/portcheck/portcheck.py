...
def scan(arg):
	...
	try:
		...
		return target_ip, port,key, True, pingstatus
...
def main():
	db = MySQLdb.connect(host="localhost", # your host, usually localhost
		...
		passwd="Test100", # your password
		db="test") # name of the data base
	db.autocommit(True)

	cur = db.cursor() 

	cur.execute("SELECT * FROM portcheck ")
	...
	for target_ip, port, key, status, pingstatus in pool.imap_unordered(scan, [(row[0], row[1], row[3]) for row in addresslist]):
		...
		pingstatus_result = str(round(pingstatus, 4)) if pingstatus>0 else  "'Unreachable'"
		
		print pingstatus_result	
