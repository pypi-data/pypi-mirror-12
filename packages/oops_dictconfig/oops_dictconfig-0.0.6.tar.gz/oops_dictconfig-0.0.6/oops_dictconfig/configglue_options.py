...
class BasePublisherDescriptionOption(schema.DictOption):
    ...
    def __init__(self, **kwargs):
        if 'spec' not in kwargs:
            kwargs['spec'] = dict(
                ...
                inherit_id=schema.BoolOption(
                    help="Re-use any oops id that is already present in the oops.",
                    ),
                )
        if 'help' not in kwargs:
            kwargs['help'] = "Config for an oops publisher."
        super(BasePublisherDescriptionOption, self).__init__(**kwargs)


class FallbackChainOption(schema.ListOption):

    def __init__(self, **kwargs):
        if 'item' not in kwargs:
            kwargs['item'] = BasePublisherDescriptionOption()
        if 'help' not in kwargs:
            kwargs['help'] = ("List of oops publishers in order to publish to."
                " Each publisher will only be used if all of the publishers"
                " earlier in the list reported a failure to publish.")
        super(FallbackChainOption, self).__init__(**kwargs)


class PublisherDescriptionOption(BasePublisherDescriptionOption):

    def __init__(self, **kwargs):
        add_fallback_chain = False
        if 'spec' not in kwargs:
            add_fallback_chain = True
        super(PublisherDescriptionOption, self).__init__(**kwargs)
        if add_fallback_chain:
            self.spec['fallback_chain'] = FallbackChainOption()
