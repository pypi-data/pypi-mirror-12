...
- A traffic light example is added, featuring multiple modes and a dimmable streetlight. This example including I/O assignment and runs on a minimum of hardware: five LED's, some resistors and two buttons.

.. figure:: http://www.qquick.org/trafficlights.jpg
	:alt: Picture of hardware for the traffic lights example
	
	**Traffic lights hardware**

- The Arduino code generator now generates complete sketches, rather than code that has to be inserted into a sketch. I/O assignment and boilerplate code are taken from a file native.py in each SimPyLC Arduino project. The resulting .ino file can be loaded into the Arduino IDE and subsequently be uploaded to the Arduino. Advantage of this workflow is that all relevant code is now kept together and no editting or inserting of sketches in the IDE is needed.

Bugs fixed:

- Oneshot did not trigger properly, it does now (fixed in previous version).
