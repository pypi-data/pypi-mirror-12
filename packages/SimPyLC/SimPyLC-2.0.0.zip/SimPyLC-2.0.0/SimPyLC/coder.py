...
from base import _programDir, decapitalize, _programNameAndVersion

def generateCode (modules):
	return coder.code (modules)	
	
class Coder:
	def __init__ (self):
		self.symbolTable = {}

	def code (self, modules):
		if len (argv) < 2:
			return False
		...
		self.cDefaultInits = {
			'Marker': '{0}False'.format (self.plcPrefix),
			'Oneshot': '{0}False'.format (self.plcPrefix),
			'Latch': '{0}False'.format (self.plcPrefix),
			'Register': '0',
			'Timer': '{0}now'.format (self.plcPrefix)
		}
	...
	def dump (self):
		...
		self.targetFile = open ('{0}.{1}'.format (_programDir, 'tree'), 'w')
	...
	def generate (self):
		...
		self.targetFile = open ('{0}.{1}'.format (_programDir, 'c'), 'w')
...
//
// THIS CODE IS INTENTED SOLELY FOR EDUCATIONAL PURPOSES AND IS FUNDAMENTALLY UNSUITABLE FOR CONTROLLING REAL SYSTEMS.
...
#define {0}reset2(timer, condition) if (condition) timer = ({0}now)
#define {0}reset1(timer) timer = ({0}now)
#define {0}elapsed1(timer) ({0}now - timer)

// Support operations

#define {0}update() {0}then = {0}now; {0}now = {0}getNow(); {0}period = {0}now - {0}then; {0}first = {0}False 
...
{0}Float {0}then = 0;
{0}Float {0}period = 0;
{0}Float {0}first = {0}True;
'''
	).format (self.plcPrefix, _programNameAndVersion, datetime.now (), self.device.capitalize (), self.getPlatformMacros ())
	...
	def visit_Assign (self, node):
		if self.init:
			if node.value.func.id == 'Runner':
				...
				initialValue = (
						coder.cDefaultInits [node.value.func.id]
					if not node.value.args else
						'{0}{1}'.format (coder.plcPrefix, node.value.args [0] .id)
					if type (node.value.args [0]) == Name else
						node.value.args [0] .n
				)
					
				if node.value.func.id == 'Oneshot':
					self.emit ('{{{0}, {1}False}}'.format (initialValue, coder.plcPrefix))
				else:
					self.emit ('{0}'.format (initialValue))
