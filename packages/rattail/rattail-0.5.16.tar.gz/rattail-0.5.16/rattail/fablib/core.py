...
def rsync(production_server, owner, *paths, **kwargs):
    """
    Perform a full rsync (production -> target host) of one or more file paths.
    """
    if not paths:
        raise ValueError("Must specify one or more paths to synchronize.")
    for path in paths:
        if not path.startswith('/'):
            raise ValueError("Only absolute paths are supported (got '{0}')".format(repr(path)))

    production_user = kwargs.get('production_user') or env.user
    ssh = "ssh -p {0}{1}".format(
