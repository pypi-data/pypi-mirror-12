...
def create_mysql_db(name, checkfirst=True, user=None):
    ...
    if not checkfirst or not mysql_db_exists(name):
        ...
        if user:
            grant_mysql_access(name, user)


def grant_mysql_access(dbname, username):
    """
    ...
    """
    mysql('grant all on `{0}`.* to {1}'.format(dbname, username))
...
def create_mysql_user(name, host='localhost', password=None, checkfirst=True):
    ...
    """
    if not checkfirst or not mysql_user_exists(name, host):
