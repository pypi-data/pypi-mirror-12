...
class CheckoutOrderApi(object):
    ...
    def checkout_order_shipping_options(self, id):
        url = self.base_url + "checkoutOrders/%s/shippingOptions" % id
        contents = self.get(url)
        return contents

    def update_checkout_order_shipping_options(self, id, shipping_options):
        url = self.base_url + "checkoutOrders/%s/shippingOptions" % id
        contents = self.put(url, data_j = shipping_options)
        return contents

    def create_checkout_order_promocode(self, id, code):
