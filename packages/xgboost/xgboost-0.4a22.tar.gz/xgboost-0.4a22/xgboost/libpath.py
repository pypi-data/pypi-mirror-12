...
def find_lib_path():
    ...
    # add ./../../xgboost/wrapper/ for newest pip installation while
    # install_data will copy the compiled so to a standalone directory
    dll_path = [curr_path, os.path.join(curr_path, '../../wrapper/'),
                os.path.join(curr_path, './wrapper/'),
                os.path.join(curr_path, '../../../xgboost/wrapper/'),
                ]
    if os.name == 'nt':
