...
class GUI(object):
    ...
    def setOriginalImage(self,image=None):
        ...
        if image is None:
            ...
            #This is the correct way to do it, but it makes PyGTK rely on numpy, and the Homebrew
            #OS X distributions don't include that. To prevent having to compile from source I save
            #and read a temporary image
            #img_pixbuf=gtk.gdk.pixbuf_new_from_array(image, gtk.gdk.COLORSPACE_RGB, 8)
            
            tf = tempfile.NamedTemporaryFile()
            temp_file_name = tf.name+".tiff"
            tf.close()
            
            cv2.imwrite(temp_file_name, np.dstack((image[:,:,2],image[:,:,1],image[:,:,0])))
            img_pixbuf=gtk.gdk.pixbuf_new_from_file(temp_file_name)
