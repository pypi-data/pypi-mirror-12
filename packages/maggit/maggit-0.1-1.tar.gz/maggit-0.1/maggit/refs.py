...
class Ref(BaseRef):
    ...
    def _compute_commit(self):
       return self.repo.get_commit(self.sha)


class Branche(BaseRef):
    ...
    @property
    def sha(self):
        return self.repo.get_ref_sha('heads/%s'%self.name)
    ...
    def commit(self):
        ...
        self._sha = sha
        self._object = self.repo.get_commit(sha)
        return self._object
...
class Tag(BaseRef):
    ...
    def _compute_sha(self):
        return self.repo.get_ref_sha('tags/%s'%self.name)

    def _compute_object(self):
        return self.repo.get_object(self.sha)

    def _compute_commit(self):
        commitSha = self.repo.get_peel_ref_sha('tags/%s'%self.name)
        if commitSha:
            obj = self.repo.get_object(commitSha)
        else:
            obj = self.object
        while obj.gitType == ObjectType.tag:
            obj = obj.object
        if obj.gitType == ObjectType.commit:
            return obj
