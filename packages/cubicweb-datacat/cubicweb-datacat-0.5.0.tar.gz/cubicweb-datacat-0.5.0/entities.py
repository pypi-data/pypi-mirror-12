...
class LicenseDocument(AnyEntity):
    ...
    def dc_title(self):
        return self.license_type[0].label()
...
def add_license_to_graph(license, subject_uri, graph, reg, adapterid):
    """Add license information to RDF graph for `subject_uri`."""
    license_uri = graph.uri(license.absolute_url())
    ...
    graph.add(license_uri,
              ...
              concept_uri)
    fill_graph_with(concept, graph, adapterid)
...
class DatasetRDFAdapter(AbstractRDFAdapter):
    ...
    def fill(self, graph):
        ...
        for concept in self.entity.dataset_frequency:
            fill_graph_with(concept, graph, self.__regid__)
