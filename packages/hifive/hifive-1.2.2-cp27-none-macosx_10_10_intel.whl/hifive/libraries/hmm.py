...
class HMM:
    ...
    def __init__(self, states=2, num_distributions=1, transitions=None, pi=None, distributions=None, seed=None):
        ...
        if transitions is not None:
            ...
            self.transitions = self.rng.rand(states, states).astype(numpy.float64)
        self.transitions /= numpy.sum(self.transitions, axis=1).reshape(-1, 1)
        if pi is not None:
            self.pi = numpy.array(pi, dtype=numpy.float64)
        else:
            self.pi = self.rng.rand(states).astype(numpy.float64)
    ...
    def train(self, observations, threshold=1e-2, max_iterations=100):
        ...
        while change > threshold and iteration < max_iterations:
            ...
            for h in range(len(observations)):
                ...
                #print list(self.probs[:, :-1, :].ravel())
            #print list(new_transitions.ravel())
            #print list(new_distributions.ravel())
...
def run():
    ...
    observations = []
    for i in range(1):
        temp = hmm1.generate_sequence(25)
        observations.append(temp[1])
    #hmm1.train(observations, max_iterations=1)
    hmm2 = HMM(
        seed=2002,
        num_distributions=2,
        states=2,
        )
    hmm2.train(observations, threshold=1e-4, max_iterations=100)
    print 'pi', hmm1.pi, hmm2.pi
    for i in range(hmm1.num_states):
        print '%i transition' % i, hmm1.transitions[i, :], hmm2.transitions[i, :]
        print '%i probs' % i, hmm1.distributions[i, :], hmm2.distributions[i, :]
