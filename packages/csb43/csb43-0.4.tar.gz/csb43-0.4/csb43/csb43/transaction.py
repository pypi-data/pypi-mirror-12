...
0                '
    ...
    def __init__(self,
        ...
        self.__informationMode = informationMode
...
5                                                           '
    ...
    @check_strict(r'^\d{12}$')
    def _set_reference_1_mode_3(self, value, strict=True):
        self.__reference1 = Transaction._validate_reference(value, strict)

    @check_strict(r'^[ \w]{12}$')
    def _set_reference_1_other_modes(self, value, strict=True):
        self.__reference1 = value.rstrip(' ')

    def _set_reference_1(self, value, strict=True):
        # modalidad 1 y 2 = libre, modalidad 3 = checksum
        if str(self.informationMode) == '3':
            self._set_reference_1_mode_3(value, self.__strict)
        else:
            self._set_reference_1_other_modes(value, self.__strict)
