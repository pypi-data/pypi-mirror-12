...
def add_hooks(run_hook, get_agent_func=None):
    ...
    hook_os_system(run_hook)
    hook_subprocess_Popen(run_hook)
    hook_popen2_init(run_hook)
...
def hook_subprocess_Popen(run_hook):
    """
    Add our hook into subprocess.Popen
    """
    # Replace the original
    @monkeypatch(subprocess.Popen, "_execute_child")
...
def hook_popen2_init(run_hook):
    """
    Add our hook into popen2 and family which use _run_child internally
    """
    # Replace the original
    @monkeypatch(popen2.Popen3, "__init__")
    def _our_init(orig_init, *args, **kwargs):
        log.debug("popen2.Popen3.__init__(%(args)s, %(kwargs)s)", {
            "args": args,
            "kwargs": kwargs,
            })
