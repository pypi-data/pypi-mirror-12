...
def add_hooks(run_hook, get_agent_func=None):
    ...
    hook_queryset(get_agent_func)
    hook_fields(get_agent_func)
    hook_Q(get_agent_func)
...
def hook_queryset(get_agent_func):
    """
    ...
    while to_patch:
        ...
        _hook_queryset_class(get_agent_func, klass, Q)


def _hook_queryset_class(get_agent_func, klass, Q):
    ...
    def make_method_sig(method_name, args, kwargs):
        ...
        kwargs_sig = ",".join(["{}:{}".format(key, type(value)) for key, value
                           in kwargs.iteritems()])
        param_sig = ",".join(filter(None, [args_sig, kwargs_sig]))
        method_sig = "{}({})".format(method_name, param_sig)
        return method_sig

    # Setup new QuerySets:
    if "__init__" in klass.__dict__:
        @monkeypatch(klass, "__init__")
        def __init__(orig, self, *args, **kwargs):
            ...
            if model:
                ...
                self._immunio_path.append("{}.__init__({})".format(
    ...
    if "__getitem__" in klass.__dict__:
        ...
        def _log_limits(orig, self, k, **kwargs):
            ...
            get_path = "__getitem__({},{})".format(start, stop)
...
def hook_fields(get_agent_func):
    """
        ...
        for field in fields:
            if hasattr(field, 'attname'):
                try:
                    ...
                    vtype = type(None)
                field_types.append("{}:{}".format(field.attname, vtype))
        return "|".join(field_types)

    @receiver(pre_save, weak=False)
    def _set_fields(sender, instance, raw, update_fields, *args, **kwargs):
    ...
    def _immunio_str(self):
        ...
        for child in self.children:
            if isinstance(child, django.db.models.Q):
                ...
                k, v = child
                children_str.append("({}, {})".format(k, type(v)))
            else:
                children_str.append(str(child))
        if self.negated:
            return '(NOT: ({}: {}))'.format(self.connector,
                                            ", ".join(children_str))
        else:
            return '({}: {}'.format(self.connector, ", ".join(children_str))
