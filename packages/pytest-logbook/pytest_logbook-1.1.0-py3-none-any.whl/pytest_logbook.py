...
class LogbookPlugin:
    ...
    def pytest_runtest_makereport(self, call):
        ...
        try:
            ...
            err_repr = py.code.ExceptionInfo().getrepr()
            prev_handler.close()
            new_handler = logbook.TestHandler()
            setattr(self, 'handler_' + call.when, new_handler)
            report.outcome = 'failed'
            if report.longrepr is None:
                report.longrepr = err_repr
            report.sections.append(
                ('Logbook {} stack'.format(call.when),
