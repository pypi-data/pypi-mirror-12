...
class Connection(object):
    ...
    EMPTY, INFO, WARN, FAILED, FATAL = range(5)
    OK, OFFLINE, NO_AUTH, INVALID_CLIENT = range(4)

    def __init__(self, port=None, client=None, user=None, executable='p4', level=FAILED):
    ...
    def status(self):
        try:
            ...
            if res[0]['clientName'] == '*unknown*':
                return Connection.INVALID_CLIENT
        except errors.CommandError as err:
            if 'login' in err.args[0]:
                return Connection.NO_AUTH
