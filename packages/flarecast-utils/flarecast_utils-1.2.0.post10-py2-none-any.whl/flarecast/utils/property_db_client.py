...
class PropertyDBClient(object):
    ...
    __QUERY_PROPERTY_GROUPS_BY_OBS_DATE = '/query/%s/%s/%s'
    __QUERY_PROPERTY_GROUPS_BY_TIME_RANGE = '/query/%s/%s/%s/%s'
    __QUERY_LINKS_BY_TIME_RANGE = '/link/%s/%s/%s'
    __QUERY_LINKS_BY_FC_ID = '/link/?fc_id=%s'
    ...
    def query_by_time(self, provenance, obs_date, sirql_arguments=''):
        if sirql_arguments != '':
            sirql_arguments = '?' + sirql_arguments

        url = self.property_db_url + self.__QUERY_PROPERTY_GROUPS_BY_OBS_DATE % (provenance, obs_date, sirql_arguments)
        return self.__get_request(url)

    def query_by_time_range(self, provenance, from_date, to_date, sirql_arguments=''):
        if sirql_arguments != '':
            sirql_arguments = '?' + sirql_arguments

        url = self.property_db_url + self.__QUERY_PROPERTY_GROUPS_BY_TIME_RANGE % (provenance, from_date, to_date, sirql_arguments)
        return self.__get_request(url)
    ...
    def add_properties(self, property_group_fcid, provenance=None,
                       **properties):
    ...
    def get_links_by_time_range(self, from_date, to_date, sirql_arguments=''):
        if sirql_arguments != '':
            sirql_arguments = '?' + sirql_arguments

        url = self.property_db_url + self.__QUERY_LINKS_BY_TIME_RANGE % (from_date, to_date, sirql_arguments)
        return self.__get_request(url)

    def get_links_by_fc_id(self, fc_id):
        url = self.property_db_url + self.__QUERY_LINKS_BY_FC_ID % fc_id
        return self.__get_request(url)
