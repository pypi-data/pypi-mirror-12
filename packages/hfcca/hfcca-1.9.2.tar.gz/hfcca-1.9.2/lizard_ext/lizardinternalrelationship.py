...
class LizardExtension(object):  # pylint: disable=R0903
    ...
    class StopSplitting: pass

    def __call__(self, tokens, reader):
        try:
            while True:
                if reader.is_in_function():
                    for token in self.subcall(self.wild(tokens, reader), reader):
                        yield token
                yield tokens.next()
    ...
    def subcall(self, tokens, reader):
        fc = FunctionDependenciesCounter(reader.context.current_function)
        return fc(tokens)
