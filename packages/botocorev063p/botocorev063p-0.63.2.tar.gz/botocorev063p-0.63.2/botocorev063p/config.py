...
def multi_file_load_config(*filenames):
    ...
    for filename in filenames:
        ...
        except botocorev063p.exceptions.ConfigNotFound:
            continue
...
def raw_config_parse(config_filename):
    ...
    if path is not None:
        ...
        if not os.path.isfile(path):
            raise botocorev063p.exceptions.ConfigNotFound(path=path)
        cp = configparser.RawConfigParser()
        try:
            cp.read(path)
        except configparser.Error:
            raise botocorev063p.exceptions.ConfigParseError(path=path)
        else:
            config['_path'] = path
                ...
                for option in cp.options(section):
                    ...
                    if config_value.startswith('\n'):
                        ...
                        try:
                            ...
                            raise botocorev063p.exceptions.ConfigParseError(
                                path=path)
