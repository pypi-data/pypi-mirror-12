...
from flyforms.common import jsonify_types, UNSET
...
class Validator(with_metaclass(ABCMeta, object)):
    ...
    def is_valid(self, value):
        ...
        :return: True if :py:class`.ValidationError` was not raised, otherwise False
        :rtype: bool
        """
        try:
            self.validate(value)
...
class SimpleValidator(with_metaclass(ABCMeta, Validator)):
    ...
    def validate(self, value):
        ...
        """
        try:
            if not self.validation_case(value):   # if positive case wasn't satisfied then ValidationError will be raised
...
class TypedValidator(SimpleValidator):
    """
    Validates is given value instance of defined :code:`value_types`
    """
...
class ItemTypedValidator(SimpleValidator):
    """
    Validates is all items from iterable instance of defined :code:`value_types`
    """

    message = "Item is not type matching"

    def __init__(self, item_types):
        self.item_types = item_types

    validation_case = lambda self, value: all(isinstance(item, self.item_types) for item in value)


class JsonItemTypedValidator(ItemTypedValidator):
    """
    Validates is all items from iterable instance of :py:data:`.jsonify_types`
    """
    def __init__(self):
        super(JsonItemTypedValidator, self).__init__(jsonify_types)
