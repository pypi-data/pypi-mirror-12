...
@asyncio.coroutine
def create_localqueue_connection(protocol_factory, queue, **kwds):
    p = protocol_factory()
    t = LocalQueueTransport(queue, p)
    p.set_transport(t)
    return (t, p)


@asyncio.coroutine
def create_localqueue_server(protocol_factory, queue, **kwds):
    p = protocol_factory()
    t = LocalQueueTransport(queue, p)
    p.set_transport(t)
    return  # addr
...
class LocalQueueTransport(asyncio.Transport):
    ...
    def pause_reading(self):
        ...
        """
        if self._wait_read is not None:
            raise RuntimeError('Transport is already paused.')
        self._wait_read = asyncio.Future()
    ...
    def resume_reading(self):
        ...
        """
        if self._wait_read is None:
            raise RuntimeError('Transport is not paused.')
        fut, self._wait_read = self._wait_read, None
        fut.succeed(True)
    ...
    def set_write_buffer_limits(self, high=None, low=None):
        ...
        the low-water limit must be less than or equal to the high-water limit.
        Neither value can be negative.

        The defaults are implementation-specific.  If only the high-water limit
        is given, the low-water limit defaults to a implementation-specific
        ...
        ``resume_writing()`` to be called only once the buffer is empty.  Use
        of zero for either limit is generally sub-optimal as it reduces
    ...
    def can_write_eof(self):
        """Return ``True`` if this transport supports ``write_eof(),``
        ``False`` if not."""
        raise False
