...
def _use_cython(lang):
    if not USE_CYTHON:
        return USE_CYTHON
    try:
        import Cython
    except ImportError:
        print('Cython is not installed, defaulting to {0} files.'
              .format(lang.upper()))
        return False
    out = Cython.__version__ >= MIN_VERSION_CYTHON
    if not out:
...
class BuildSrcCommand(build_src):
    ...
    def run(self):
        ...
        if has_cython:
            ...
            new_extensions = cythonize(self.extensions, build_dir=build_dir)
