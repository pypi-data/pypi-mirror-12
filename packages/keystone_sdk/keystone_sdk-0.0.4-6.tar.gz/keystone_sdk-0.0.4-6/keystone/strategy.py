...
class KSStrategyRunner(object):
    ...
    def run(self):
        ...
        # Initialize drain
        self.drain = None
        if self.drainAddress is not None and self.synAddress is not None:
            self.drain = Drain(strategyManager)
            self.drain.initSocket(self.drainAddress, self.synAddress)
            self.drain.start()
