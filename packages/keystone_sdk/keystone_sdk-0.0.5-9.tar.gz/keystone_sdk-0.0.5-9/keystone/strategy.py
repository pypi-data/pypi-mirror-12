...
class KSStrategyRunner(object):
    ...
    def setStrategy(self, strategy):
        if not issubclass(strategy, KSStrategy):
            raise TypeError(u"strategy必须为继承自'keystone.strategy.KSStrategy'的子类。".encode('utf8'))
    ...
    def setBenchmark(self, column, sid=[]):
        if sid != [] and not isinstance(sid, (str, unicode)):
            raise TypeError(u"sid必须为字符串。".encode('utf8'))
        if not isinstance(column, (str, unicode)):
            raise TypeError(u"column必须为字符串。".encode('utf8'))
        PeriodReturn.setBenchmark(sid, column)

    def setSampleRate(self, sampleRate):
        if sampleRate not in [KSSampleRate.DAY, KSSampleRate.MINUTE, KSSampleRate.HOUR, KSSampleRate.SECOND]:
            raise TypeError(u"sampleRate格式错误。".encode('utf8'))
        self.perf.setSampleRate(sampleRate)

    def setRiskless(self, riskless):
        if not keystone.utils.isnumber(riskless):
            raise TypeError(u"riskless必须为整数。".encode('utf8'))
        self.perf.setRiskless(riskless)

    def setTradingDays(self, days):
        if not keystone.utils.isnumber(days):
            raise TypeError(u"days必须为整数。".encode('utf8'))
        self.perf.setTradingDays(days)

    def setTradingHours(self, hours):
        if not keystone.utils.isnumber(hours):
            raise TypeError(u"hours必须为整数。".encode('utf8'))
        self.perf.setTradingHours(hours)

    def run(self):
        if self.strategyClass is None:
            raise ValueError(u"can not find user strategy".encode('utf8'))
