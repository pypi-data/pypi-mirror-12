...
class KPdb(Pdb):
	def __init__(self, session, notify_url, syn_url, completekey='tab', stdin=None, stdout=None, skip=None):
	...
	def notify_frontend(self, frame):
		...
		if DatetimeBreakpoint.hit_datetime_breakpoint is None:
			...
			content = json_clean(dict(type='datetime_breakpoint', datetime=DatetimeBreakpoint.hit_datetime_breakpoint, filename=fname, lineno=lineno))
	...
	def do_break_lines(self, arg, temporary = 0):
		...
		if colon >= 0:
			...
			try:
				...
				print >>self.stdout, '***(KPdb) Bad lineno:', arg
				return
		else:
			print >>sys.stdout, "***(KPdb) Bad arguments: ", arg

	def do_unbreak_lines(self, arg, temporary = 0):
		filename = None
		lineno = None
		colon = arg.rfind(':')
		if colon >= 0:
