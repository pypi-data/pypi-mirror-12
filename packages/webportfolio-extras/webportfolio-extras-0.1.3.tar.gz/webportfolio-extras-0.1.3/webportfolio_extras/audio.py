...
def gen_waveform(src,
    ...
    :param fg_color: foreground color of the png
    :param bg_color: background color of the png
    ...
    wav2png = "wav2png {tmp_wav} " \
              "--foreground-color={fg_color} --background-color={bg_color} " \
    ...
    if not os.path.isfile(png_file) or not os.path.isfile(json_file):
        for f in [png_file, json_file]:
            if os.path.isfile(f):
                os.remove(f)
        raise IOError("Failed to save png or json file")
