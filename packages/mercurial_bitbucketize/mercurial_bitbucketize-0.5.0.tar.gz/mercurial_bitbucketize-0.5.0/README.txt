...
Install the extension as described below. Then enable it and configure
a few crucial parameters by writing in ``~/.hgrc``::
...
For all commands help is available (``hg help bb_create``),
with info about all options. 
...
PyBitBucket_ is required, as it is currently not PyPi-intallable,
...
.. note::
   ...
   Feel free to ask ``python-bitbucket`` author for PyPi distribution.
...
If you don't have Python, install one. Or work out alternative
installation method and let me know about it.
...
That extension seems interested in different parts of BitBucket_
...
Also Bitbucketize is using official Atlassian API wrapper, which is
more likely to survive future changes.
