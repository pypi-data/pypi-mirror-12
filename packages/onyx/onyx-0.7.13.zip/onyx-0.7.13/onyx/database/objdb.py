...
QUERY_ARC_UPSERT = """
SELECT upsert_dated(%s::varchar, %s::varchar, %s::date,
                    %s::jsonb, %s::varchar, %s::timestamptz);"""
QUERY_ARC_GET = """
...
CREATE OR REPLACE FUNCTION delete_obj(varchar, integer)
RETURNS boolean AS $$
DECLARE
    deleted record;
    remaining record;
...
-- store procedure for upserting dated records
CREATE OR REPLACE FUNCTION
    upsert_dated(varchar, varchar, date, jsonb, varchar, timestamptz)
RETURNS VOID AS $$
BEGIN
    LOOP
        -- first try to update the record
        UPDATE Archive
            SET Value=$4, ChangedBy=$5, TimeStamp=$6
            WHERE Name=$1 AND Attribute=$2 AND Date=$3;
        IF FOUND THEN
            RETURN;
        END IF;
        BEGIN
            INSERT INTO Archive VALUES ($1, $2, $3, $4, $5, $6);
            RETURN;
...
LANGUAGE plpgsql;""")
    ...
    def delete(self, obj):
        ...
        try:
            with self.conn.cursor() as curs:
                try:
                    curs.execute(QUERY_OBJ_DELETE, (name, obj.Version))
                    ...
                    if res is None:
                        ...
                        msg = MSG_RELOAD.format(name, obj.Version, res.version)
                        raise ObjDbError(msg)
                    else:
                        raise ObjDbError("delete on {0!s} failed without "
                                         "an obvious cause...".format(name))
    ...
    def set_dated(self, name, attr, date, value, overwrite=False):
        ...
        value_str = json.dumps(value, cls=custom_encoder)
        record_id = (name, attr, date)
        record = (name, attr, date, value_str, self.user.upper(), Date.now())

        with self.conn.cursor() as curs:
            if overwrite:
                try:
                    curs.execute(QUERY_ARC_UPSERT, record)
                except psql.IntegrityError as err:
                ...
                try:
                    ...
                    if psql_err.lookup(err.pgcode) == "UNIQUE_VIOLATION":
                        ...
                        msg = ("{0!s}. Archived tuple "
                               "is {1!s}").format(err.message, record_id)
                        raise ObjDbError(msg)
