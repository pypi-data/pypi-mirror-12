...
ylab="Observed rate of segregation between marker and phenotype", \
pch=0' + optional_str + ')'
...
def plot_histogram(data, title = '',
    ...
    for n, binned_data in enumerate(data.items()):
        ...
        if not ylim:
            ...
            # scaling copied from the original CloudMap code
            max_y_observed = max(value for series in data.values()
                                       for value in series.values())
            ylim = myround(max_observed_y + \
