    ...
    try:
        with pyvcf.open(inputfile, 'rb') as raw_vcf:
            ...
            for cov_info in raw_vcf.field_iter('DP'):
                try:
                    chr_cov_sum = cov_sum[cov_info[0]]
                except KeyError:
                    cov_sum[cov_info[0]] = [0 for sample in sample_names]
                    chr_cov_sum = cov_sum[cov_info[0]]
                    contig_lengths[cov_info[0]] = 0
                for i, coverage in enumerate(cov_info[2]):
                    chr_cov_sum[i] += int(coverage)
                contig_lengths[cov_info[0]] += 1
            # write results
            for chrom, per_sample_covs in cov_sum.items():
                cov_mean = [str(c // contig_lengths[chrom]) for c in per_sample_covs]
                out_stats.write('{0}\t{1}\n'.format(chrom, '\t'.join(cov_mean)))
    finally:
