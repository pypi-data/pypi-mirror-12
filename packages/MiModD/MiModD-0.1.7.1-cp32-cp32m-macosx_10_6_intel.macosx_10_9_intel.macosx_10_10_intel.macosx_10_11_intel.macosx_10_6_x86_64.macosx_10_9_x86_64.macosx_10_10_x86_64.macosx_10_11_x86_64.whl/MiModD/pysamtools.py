...
def header (ifile, iformat=None):
    ...
    if a_bam:
        ...
        # for bam files we delegate the job of header parsing to samtools
        call = [config.samtools_exe, 'view', '-H', ifile]
        results, errors = (s.decode() for s in subprocess.Popen(call, stdout = subprocess.PIPE, stderr = subprocess.PIPE).communicate())
        if not results.strip('\r\n'):
            # if samtools does not return results, we have to rely on
            # what it reports on stderr
            if errors:
        ...
        if iformat == 'bam':
            ...
            # for sam files we perform the header parsing ourselves
            with open(ifile, 'r') as ifo:
                ...
                for line in ifo:
                    if line.startswith('@'):
                        ...
                        break
            # All apparent header lines have been passed on,
            # but we want to make sure what we read from really
            # looks like a SAM file
            if line is None:
            ...
            if line.startswith('@') or line.strip('\r\n') == '':
                # This is or could be a file with only header lines.
                # We are done.
                return
            # Check if the first line not starting with an @
            # can possibly be a valid SAM body line.
            maybe_sam = True
            first_body_line_fields = line.split('\t')
            # Does it have at least 11 tab-separated fields?
            if len(first_body_line_fields) < 11:
                maybe_sam = False
            else:
                # Can the second field be parsed as an int
                # like a valid FLAG field?
                try:
                    flag = int(first_body_line_fields[1])
                except ValueError:
                    maybe_sam = False
            if not maybe_sam:
                if iformat == 'sam':
                    raise FormatParseError(
                        'The input does not look like a SAM file.'
                        )
                elif iformat is None:
                    raise FormatParseError(
                        'The input does not look like a SAM or BAM file.'
                        )
