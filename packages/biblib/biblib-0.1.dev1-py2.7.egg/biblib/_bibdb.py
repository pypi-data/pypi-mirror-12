...
class BibDB(object):
    ...
    def __init__(self, ListOfEntryObj=None, method=None):
        ...
        #: Counter style for the citation-key template: ``alpha`` (*default* a,b,...,z) or ``num`` (1,..,1000).
    ...
    #def next(self):
        #"""
        #*Iterator function*: Returns the next BibTeX Entry object from the database.

        #:return: BibTeX Entry object
        #:rtype:  :class:`.Entry.article` | :class:`.Entry.book` | ...
        #"""
    ...
    def add_entry(self, entryObj, ckey=None, method=None):
        ...
        * ``None``: (default) a invalid or conflicting citation-key will raise a KeyError.
        ...
        * ``'force'``: Use :attr:`.Entry.ckey` of *entryObj* or *ckey*  as citation-key. If the it is already in used, the old Entry object will be replaced. If it is invalid it will raise a KeyError.
    ...
    def del_entry(self, ckey):
        ...
        :raises NameError: if citation-key did not exist in database
        """
        if ckey not in self._data:
            msg = 'Citation-key "' + ckey + '" did not exist in database!'
            raise NameError(msg)
        del self._data[ckey]
    ...
    def proposeCKey(self, entryObj):
        ...
        try:
            dataDict.update( entryObj.AuthorsList()[0] )
        except NameError:
            dataDict.update( dict(given="Jon", family="Dow") )
        dataDict['cnt'] = ''
    ...
    def merge_bibdb(self, dbObj, method=None):
        ...
        out = {}
        for ckey, eObj in dbObj._data.items():
    ...
    def bibtex(self):
        ...
        """
        out=''
        for ckey, entryObj in self._data.items():
