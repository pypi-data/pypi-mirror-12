...
class BaseReporter(object):
    ...
    def summary(self):
        ...
        for bench in self.runner.runned:
            key = self.key(bench)
            runs = {}
            for method, results in list(bench.results.items()):
...
class CsvReporter(FileReporter):
    ...
    def output(self, out):
        ...
        for row in list(self.summary().values()):
            for run in list(row['runs'].values()):
                writer.writerow((
                    row['name'],
                    run['name'],
                    row['times'],
    ...
    def with_sizes(self, *headers):
        ...
        for row in list(summary.values()):
            ...
            for idx, field in [(3, 'total'), (4, 'mean')]:
                ...
                max_length = max(float_len(r) for r in list(row['runs'].values()))
...
class MarkdownReporter(FileReporter, FixedWidth):
    ...
    '''
    def output(self, out):
        for bench in list(self.with_sizes('', *self.headers).values()):
            ...
            # Table body
            for run in list(bench['runs'].values()):
