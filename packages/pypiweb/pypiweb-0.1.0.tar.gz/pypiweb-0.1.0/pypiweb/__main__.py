...
import sys
sys.path.append("C:\\Users\\faoustin\\Documents\\workspace\\pypiweb")


DEFAULT_SERVER = "auto"

log = logging.getLogger('pypiweb.main')
...
def usage():
    return dedent("""\
  pypiweb [OPTIONS] [PACKAGES_DIRECTORY...]
   fork of pypiserver: add interface web pypi
    start PyPI compatible package server serving packages from
    PACKAGES_DIRECTORY. If PACKAGES_DIRECTORY is not given on the
    command line, it uses the default ~/packages.  pypiweb scans this
    directory recursively for packages. It skips packages and
    directories starting with a dot. Multiple package directories can be
    specified.

  pypiweb understands the following options:
  ...
  pypiweb --version
    show pypiweb's version

  pypiweb -U [OPTIONS] [PACKAGES_DIRECTORY...]
  ...
  Visit https://pypi.python.org/pypi/pypiweb for more information.
  """)
...
def main(argv=None):
    ...
    for k, v in opts:
        if k in ("-p", "--port"):
            ...
            from pypiweb import __version__
            print("pypiweb %s\n" % __version__)
            return
        elif k == "-U":
            command = "update"
    ...
    if command == "update":
        from pypiweb.manage import update_all_packages
        update_all_packages(
    ...
    from pypiweb.bottle import server_names, run
    if server not in server_names:
        sys.exit("unknown server %r. choose one of %s" % (
            server, ", ".join(server_names.keys())))

    from pypiweb import __version__, app
    a=app(
        root=roots,
    ...
    log.info("This is pypiweb %s serving %r on http://%s:%s\n\n",
