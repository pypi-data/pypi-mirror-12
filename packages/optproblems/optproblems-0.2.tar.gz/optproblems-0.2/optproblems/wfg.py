...
def s_multi(y, a, b, c):
    ...
    return correct_to_01((1.0 + math.cos(tmp2) + 4.0 * b * math.pow(tmp1, 2.0)) / (b + 2.0))
...
def mixed(x, a, alpha):
    ...
    assert alpha > 0.0
    tmp = 2.0 * a * math.pi
    return correct_to_01(math.pow(1.0 - x[0] - math.cos(tmp * x[0] + math.pi / 2.0) / tmp, alpha))
...
def disc(x, a, alpha, beta):
    ...
    return correct_to_01(1.0 - math.pow(x[0], alpha) * math.pow(math.cos(tmp1), 2.0))
...
class WFGBaseProblem(MultiObjectiveTestProblem):
    ...
    def __init__(self,
        ...
        # maxObjectiveLimits = None
        # minObjectiveLimits = None
        # if num_objectives == 3:
        #     maxObjectiveLimits = [10, 10, 10]
        #     minObjectiveLimits = [0, 0, 0]
        # elif num_objectives == 5:
        #     maxObjectiveLimits = [10, 10, 10, 10, 12]
        #     minObjectiveLimits = [0, 0, 0, 0, 0]
    ...
    def get_optimal_solutions(self, max_number):
        ...
        for pos_params in param_combis:
            solutions.append(self.optimal_solution(self.k, l, pos_params))
        # fill up with random optimal solutions
        while len(solutions) < max_number:
            solutions.append(self.rand_optimal_solution(self.k, l, rand_generator))
