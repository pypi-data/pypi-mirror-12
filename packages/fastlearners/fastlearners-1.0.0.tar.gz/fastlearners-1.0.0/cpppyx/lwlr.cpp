...
_cLwlr::_cLwlr(int dim_x, int dim_y, int k, double sigma, _cNNSet* nnset) {
    this->dim_x    = dim_x;
    this->dim_y    = dim_y;
    this->k        = k;
    this->sigma    = sigma;
...
void _cLwlr::predict_y(double xq[], double yq[], int k, double sigma_sq) {
    ...
    assert(this->nnset->size >= k);

    vector<int>    index;
    vector<double> dists;
    this->nnset->nn_x(k, xq, dists, index);
    ...
    for (int i = 0; i < knn; i++) {
        ...
        for (int j = 0; j < this->dim_x; j++) {
            this->_x[(this->dim_x+1)*i+j+1] = this->nnset->get_xi(idxi, j);
        }
        for (int j = 0; j < this->dim_y; j++) {
            this->_y[(this->dim_y)*i+j] = this->nnset->get_yi(idxi, j);
        }
    }
...
// k is the number of effective neighbors in nnset->_index and nnset->_dists
