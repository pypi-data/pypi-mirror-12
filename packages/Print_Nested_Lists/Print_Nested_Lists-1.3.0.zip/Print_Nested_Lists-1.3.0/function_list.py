...
is used to create a tab-stop when a nested list is encountered. Adds an
option to turn on/off tab-stops"""

def print_list(the_list, indent=False, level=0): #level option enable to turn tab_stop off/on
        for each_item in the_list:
                if isinstance(each_item, list):
                        print_list(each_item, indent, level+1) #second argument increments level by 1
                else:
                        if indent:
