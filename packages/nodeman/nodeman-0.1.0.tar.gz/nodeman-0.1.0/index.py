...
def cli(version):
    ...
    if version:
        click.echo(require('nodeman')[0].version)
...
def sync():
    ...
    for version in installed_versions():
        click.echo(':: installing for', version)
...
def rm(version):
    ...
    if not os.path.exists(STORAGE + '/' + version):
        click.echo(':: %s is not installed' % version)
        return

    click.echo(':: deleting binary...')
...
def install(version, link='', from_ctx=False):
    ...
    if os.path.exists(STORAGE + '/' + version):
        click.echo(':: %s is already installed' % version)
        return

    if not from_ctx:
        link, version = extract_link(version)

    click.echo(':: downloading...v%s' % version)
...
def use(version):
    ...
    if not os.path.exists(STORAGE + '/' + version):
        click.echo(':: version', version, 'is not installed.')
        sys.exit(1)
