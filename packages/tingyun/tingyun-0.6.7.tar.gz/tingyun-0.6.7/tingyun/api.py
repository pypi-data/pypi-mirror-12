...
import tingyun.armoury.trigger.wsgi_django
import tingyun.armoury.ammunition.function_tracker


"""
   Description: Init the agent for detecting all of the python plugins that can be detected(detected plugins can be
                configured in config file with section([tingyun:exclude]) and  key(plugins), more detail listed in
                agent doc settings chapter.)
...
"""
    ...
    Usage:
        ...
        @wsgi_app_decorator(framework='undefined')
    ...
    Params:
        framework: decorator has default value-`undefined` for this parameter. this param indicate witch framework
                   your application used.
"""
wsgi_app_decorator = tingyun.armoury.trigger.wsgi_django.wsgi_application_decorator
