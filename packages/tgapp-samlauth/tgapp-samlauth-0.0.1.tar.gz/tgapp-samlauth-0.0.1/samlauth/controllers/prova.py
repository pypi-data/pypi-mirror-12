...
class ACS(Service):
    ...
    def do(self, response, binding, relay_state="", mtype="response"):
        ...
        logger.info("AVA: %s - %s" % (self.response.name_id, self.response.ava))
...
if __name__ == '__main__':
    ...
    # Use keyfile also for encryption as mod_shib2 requires enc
    # remember to remove use="signature" attr from SP xml
    SP.sec.enc_key_files = [SP.config.key_file]

    add_urls()

    SRV = make_server(HOST, PORT, application)
    #SRV.socket = ssl.wrap_socket(SRV.socket, keyfile='pki/mykey.pem', certfile='pki/mycert.pem', server_side=True)
