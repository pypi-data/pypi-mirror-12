...
def submit_aggregates(aggregates, dry_run=False):
    ...
    rdd.foreachPartition(lambda x: _upsert_build_id_aggregates(_group_by_iterator(x), connection_string, dry_run=dry_run))

    submission_date_count, rdd = _repartition_by_key(submission_date_aggregates.map(lambda x: (x[0][:3], x)))
    rdd.foreachPartition(lambda x: _upsert_submission_date_aggregates(_group_by_iterator(x), connection_string, dry_run=dry_run))
...
def _upsert_build_id_aggregates(aggregates, connection_string, dry_run=False):
...
def _upsert_submission_date_aggregates(aggregates, connection_string, dry_run=False):
    conn = _create_connection(autocommit=False, connection_string_override=connection_string)
    cursor = conn.cursor()
