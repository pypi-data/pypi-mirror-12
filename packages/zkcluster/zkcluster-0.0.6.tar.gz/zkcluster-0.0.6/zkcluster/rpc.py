...
class RPCService(object):
    ...
    def message_received(self, message, expected=None, debug=None):
        match = re.match(
            "^RPC_(REQ|RESP|ERR|EVT)\((\d+), (.+?)\)\: (.*)", message)
        if not match:
            return
        log.debug("received RPC message %s %s", message, debug)
        ...
        if message_type == 'REQ':
            ...
            raise exc.RPCError(
                "Expected only request of type %s, received %s %s" %
                (expected, message, debug))
